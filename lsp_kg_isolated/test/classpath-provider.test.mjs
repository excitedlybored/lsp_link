...
} from '../dist/artifact/classpath-provider.js';
...
test('M2E and Buildship use the classpaths imported by JDT LS and merge provider evidence', async (t) => {
  ...
  const context = {
    ...
    adapter, documentUris: ['file:///workspace/project/src/Main.java'],
  };
  const registry = new ArtifactClasspathProviderRegistry([
...
test('generic JDT provider falls back to a representative document URI', async (t) => {
  ...
  const resolution = await new ArtifactClasspathProviderRegistry([
...
test('explicit manifests normalize relative artifact metadata', async (t) => {
  ...
  const descriptors = await new ExplicitClasspathManifestProvider().resolve({
