...
} from './classpath-provider.js';

const execFileAsync = promisify(execFile);

export async function crawlJvmArtifacts(input: {
  lspRunId: string;
  ...
  for (const descriptor of input.artifacts) {
      ...
      if (existingArtifact) {
        ...
        registerArtifactJars(artifactsByJar, existingArtifact, classpathEntryPath, headerJarPath, binaryJarPath);
        continue;
      }
      const source = descriptor.sourceJarPath
        ? { path: path.resolve(descriptor.sourceJarPath), origin: 'provided' as const }
        : await findSourceJar(crawlJarPath, input.cacheDirectory, fetchSources);
      ...
      registerArtifactJars(artifactsByJar, artifact, classpathEntryPath, headerJarPath, binaryJarPath);
      batch.relations.push(relation(stageId, 'JvmArtifactEnrichmentRun', stageId,
        'JvmArtifact', artifact.id, 'HAS_ARTIFACT', batch.artifacts.length - 1));

      try {
        const entries = await jarEntries(crawlJarPath);
        for (const entry of entries) {
          ...
          batch.relations.push(relation(stageId, 'JvmArtifact', artifact.id,
            'JvmClass', clazz.id, 'CONTAINS_CLASS', artifact.classCount - 1));
        }
      } catch { errors += 1; }
  }

  const seeds = externalSeedClasses(input.lspBatch, artifactsByJar);
  ...
  try { providerVersion = String((await execFileAsync(javap, ['-version'])).stdout).trim(); } catch { errors += 1; }
  ...
  while (queue.length > 0 && visited.size < maxClasses) {
    ...
    try {
      const { stdout } = await execFileAsync(javap, ['-classpath', classpath, '-p', '-s', '-c', ...names], {
        maxBuffer: 64 * 1024 * 1024,
      });
      const targets = parseJavap(String(stdout), stageId, batch, classByName);
  ...
  for (const line of output.split(/\r?\n/)) {
    ...
    if (classMatch) {
      ...
      if (clazz) {
        ...
        if (clazz.superName) addClassRelation(
          batch, stageId, clazz, clazz.superName, 'BYTECODE_SUPERCLASS', classByName,
        );
        clazz.interfaces.forEach((name, index) => addClassRelation(
    ...
    if (instruction && currentMethod) {
      ...
      const targetMethod = targetClass
        ? ensureMethod(batch, stageId, targetClass, instruction[4], instruction[5], undefined, true)
        : undefined;
      const callSite = {
      ...
      batch.relations.push(relation(stageId, 'JvmMethod', currentMethod.id,
        'JvmCallSite', callSite.id, 'HAS_BYTECODE_CALLSITE', batch.callSites.length - 1));
      if (targetMethod) batch.relations.push(relation(stageId, 'JvmCallSite', callSite.id,
    ...
    if (descriptor && pending) {
      if (pending.method) {
        ...
        currentMethod = ensureMethod(batch, stageId, clazz, name, descriptor, pending.declaration, false);
      } else {
        ...
        if (!batch.fields.some((value) => value.id === field.id)) {
          ...
          batch.relations.push(relation(stageId, 'JvmClass', clazz.id,
...
async function jarEntries(jarPath: string): Promise<string[]> {
  const { stdout } = await execFileAsync(findJar(), ['tf', jarPath], { maxBuffer: 64 * 1024 * 1024 });
  return String(stdout).split(/\r?\n/).filter(Boolean);
}

async function findSourceJar(
  artifactJar: string,
  cacheDirectory: string,
  ...
  const sibling = globSync('*sources*.jar', { cwd: path.dirname(artifactJar), absolute: true })[0]
    ?? gradleSourceJar(artifactJar, coordinate);
...
function safeClasspathEntry(value: string, artifactId: string, cacheDirectory: string): string {
...
function dedupe(batch: JvmArtifactBatch): JvmArtifactBatch {
