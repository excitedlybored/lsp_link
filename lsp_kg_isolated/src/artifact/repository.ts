...
export class JvmArtifactRepository {
  ...
  async writeBatch(batch: JvmArtifactBatch): Promise<void> {
    ...
    try {
      await this.insertRows('JvmArtifactEnrichmentRun', batch.runs.map(runRow));
      await this.insertRows('JvmArtifact', batch.artifacts.map(artifactRow));
      await this.insertRows('JvmClass', batch.classes.map(classRow));
      await this.insertRows('JvmMethod', batch.methods.map(methodRow));
      await this.insertRows('JvmField', batch.fields.map(fieldRow));
      await this.insertRows('JvmCallSite', batch.callSites);
