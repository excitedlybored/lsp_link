...
/**
 * Complete, LSP-first crawl for one build root. It never consults Tree-sitter
...
export async function crawlLspBuildRoot(input: CompleteCrawlInput): Promise<LspObservationBatch> {
  ...
  const support = capabilitySupport(capabilities);
  const coverage = new Map<string, CoverageState>();
  for (const capability of LSP_KG_CAPABILITIES) {
    coverage.set(capability, emptyCoverage(capability, support.get(capability) ?? false));
  }
  ...
  excludeCoverage(coverage, 'textDocument/semanticTokens/full/delta',
    'cold snapshot uses textDocument/semanticTokens/full; no prior resultId exists');
  excludeCoverage(coverage, 'textDocument/semanticTokens/range',
    'full-document semantic tokens are requested for every opened document');
  excludeCoverage(coverage, 'workspace/diagnostic',
  ...
  for (const document of documents) {
    ...
    await observe(coverage, 'textDocument/documentSymbol', async () => {
      await adapter.openDocument(filePath);
      document.wasOpened = true;
      try {
        const observations = await adapter.documentSymbols(filePath);
        const context = ingestionContext(run, server, document, 'textDocument/documentSymbol');
  ...
  for (const document of documents) {
    ...
    try {
      for (const symbol of symbols) {
        ...
        await crawlLocations(adapter, registry, coverage, batch, run, server, document, symbol,
        ...
        await crawlHover(adapter, coverage, batch, run, server, document, symbol, filePath, position);

        if (CALLABLE_KINDS.has(symbol.kind)) {
          await crawlCalls(adapter, registry, coverage, batch, run, server, document, symbol, filePath, 'outgoing');
          await crawlCalls(adapter, registry, coverage, batch, run, server, document, symbol, filePath, 'incoming');
        }
        if (TYPE_KINDS.has(symbol.kind)) {
          await crawlTypeHierarchy(adapter, registry, coverage, batch, run, server, document, symbol, filePath, 'supertypes');
          await crawlTypeHierarchy(adapter, registry, coverage, batch, run, server, document, symbol, filePath, 'subtypes');
        }
      }

      await crawlSemanticTokens(adapter, registry, coverage, batch, run, server, document, capabilities);
      await crawlSemanticTokenPositions(adapter, registry, coverage, batch, run, server, document, filePath, symbols);
      await crawlSignatureHelp(adapter, coverage, batch, run, server, document, filePath);
      await crawlDocumentDiagnostics(adapter, coverage, batch, run, server, document);
      drainPublishedDiagnostics(adapter, coverage, batch, run, server, document);
    } finally {
      await adapter.closeDocument(filePath);
    }
  }
  ...
  const coverageBatch = coverageToBatch(run, server, coverage);
  return dedupeObservationBatch(mergeObservationBatches(batch, coverageBatch));
}

async function crawlLocations(
  adapter: CompleteCrawlAdapter,
  ...
  await observe(coverage, capability, async () => {
    ...
    for (const [ordinal, location] of locations.entries()) {
      ...
      const occurrence = ingestOccurrence(
        ingestionContext(run, server, occurrenceDocument, capability),
        {
      ...
      appendBatch(batch, occurrence);
      if (target) {
        mapped += 1;
        if (emitImplementationRelation) {
          batch.relations.push(makeMappedSymbolRelation(
            ingestionContext(run, server, document, capability), target, source, 'implementation',
          ));
        }
      }
    }
...
async function crawlCalls(
  ...
  await observe(coverage, capability, async () => {
    ...
    const observed = ingestCalls(
      ingestionContext(run, server, document, capability), direction, normalized,
    );
    appendBatch(batch, observed);
    return { resultCount: observed.callSites.length, mappedCount: observed.callSites.length, externalCount: external };
  });
}

async function crawlTypeHierarchy(
  adapter: CompleteCrawlAdapter,
  registry: SymbolRegistry,
  coverage: Map<string, CoverageState>,
  batch: LspObservationBatch,
  ...
  await observe(coverage, capability, async () => {
    ...
    for (const [ordinal, item] of (items ?? []).entries()) {
      ...
      batch.relations.push(direction === 'supertypes'
        ? makeMappedSymbolRelation(ingestionContext(run, server, document, capability), source, related, 'type_super')
        : makeMappedSymbolRelation(ingestionContext(run, server, document, capability), related, source, 'type_super'));
      const occurrenceDocument = registry.documentForUri(item.uri) ?? document;
      appendBatch(batch, ingestOccurrence(
        ingestionContext(run, server, occurrenceDocument, capability),
        {
...
async function crawlSemanticTokens(
  adapter: CompleteCrawlAdapter,
  registry: SymbolRegistry,
  coverage: Map<string, CoverageState>,
  batch: LspObservationBatch,
...
async function crawlSignatureHelp(
  adapter: CompleteCrawlAdapter,
  coverage: Map<string, CoverageState>,
  batch: LspObservationBatch,
  run: LspAnalysisRun,
...
async function crawlSemanticTokenPositions(
  adapter: CompleteCrawlAdapter,
  registry: SymbolRegistry,
  coverage: Map<string, CoverageState>,
  batch: LspObservationBatch,
  ...
  for (const token of tokens) {
    ...
    await crawlPositionLocations(adapter, registry, coverage, batch, run, server, document,
      filePath, position, 'textDocument/definition', 'definition');
    await crawlPositionLocations(adapter, registry, coverage, batch, run, server, document,
      filePath, position, 'textDocument/declaration', 'declaration');
    ...
    await crawlPositionHover(adapter, coverage, batch, run, server, document, filePath, position);
  }
}

async function crawlPositionLocations(
  adapter: CompleteCrawlAdapter,
  registry: SymbolRegistry,
  coverage: Map<string, CoverageState>,
  batch: LspObservationBatch,
...
async function crawlPositionHover(
  adapter: CompleteCrawlAdapter,
  coverage: Map<string, CoverageState>,
  batch: LspObservationBatch,
  run: LspAnalysisRun,
...
async function crawlDocumentDiagnostics(
  adapter: CompleteCrawlAdapter,
  coverage: Map<string, CoverageState>,
  batch: LspObservationBatch,
  run: LspAnalysisRun,
  ...
  await observe(coverage, capability, async () => {
    ...
    const diagnostics = ingestDiagnostics(run, server, document, capability, raw?.items ?? [], batch);
    return { resultCount: diagnostics };
  });
}

function drainPublishedDiagnostics(
  adapter: CompleteCrawlAdapter,
  coverage: Map<string, CoverageState>,
  batch: LspObservationBatch,
  run: LspAnalysisRun,
  ...
  const state = requireCoverage(coverage, capability);
  ...
  const count = matching.reduce((total, notification) =>
    total + ingestDiagnostics(run, server, document, capability, notification.diagnostics ?? [], batch), 0);
...
class SymbolRegistry {
  ...
  addBatch(batch: LspObservationBatch): void {
    ...
    this.symbols.push(...batch.symbols);
  }
...
function capabilitySupport(capabilities: Record<string, unknown>): Map<string, boolean> {
...
function countBatch(batch: LspObservationBatch): ObservationCounts {
  return {
...
function appendBatch(target: LspObservationBatch, source: LspObservationBatch): void {
  for (const key of Object.keys(target) as Array<keyof LspObservationBatch>) {
    (target[key] as unknown[]).push(...(source[key] as unknown[]));
  }
}

function emptyCoverage(capability: string, supported: boolean): CoverageState {
  return {
...
function excludeCoverage(states: Map<string, CoverageState>, capability: string, reason: string): void {
  requireCoverage(states, capability).exclusionReason = reason;
}

function requireCoverage(states: Map<string, CoverageState>, capability: string): CoverageState {
  const state = states.get(capability);
  if (!state) throw new Error(`Capability ${capability} is absent from the complete inventory`);
  return state;
}

function coverageStatus(state: CoverageState): LspCoverage['status'] {
