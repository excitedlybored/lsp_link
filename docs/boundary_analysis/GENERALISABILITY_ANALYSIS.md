...
### Pillar 1: Declarative SDK Configuration ([`sdk_registry.json`](file:///Users/zijie-machine/code_ai/ide_link/custom_tools/sdk_registry.json))
...
* **Mitigation**: Add the corporate wrapper regex pattern to `custom_tools/sdk_registry.json` using `uv run python custom_tools/ingress_egress_analyzer.py add-sdk`.
