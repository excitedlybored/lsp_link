...
WHERE declares.kind = 'DECLARES_METHOD' AND owner.kind = 'interface'
  AND (list_contains(method.annotations, $workflowEntryPointType)
...
RETURN DISTINCT owner.id AS ownerId, owner.simpleName AS ownerName,
       owner.packageName AS packageName, method.id AS methodId,
       method.name AS methodName, method.descriptor AS signature,
       owner.sourceEntry AS uri, -1 AS startLine,
       CASE WHEN list_contains(method.annotations, $workflowEntryPointType) THEN 'workflow'
         ...
         ELSE 'activity' END AS methodRole,
       CASE WHEN list_contains(method.annotations, $workflowEntryPointType) THEN $workflowEntryPointType
         ...
         ELSE $activityContractType END AS semanticType,
       method.id AS evidenceId, 1.0 AS confidence
