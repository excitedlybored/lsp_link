 ...
 */
export function* causeChain(err, maxDepth = CAUSE_CHAIN_MAX_DEPTH) {
    let current = err;
 ...
 */
export function findInCauseChain(err, match, maxDepth = CAUSE_CHAIN_MAX_DEPTH) {
 ...
 */
export const stripWindowsLongPathPrefix = (p, platform = process.platform) => {
    if (platform !== 'win32')
        return p;
 ...
 */
export async function mapConcurrent(items, run, options = {}) {
