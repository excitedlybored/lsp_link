 ...
 */
export const saveMeta = async (metaDir, meta) => {
...
const reconcileMetaDir = async (dir) => {
    ...
    if (!primary && !legacy) {
        ...
        for (const filename of [INDEX_METADATA_FILE, LEGACY_METADATA_FILE]) {
            try {
                ...
                logger.warn({ dir, filename }, 'Metadata file exists but is unreadable/corrupt; leaving as-is (next successful analyze rewrites it)');
            }
            catch {
    ...
    if (primary && legacy) {
        ...
        logger.info({ dir, winner: winner === legacy ? LEGACY_METADATA_FILE : INDEX_METADATA_FILE }, 'Reconciled diverged metadata files (fresher indexedAt wins, written to both)');
        return true;
    }
 ...
 */
export const reconcileMetadataFiles = async (repoPath) => {
    const storagePath = getStoragePath(repoPath);
    ...
    for (const branchDir of branchDirs) {
        ...
        try {
            ...
            logger.warn({ branchDir, err }, 'Skipping branch directory during metadata reconciliation (non-critical)');
        }
    }
    return changed;
};
...
export const ensureGitNexusIgnored = async (repoPath) => {
    ...
    try {
        ...
        if (isReadOnlyFilesystemError(err)) {
            logger.warn({ path: gitignorePath, code: err.code }, 'GitNexus storage filesystem is not writable; skipping .gitnexus/.gitignore. Cache files may appear as untracked in this repo locally.');
        }
        else {
            throw err;
        }
...
const ensureGitInfoExclude = async (repoPath) => {
    ...
    try {
        ...
        if (isReadOnlyFilesystemError(err)) {
            logger.warn({ path: excludePath, code: err.code }, 'GitNexus storage filesystem is not writable; skipping .git/info/exclude update. .gitnexus/ cache directory may appear as untracked in `git status` locally.');
        }
        else {
            throw err;
        }
 ...
 */
const getRegistryLockDir = () => path.join(getGlobalDir(), 'registry-lock');
/**
 ...
 */
const withRegistryLock = async (operation) => {
    let lock = null;
    try {
        lock = await acquireIndexLock(getRegistryLockDir(), {
            ...
            onWaitStart: () => logger.info('Waiting for another GitNexus process to finish a registry update…'),
        });
    }
    catch (err) {
        if (!(err instanceof IndexLockTimeoutError))
            throw err;
        logger.warn({ timeoutMs: REGISTRY_LOCK_TIMEOUT_MS }, 'Timed out waiting for the global registry lock; proceeding without it. A concurrent registry write may be lost.');
    }
    try {
 ...
 */
const writeRegistry = async (entries, attempts) => {
    await fs.mkdir(getGlobalDir(), { recursive: true });
    await writeFileAtomic(getGlobalRegistryPath(), JSON.stringify(sanitizeEntries(entries), null, 2), attempts);
};
/**
 ...
 */
const registerRepoUnlocked = async (repoPath, meta, opts) => {
...
export const registerRepo = async (repoPath, meta, opts) => withRegistryLock(() => registerRepoUnlocked(repoPath, meta, opts));
/**
 * Remove a repo from the global registry.
 * Called after `gitnexus clean`.
 */
const unregisterRepoUnlocked = async (repoPath) => {
...
export const unregisterRepo = async (repoPath) => withRegistryLock(() => unregisterRepoUnlocked(repoPath));
/**
 ...
 */
const removeBranchIndexUnlocked = async (repoPath, branch) => {
    const resolved = canonicalizePath(repoPath);
    const entries = await readRegistry();
...
export const removeBranchIndex = async (repoPath, branch) => withRegistryLock(() => removeBranchIndexUnlocked(repoPath, branch));
/**
...
export const listRegisteredRepos = async (opts) => {
    ...
    for (const entry of entries) {
        ...
        if (indexFound) {
            ...
            logger.warn({ name: entry.name, storagePath: entry.storagePath, code: firstNonMissingError?.code }, 'Keeping registry entry despite fs.access failure (not provably absent); not pruning to avoid mass registry wipe.');
            valid.push(entry);
        }
    }
    ...
    if (valid.length !== entries.length) {
        const pruned = new Set(entries.filter((entry) => !valid.includes(entry)).map((entry) => entry.path));
        try {
            await withRegistryLock(async () => {
                const fresh = await readRegistry();
            ...
            logger.warn({ err, prunedCount: pruned.size }, 'Could not persist the pruned global registry; continuing with the in-memory pruned view.');
        }
    }
    return valid;
};
