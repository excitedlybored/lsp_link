...
export const getScopeIndexStoreDir = (storagePath) => path.join(storagePath, STORE_DIRNAME);
/**
 ...
 */
export const clearScopeIndexStore = (storagePath) => {
 ...
 */
export const persistScopeShards = (storagePath, scopes) => {
 ...
 */
export class DiskBackedScopeTree {
    dir;
    skeleton;
    maxResidentShards;
    /** Decoded shards, most-recently-used last (Map preserves insertion order). */
    lru = new Map();
    constructor(storagePath, skeleton, maxResidentShards = 64) {
    ...
    get byId() {
        throw new Error('DiskBackedScopeTree.byId is unsupported (INGESTION_EMIT_SCOPES is incompatible with GITNEXUS_DISK_SCOPE_INDEX).');
    }
    has(id) {
    ...
    loadShard(shard) {
        ...
        const fileScopes = JSON.parse(raw, makeInterningReviver(pool, defPool));
 ...
 */
export class TransitionalScopeTree {
    resident;
    disk = null;
    constructor(scopes) {
    ...
    seal(storagePath, maxResidentShards) {
        if (this.disk !== null || this.resident === null)
            return;
    ...
    get active() {
        const t = this.disk ?? this.resident;
        if (t === null)
            throw new Error('TransitionalScopeTree has no active backing (resident dropped without seal).');
        return t;
    }
    get size() {
