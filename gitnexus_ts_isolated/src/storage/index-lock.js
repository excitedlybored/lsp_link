...
const noopHandle = (record) => ({ record, release: () => { } });
/**
 ...
 */
export const sweepStagingArtifacts = (lockDir, log) => {
...
const jitteredDelay = (pollMs, timeoutMs, waited) => {
 ...
 */
const resolveTimeoutMs = (opt) => {
    const raw = typeof opt === 'number'
        ? opt
        : (() => {
    ...
    for (;;) {
        ...
        if (holder) {
            ...
            if (!announcedWait) {
                ...
                opts.log?.(`Another gitnexus analyze (pid ${holder.pid} on ${holder.hostname}) is ` +
                    `refreshing this index — waiting for it to finish.`);
            }
            if (waited >= timeoutMs)
                throw new IndexLockTimeoutError(holder, waited);
            if (Date.now() - lastDiagnosticAt >= DIAGNOSTIC_INTERVAL_MS) {
                lastDiagnosticAt = Date.now();
                if (waited >= DIAGNOSTIC_INTERVAL_MS) {
                    opts.log?.(`Still waiting for analyze pid ${holder.pid} (${Math.round(waited / 1000)}s elapsed).`);
                }
            }
...
const canonicalizeDir = (p) => {
    ...
    for (;;) {
        try {
            ...
            if (parent === dir)
                return resolved; // reached the root with nothing to resolve
...
const tryListen = (server, name) => new Promise((resolve) => {
    const onError = (err) => {
 ...
 */
const acquireViaSocket = async (lockDir, me, opts) => {
    ...
    for (;;) {
        ...
        if (listenErr.code !== 'EADDRINUSE')
            throw new SocketLockUnavailable(listenErr);
        if (!announcedWait) {
            announcedWait = true;
            opts.onWaitStart?.(me);
...
const selectBackend = () => {
    ...
    if (override === 'socket' || override === 'file')
        return override;
 ...
 * the lock.
 */
export const acquireIndexLock = async (lockDir, opts = {}) => {
    ...
    if (selectBackend() === 'socket') {
        try {
            ...
            if (!(err instanceof SocketLockUnavailable))
                throw err; // timeout etc. propagate
