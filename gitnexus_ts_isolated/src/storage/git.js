 ...
 */
export const snapshotSelfCommitSafety = (repoPath, candidateFiles) => {
 ...
 */
export const selfCommitContextFiles = (repoPath, candidateFiles, preRunSafety) => {
    ...
    if (skippedDirty.length > 0) {
        logger.warn({ files: skippedDirty }, 'gitnexus: --self-commit skipping file(s) with uncommitted changes from before this analyze run');
    }
    if (safe.length === 0)
        return;
    try {
    ...
    try {
        execFileSync('git', ['commit', '-m', 'chore(gitnexus): refresh index stats [skip ci]', '--', ...safe], { cwd: repoPath, stdio: 'ignore', windowsHide: true });
    }
    catch (err) {
 ...
 */
export const stripUrlCredentials = (url) => url.replace(/^(https?:\/\/)[^/]*@/i, '$1');
/**
...
const gitInfoExcludePathCache = new Map();
const coreExcludesFilePathCache = new Map();
/**
 ...
 */
export const getGitInfoExcludePath = (fromPath) => {
 ...
 */
export const getCoreExcludesFilePath = (fromPath) => {
    const cached = coreExcludesFilePathCache.get(fromPath);
    if (cached !== undefined)
        return cached;
    let result;
    try {
        const configured = chompGitOutput(execSync('git config --get --type=path core.excludesFile', {
            cwd: fromPath,
            stdio: ['ignore', 'pipe', 'ignore'],
            windowsHide: true,
        }));
 ...
 */
export function coalesceHunks(hunks) {
    if (hunks.length === 0)
        return [];
 ...
 * sort rather than K.
 */
export function coalesceHunksByPath(fileDiffs) {
    ...
    for (const fileDiff of fileDiffs) {
        ...
        if (ranges.length > 0)
            rawByPath.set(fileDiff.filePath, ranges);
    }
    const byPath = new Map();
    for (const [filePath, ranges] of rawByPath)
        byPath.set(filePath, coalesceHunks(ranges));
    return byPath;
}
/**
 ...
 * base.
 */
export function hunksOverlapRange(coalesced, startLine, endLine) {
