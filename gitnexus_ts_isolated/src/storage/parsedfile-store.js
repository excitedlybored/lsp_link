 ...
 */
export const makeInterningReviver = (pool, defPool) => {
    return (key, value) => {
        ...
        if (revived !== null &&
            ...
            typeof revived.nodeId === 'string' &&
            typeof revived.filePath === 'string') {
            const def = revived;
...
export const forceGc = () => {
    ...
    if (cachedGc === undefined) {
        ...
        try {
            ...
            cachedGc = vm.runInNewContext('gc');
...
export const getParsedFileStoreDir = (storagePath) => path.join(storagePath, STORE_DIRNAME);
...
function sanitizeCallableFlowSites(value) {
    ...
    if (!Array.isArray(value))
        return undefined;
    const bounded = value.length > MAX_CALLABLE_FLOW_SITES_PER_FILE
 ...
 */
function sanitizeReceiverChains(value) {
    if (value === undefined)
        return { sites: undefined, dropped: 0 };
    if (!Array.isArray(value))
        return undefined;
    ...
    if (dropped === 0)
        return { sites: value, dropped: 0 };
    const sites = value.map((site) => {
        if (!isRecord(site) || site.receiverChain === undefined)
            return site;
        if (isValidReceiverChain(site.receiverChain))
            return site;
...
function isValidCallableFlowSite(value) {
    if (!isRecord(value) || typeof value.kind !== 'string')
        return false;
    switch (value.kind) {
        case 'seed':
            return (isValidOperand(value.destination) &&
        ...
        case 'formal':
            return (isBoundedString(value.ownerName) &&
                ...
                isValidExpectedSignature(value.expectedSignature));
        case 'argument':
            return (isValidRange(value.callSite) &&
                isSafeIndex(value.parameterIndex) &&
...
function isValidOperand(value) {
    ...
    return (isBoundedString(value.name) &&
        ...
        value.indirection >= 0 &&
        value.indirection <= MAX_CALLABLE_FLOW_INDIRECTION &&
...
function isValidExpectedSignature(value) {
    if (value === undefined)
        return true;
    if (!isRecord(value))
        return false;
    return ((value.parameterCount === undefined || isSafeIndex(value.parameterCount)) &&
...
function isValidBoundedStringArray(value) {
    return (value === undefined ||
        (Array.isArray(value) &&
            ...
            value.every((entry) => typeof entry === 'string' && entry.length <= MAX_CALLABLE_FLOW_NAME_LENGTH)));
}
function isValidRange(value) {
    if (!isRecord(value))
        return false;
    return (isNonNegativeInteger(value.startLine) &&
...
function isBoundedString(value) {
    return (typeof value === 'string' && value.length > 0 && value.length <= MAX_CALLABLE_FLOW_NAME_LENGTH);
}
...
function isSafeIndex(value) {
    return isNonNegativeInteger(value) && value <= MAX_CALLABLE_FLOW_PARAMETERS;
}
/** Durable store dir — a sibling of `parsedfile-store/`, NEVER cleared per run. */
export const getDurableParsedFileDir = (storagePath) => path.join(storagePath, DURABLE_DIRNAME);
const durableChunkDir = (durableDir, chunkHash) => path.join(durableDir, chunkHash);
/**
 ...
 */
export const prepareDurableParsedFileChunk = async (durableDir, chunkHash) => {
