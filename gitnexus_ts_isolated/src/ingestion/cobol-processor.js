...
export const processCobol = (graph, files, allPathSet) => {
    ...
    for (const file of programs) {
        ...
        if (file.content.length > MAX_COBOL_FILE_SIZE) {
            logger.warn(`[cobol-processor] Skipping oversized file (${(file.content.length / 1024 / 1024).toFixed(1)}MB > ${(MAX_COBOL_FILE_SIZE / 1024 / 1024).toFixed(0)}MB): ${file.path}`);
            continue;
        }
...
function stripMoveSubscript(name) {
