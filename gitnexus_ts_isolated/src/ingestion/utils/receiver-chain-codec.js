...
const NAME_FREE_KINDS = new Set(['await', 'index']);
const KIND_BY_SIGIL = new Map(Object.entries(SIGIL_BY_KIND).map(([kind, sigil]) => [sigil, kind]));
...
function isEncodableSegment(value) {
 ...
 */
export function encodeReceiverChain(baseReceiverName, steps, options) {
    if (steps.length === 0 || steps.length > MAX_CHAIN_DEPTH)
        return undefined;
    if (!isEncodableSegment(baseReceiverName))
        return undefined;
    ...
    for (const step of steps) {
        ...
        if (!isEncodableSegment(step.name))
            return undefined;
 ...
 */
export function decodeReceiverChain(value) {
    if (typeof value !== 'string')
        return undefined;
    if (value.length === 0 || value.length > MAX_RECEIVER_CHAIN_BYTES)
        return undefined;
    const parts = value.split(SEPARATOR);
    if (parts.length < 3)
        return undefined; // version + base + at least one step
    if (parts[0] !== VERSION)
        return undefined;
    ...
    if (truncated)
        stepParts = stepParts.slice(0, -1);
    if (stepParts.length === 0 || stepParts.length > MAX_CHAIN_DEPTH)
        return undefined;
    const steps = [];
    for (const part of stepParts) {
        ...
        if (NAME_FREE_KINDS.has(kind)) {
            ...
            if (name.length > 0)
                return undefined;
            steps.push({ kind: kind });
            continue;
        }
        if (!isEncodableSegment(name))
            return undefined;
        steps.push({ kind: kind, name });
    }
...
export function isValidReceiverChain(value) {
