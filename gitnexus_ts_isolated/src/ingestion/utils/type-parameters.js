 ...
 */
export function parseTypeParameterList(text) {
...
function parseEntry(entry) {
    ...
    const bound = boundAt === undefined ? undefined : head.slice(boundAt.index + boundAt.length).trim();
    ...
    if (matched === undefined || matched === null || name === undefined)
        return undefined;
...
function beforeTopLevelDefault(entry) {
    let depth = 0;
 ...
 */
function findBoundIntroducer(head) {
    let depth = 0;
...
function isWholeWord(text, index, length) {
