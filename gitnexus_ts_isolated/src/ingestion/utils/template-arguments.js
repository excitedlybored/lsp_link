 ...
 */
function balancedTailList(text, start) {
 ...
 */
export function matchingOpenParen(text) {
    if (!text.endsWith(')'))
        return -1;
    let depth = 0;
...
export function stripTrailingCallSuffix(text) {
 ...
 */
export function erasedTypeApplication(typeRef) {
    const spelling = typeRef.declaredSpelling?.trim();
    if (spelling === undefined)
        return undefined;
    const base = typeRef.rawName.trim();
    if (base.length === 0 || !spelling.startsWith(base))
        return undefined;
