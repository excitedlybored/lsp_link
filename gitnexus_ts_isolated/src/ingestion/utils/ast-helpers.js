 ...
 */
export const qualifyByEnclosingModScope = (node, rawText) => {
    const modSegments = [];
 ...
 */
export const isQualifiableScopeLabel = (nodeLabel) => nodeLabel === 'Trait';
/**
...
export const isSuppressedConcreteTypedefDuplicate = (captureMap, concreteTypedefRanges) => {
    const definitionNode = getDefinitionNodeFromCaptures(captureMap);
    return (definitionNode?.type === 'type_definition' &&
...
export const isValueDefinitionLabel = (label) => VALUE_DEFINITION_LABELS.has(label);
/**
 ...
 */
export const buildDefinitionPreScan = (matches, provider) => {
    ...
    for (const match of matches) {
        ...
        if (label === null || isValueDefinitionLabel(label))
            continue;
        const key = `${definitionNode.startIndex}:${nameNode.text}`;
        nonValue.add(key);
        if (isOverloadableCallable(label))
            callable.add(key);
    }
 ...
 */
export const MEMBER_OWNER_NODE_TYPES = new Set([
 ...
 */
export function walkNamedTree(node, cb) {
    cb(node);
 ...
 */
export function findSplitBodyCallableAncestor(node, signatureOnlyTypes, boundaryTypes) {
    let current = node.parent;
    while (current !== null) {
        if (boundaryTypes.has(current.type))
            return null;
...
export const javaLocalTypeDeclarationContainer = (node) => {
...
const isJavaLocalTypeNode = (node) => javaLocalTypeDeclarationContainer(node) !== null;
 ...
 * file path.
 */
export const synthesizeJavaTypeIdentity = (node) => {
        ...
        if (CLASS_CONTAINER_TYPES.has(current.type)) {
            ...
            if (nameNode) {
                ...
                const qualifiedClassId = qualifiedOwnerName != null && qualifiedOwnerName !== nameNode.text
                    ? generateId(label, `${filePath}:${templateArguments !== undefined
 ...
 */
export const isReturnShapeProperty = (node) => {
    let current = node;
    let objectDepth = 0;
    while (current && objectDepth === 0) {
        if (current.type === 'object')
            objectDepth = 1;
        else if (FUNCTION_NODE_TYPES.has(current.type))
            return false;
        else
            current = current.parent;
    }
...
export const findReturnShapeOwnerInfo = (node, filePath) => {
    ...
    if (literal.parent?.type !== 'return_statement')
        return null;
    ...
    let fn = literal.parent.parent;
    while (fn && !FUNCTION_NODE_TYPES.has(fn.type))
        fn = fn.parent;
    if (!fn)
        return null;
    ...
    if (declarator?.type === 'variable_declarator') {
        ...
        if (declName?.type === 'identifier')
            return { ownerName: declName.text };
    }
    void filePath;
    return null;
};
export const findObjectLiteralBindingInfo = (node, filePath, options) => {
 ...
 */
export const findMemberAssignmentOwnerInfo = (node, filePath) => {
 ...
 */
export const prototypeAssignmentOwnerName = (assignment) => {
 ...
 */
export const thisAssignmentOwnerName = (assignment) => {
    const left = callableAssignmentTarget(assignment);
    if (left === null)
        return null;
 ...
 */
export const isPrototypeMemberAssignmentNode = (node) => prototypeAssignmentOwnerName(node) !== null || isThisMemberAssignmentNode(node);
/**
 ...
 * `module.exports`.
 */
export const isCjsDefaultExportAssignment = (node) => {
...
export const isThisMemberAssignmentNode = (node) => {
 ...
 */
export const LOCAL_SCOPE_BODY_NODE_TYPES = new Set([...FUNCTION_NODE_TYPES]
 ...
 */
export const SPLIT_SIGNATURE_NODE_TYPES = new Set([...FUNCTION_NODE_TYPES].filter((t) => !LOCAL_SCOPE_BODY_NODE_TYPES.has(t)));
// ============================================================================
...
export const stripBidiAndZeroWidth = (text) => Array.from(text)
    .filter((ch) => {
 ...
 */
export function nodeIfType(node, ...types) {
