 ...
 * separate call sites.
 */
function isChainableReceiverNode(node) {
    return (CALL_EXPRESSION_TYPES.has(node.type) ||
 ...
 */
const TRANSPARENT_RECEIVER_WRAPPERS = new Map([
...
function isTransparentReceiverWrapper(node) {
...
function unwrapTransparentReceiver(node) {
    let current = node;
...
export function extractMixedChain(receiverNode) {
    ...
    while (chain.length < MAX_CHAIN_DEPTH) {
        ...
        if (CALL_EXPRESSION_TYPES.has(current.type)) {
            ...
            const inner = current.childForFieldName?.('argument') ??
                current.childForFieldName?.('expression') ??
                current.namedChildren?.find((c) => c !== null) ??
                null;
            if (!inner)
                break;
            ...
            const obj = subscriptBase(current) ??
                current.childForFieldName?.('object') ??
