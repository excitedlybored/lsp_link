 ...
 */
import { nodeToCapture, syntheticCapture } from './ast-helpers.js';
/**
 ...
 */
export function synthesizeCallableFlowCaptures(root, options) {
    ...
    for (const fn of functions)
        emitFormalFacts(fn, options, out);
...
function collectFunctions(nodes, options) {
    const out = [];
    for (const node of nodes) {
...
function defaultFunctionName(node, options) {
    ...
    if (parent !== null && options.bindingNodeTypes.has(parent.type)) {
        ...
        if (destination !== undefined)
            return bindingIdentifier(destination, options)?.text;
    }
    return undefined;
}
function functionParameters(node, options) {
    ...
    if (explicit === null)
        return [];
    const direct = explicit.namedChildren.filter((child) => child !== null &&
...
function collectAssignments(nodes, options) {
    const out = [];
    const seen = new Set();
    for (const node of nodes) {
...
function buildValueBindingIndex(nodes, assignments, functions, options) {
    const assignmentRegionIdsByName = new Map();
    const formalByOwner = new Map();
    const signatureByNameAndRegion = new Map();
    const add = (index, owner, name) => {
    ...
    for (const assignment of assignments) {
        ...
        if (terminal !== undefined)
            destinationNames.add(terminal.text);
        for (const name of destinationNames) {
            ...
            if (options.functionScopedValueBindings === true) {
                ...
                if (functionOwner !== undefined)
                    regionIds.add(functionOwner.id);
            }
        }
    }
    for (const fn of functions) {
        for (const parameter of fn.parameters) {
            ...
            if (binding !== undefined)
                add(formalByOwner, fn.node.id, binding.text);
        }
    }
    for (const node of nodes) {
        if (options.callableSignatureDeclarationNodeTypes?.has(node.type) !== true)
            continue;
        const binding = bindingIdentifier(node, options);
        const signature = options.expectedSignature?.(node, node);
        if (binding === undefined || signature === undefined)
            continue;
        ...
        if (!bindingInterposesValueDeclarator(node, binding))
            continue;
        ...
        if (options.functionScopedValueBindings === true) {
            ...
            if (functionOwner !== undefined)
                byRegion.set(functionOwner.id, signature);
        }
    }
...
function bindingInterposesValueDeclarator(declaration, binding) {
    let node = binding.parent;
    while (node !== null && node.id !== declaration.id) {
        if (node.type.includes('pointer') || node.type.includes('parenthesized'))
            return true;
        node = node.parent;
    }
    return false;
}
function nearestFunctionOwner(input, options) {
...
function nearestLexicalRegion(input, options) {
    const providerOwner = options.lexicalFunctionOwner?.(input);
    let node = input.parent;
    let outermost = input;
    while (node !== null) {
        outermost = node;
        if (providerOwner !== undefined && node.id === providerOwner.id)
            return node;
        if (options.functionNodeTypes.has(node.type) || isLexicalRegionNode(node))
            return node;
        node = node.parent;
    }
    return providerOwner ?? outermost;
}
function isLexicalRegionNode(node) {
    return /(?:^|_)(?:block|body|compound_statement|source_file|program|module|script)(?:$|_)/.test(node.type);
}
function isVisibleValueBinding(input, name, bindings, options) {
    ...
    let node = input;
    while (node !== null) {
        if (assignmentRegionIds?.has(node.id) === true)
            return true;
        if (options.functionNodeTypes.has(node.type) &&
    ...
    if (bindings.formalByOwner.get(undefined)?.has(name) === true)
        return true;
...
function visibleCallableSignature(input, name, bindings, options) {
...
function assignmentParts(node, options) {
    ...
    const destination = node.childForFieldName('left') ??
    ...
    if (destination === null || source === null) {
        for (const child of node.namedChildren) {
            if (child === null || child.id === node.id)
                continue;
            if (!options.bindingNodeTypes.has(child.type))
                continue;
...
function emitAssignmentFact(assignment, knownCallableNames, valueBindings, options, out) {
    const destination = operandSyntax(assignment.destination, options, options.bindingNodeTypes.has(assignment.container.type));
    ...
    const sourceIsValueBinding = isVisibleValueBinding(assignment.source, source.name, valueBindings, options);
    if (options.bareNamesAreCalls === true &&
    ...
    const sourceIsKnownCallable = source.callableReference ||
        source.anonymousCallable ||
        (source.directDesignator &&
            !sourceIsValueBinding &&
    ...
    if (destination.indirection > 0) {
        out.push(binaryFact('store', assignment.container, 'pointer', destination, 'source', flowSource));
        return;
    }
    if (source.addressOf && !sourceIsKnownCallable) {
        ...
        if (sourceIsValueBinding) {
            out.push(binaryFact('address', assignment.container, 'destination', destination, 'source', source));
        }
            ...
            out.push(binaryFact('address', assignment.container, 'destination', destination, 'source', source));
            out.push(binaryFact('copy', assignment.container, 'destination', destination, 'source', source));
        }
        return;
    }
    if (source.indirection > 0 && !sourceIsKnownCallable) {
        out.push(binaryFact('load', assignment.container, 'destination', destination, 'pointer', source));
        return;
    }
    if (sourceIsKnownCallable) {
        const match = {
            ...
            '@callable-flow.target-name': syntheticCapture('@callable-flow.target-name', flowSource.node, flowSource.anonymousCallable ? destination.name : flowSource.name),
        };
        if (flowSource.qualifiedName !== undefined) {
            match['@callable-flow.target-qualified-name'] = syntheticCapture('@callable-flow.target-qualified-name', flowSource.node, options.normalizeQualifiedName?.(flowSource.qualifiedName) ?? flowSource.qualifiedName);
        }
    ...
    const kind = options.isTrueReferenceBinding?.(assignment.container, assignment.destination)
        ? 'alias'
        : 'copy';
    out.push(binaryFact(kind, assignment.container, 'destination', destination, 'source', flowSource));
}
function emitFormalFacts(fn, options, out) {
    for (let index = 0; index < fn.parameters.length; index++) {
        ...
        const match = {
            ...
            '@callable-flow.parameter-index': syntheticCapture('@callable-flow.parameter-index', binding, String(index)),
            '@callable-flow.passing-mode': syntheticCapture('@callable-flow.passing-mode', parameter, mode),
        };
        addSignatureCaptures(match, parameter, options.expectedSignature?.(parameter, parameter));
        out.push(match);
    }
}
function emitCallFacts(call, knownCallableNames, valueBindings, options, out) {
    const calleeNode = options.extractCallCallee?.(call) ??
    ...
    const calleeIsValueBinding = callee !== undefined && isVisibleValueBinding(call, callee.name, valueBindings, options);
    const directCalleeName = member === undefined &&
        callee !== undefined &&
    ...
    for (let index = 0; index < args.length; index++) {
        ...
        const sourceIsValueBinding = isVisibleValueBinding(args[index], rawSource.name, valueBindings, options);
        if (options.bareNamesAreCalls === true &&
        ...
        const sourceIsKnownCallable = rawSource.callableReference ||
            rawSource.anonymousCallable ||
            (rawSource.directDesignator &&
                !sourceIsValueBinding &&
        ...
        out.push({
            ...
            ...(source.indirection > 0
                ? {
                    '@callable-flow.source-indirection': syntheticCapture('@callable-flow.source-indirection', source.node, String(source.indirection)),
                }
                : {}),
            ...(source.addressOf
                ? {
                    '@callable-flow.source-address': syntheticCapture('@callable-flow.source-address', source.node, 'true'),
                }
                : {}),
            '@callable-flow.parameter-index': syntheticCapture('@callable-flow.parameter-index', args[index], String(index)),
            ...(directCalleeName !== undefined
                ? {
                    '@callable-flow.direct-callee-name': syntheticCapture('@callable-flow.direct-callee-name', callee.node, directCalleeName),
                }
                : {}),
        });
    }
    if (member !== undefined) {
        if (member.operator !== undefined &&
            options.memberPointerOperators?.has(member.operator) === true) {
            emitInvoke(callSite, member.member, 'member-pointer', args.length, out, options, member.receiver);
            return;
        }
    ...
    if (callee.indirection > 0 || calleeIsValueBinding) {
        const invokeCallee = !calleeIsValueBinding && knownCallableNames.has(callee.name)
            ? asCallableDesignator(callee)
            : callee;
        emitInvoke(callSite, invokeCallee, 'indirect', args.length, out, options);
    }
}
function emitInvoke(call, callee, invocationKind, arity, out, options, receiver) {
    out.push({
        ...
        ...(callee.indirection > 0
            ? {
                '@callable-flow.callee-indirection': syntheticCapture('@callable-flow.callee-indirection', callee.node, String(callee.indirection)),
            }
            : {}),
        ...(receiver !== undefined
            ? {
            ...
            : {}),
        '@callable-flow.invocation-kind': syntheticCapture('@callable-flow.invocation-kind', call, invocationKind),
...
function callArguments(call, options) {
    const list = call.childForFieldName('arguments') ??
        call.childForFieldName('argument') ??
        call.namedChildren.find((child) => child !== null && options.parameterListNodeTypes.has(child.type)) ??
        findFirstDescendantOfTypes(call, options.parameterListNodeTypes) ??
        null;
    if (list === null)
        return [];
    if (options.parameterListNodeTypes.has(list.type)) {
        return list.namedChildren.filter((child) => child !== null && child.type !== 'comment');
    }
    return [list];
}
function directCallMemberParts(call, options) {
    ...
    if (receiverNode === null || memberNode === null)
        return undefined;
...
function memberParts(input, options) {
    ...
    const receiverNode = node.childForFieldName('object') ??
        node.childForFieldName('argument') ??
        node.childForFieldName('receiver');
    const memberNode = node.childForFieldName('property') ??
    ...
    if (receiver === undefined || member === undefined)
        return undefined;
    const operator = node.children.find((child) => options.memberPointerOperators?.has(child.type) === true ||
        options.memberPointerOperators?.has(child.text) === true);
    return { receiver, member, ...(operator !== undefined ? { operator: operator.text } : {}) };
}
function operandSyntax(input, options, declarationBinding = false) {
    ...
    if (providerReference !== undefined) {
        return {
            ...
            anonymousCallable: false,
            expressionKind: providerReference.qualifiedName === undefined ? 'callable-designator' : 'bound-member',
        };
    }
    let node = input;
    let indirection = 0;
    ...
    const valueProducingExpression = /call|invocation|creation|new_expression|composite_literal/.test(node.type);
    return {
        name: id.text,
        node: id,
        indirection,
        ...
        directDesignator: options.identifierNodeTypes.has(node.type) ||
...
function asCallableDesignator(operand) {
    if (operand.expressionKind !== 'binding')
        return operand;
    return { ...operand, expressionKind: 'callable-designator' };
}
function wrappedExpression(node) {
    const field = node.childForFieldName('argument') ??
        node.childForFieldName('expression') ??
        node.childForFieldName('value');
    if (field !== null && field.id !== node.id && node.namedChildCount === 1)
        return field;
...
export function subscriptBase(node) {
    if (node.childForFieldName('index') === null)
        return null;
    return (node.childForFieldName('argument') ?? // C/C++ subscript_expression
...
function bindingIdentifier(node, options) {
    if (options.identifierNodeTypes.has(node.type))
        return node;
    const base = subscriptBase(node);
    if (base !== null)
        return bindingIdentifier(base, options);
    for (const fieldName of ['name', 'declarator', 'pattern', 'left']) {
        const field = node.childForFieldName(fieldName);
        if (field === null || field.id === node.id)
            continue;
...
function terminalIdentifier(node, options) {
    if (options.identifierNodeTypes.has(node.type))
        return node;
    const base = subscriptBase(node);
    if (base !== null)
        return terminalIdentifier(base, options);
...
function hasMultipleIdentifierLeaves(node, options) {
    let count = 0;
    const stack = [node];
    while (stack.length > 0) {
...
function findFirstDescendantOfTypes(root, types) {
    const stack = [...root.namedChildren].filter((child) => child !== null);
    while (stack.length > 0) {
...
function binaryFact(kind, anchor, leftRole, left, rightRole, right) {
    return {
        ...
        ...(left.indirection > 0
            ? {
                [`@callable-flow.${leftRole}-indirection`]: syntheticCapture(`@callable-flow.${leftRole}-indirection`, left.node, String(left.indirection)),
            }
            : {}),
        ...
        ...(right.indirection > 0
            ? {
                [`@callable-flow.${rightRole}-indirection`]: syntheticCapture(`@callable-flow.${rightRole}-indirection`, right.node, String(right.indirection)),
            }
            : {}),
    };
}
function operandCapture(name, operand) {
    return syntheticCapture(name, operand.node, operand.name);
}
function operandMetadataCaptures(role, operand) {
    return {
        [`@callable-flow.${role}-kind`]: syntheticCapture(`@callable-flow.${role}-kind`, operand.node, operand.expressionKind),
        ...(operand.qualifiedName !== undefined
            ? {
                [`@callable-flow.${role}-qualified-name`]: syntheticCapture(`@callable-flow.${role}-qualified-name`, operand.node, operand.qualifiedName),
            }
            : {}),
    };
}
function addSignatureCaptures(match, anchor, signature) {
    if (signature?.parameterCount !== undefined) {
        match['@callable-flow.expected-arity'] = syntheticCapture('@callable-flow.expected-arity', anchor, String(signature.parameterCount));
    }
    if (signature?.parameterTypes !== undefined) {
        match['@callable-flow.expected-types'] = syntheticCapture('@callable-flow.expected-types', anchor, JSON.stringify(signature.parameterTypes));
    }
    if (signature?.parameterTypeClasses !== undefined) {
        match['@callable-flow.expected-type-classes'] = syntheticCapture('@callable-flow.expected-type-classes', anchor, JSON.stringify(signature.parameterTypeClasses));
    }
    if (signature?.isConst !== undefined) {
        match['@callable-flow.expected-const'] = syntheticCapture('@callable-flow.expected-const', anchor, String(signature.isConst));
    }
}
