...
function isCallMatch(grouped) {
    for (const tag of Object.keys(grouped)) {
        if (tag.startsWith(CALL_TAG_PREFIX))
            return true;
    }
    return false;
}
 ...
 */
export function synthesizeReceiverChainCapture(grouped, receiverNode) {
    if (receiverNode === undefined || receiverNode === null)
        return;
    if (grouped['@reference.receiver-chain'] !== undefined)
        return;
    if (!isCallMatch(grouped))
        return;
    ...
    if (encoded === undefined)
        return;
    grouped['@reference.receiver-chain'] = syntheticCapture('@reference.receiver-chain', receiverNode, encoded);
}
