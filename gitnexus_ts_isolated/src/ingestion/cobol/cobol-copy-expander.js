        ...
        for (let ci = copyStatements.length - 1; ci >= 0; ci--) {
            ...
            if (visited.has(resolvedPath)) {
                if (!warnedCircular.has(resolvedPath)) {
                    ...
                    logger.warn(`[cobol-copy-expander] Circular COPY detected: ${cs.target} (${resolvedPath}) ` +
            ...
            if (depth >= maxDepth) {
                logger.warn(`[cobol-copy-expander] Max expansion depth (${maxDepth}) reached for ` +
            ...
            if (++totalExpansions > MAX_TOTAL_EXPANSIONS) {
                if (!warnedCircular.has('__max_total__')) {
                    ...
                    logger.warn(`[cobol-copy-expander] Max total expansions (${MAX_TOTAL_EXPANSIONS}) reached ` +
