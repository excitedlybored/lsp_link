...
export function resolveReferenceSites(input) {
    ...
    for (const site of scopes.referenceSites) {
        ...
        if (site.kind === 'value-ref')
            continue;
        sitesProcessed++;
        const resolutions = lookupForSite(site, classRegistry, methodRegistry, fieldRegistry, macroRegistry, scopes);
 ...
 */
function lookupForSite(site, classRegistry, methodRegistry, fieldRegistry, macroRegistry, scopes) {
    switch (site.kind) {
        case 'call': {
            const opts = {
        ...
        case 'type-reference': {
            ...
            if (bindsTypeParameter(site.inScope, site.name, scopes))
                return [];
            return classRegistry.lookup(site.name, site.inScope);
        }
        case 'read':
        case 'write': {
            ...
            const receiverlessFieldHits = site.explicitReceiver === undefined
                ? fieldHits.filter((hit) => hit.def?.type !== 'Property')
                : fieldHits;
            if (receiverlessFieldHits.length > 0)
                return receiverlessFieldHits;
