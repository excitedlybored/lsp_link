...
function simpleDefinitionName(def) {
...
function containsPosition(node, row, column) {
 ...
 */
export function boundCallableStartRow(definitionNode, nodeName, nodeLabel, localDefs, nameNode) {
    if (localDefs === undefined)
        return definitionNode.startPosition.row;
    const origin = nameNode?.startPosition ?? definitionNode.startPosition;
    let best;
    let tied = false;
    for (const def of localDefs) {
        if (def.type !== nodeLabel || simpleDefinitionName(def) !== nodeName)
            continue;
        const position = definitionIdPosition(def.nodeId, def.filePath);
        if (position === undefined)
            continue;
        const row = position.line - 1;
        if (!containsPosition(definitionNode, row, position.column))
            continue;
        const distance = Math.abs(row - origin.row) * 1_000_000 + Math.abs(position.column - origin.column);
 ...
 */
export const localIdentity = (node, name) => `${name}@${node.startPosition.row}:${node.startPosition.column}`;
/**
 ...
 */
export const nestedCallableQualifiedName = (prefix, node, name) => `${prefix}.${localIdentity(node, name)}`;
