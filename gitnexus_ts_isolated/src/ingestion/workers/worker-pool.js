 ...
 */
export function buildDispatchMessage(items) {
...
export function startHeartbeatStallTracker() {
    let totalStallMs = 0;
    let last = Date.now();
 ...
 */
export function resolveWorkerHeapCapMb(poolSize) {
    return (positiveInteger(process.env.GITNEXUS_WORKER_HEAP_MB) ??
...
export const createWorkerPool = (workerUrl, poolSize, options) => {
    ...
    if (poolCommitMb > 0.6 * effectiveMb) {
        logger.warn({ poolSize: size, workerHeapCapMb, effectiveMb }, `Worker pool may overcommit memory: ${size} workers × ${workerHeapCapMb}MB heap cap exceeds 60% of the ${effectiveMb}MB available to this process. Reduce GITNEXUS_WORKER_POOL_SIZE or set GITNEXUS_WORKER_HEAP_MB.`);
    }
    ...
    const dispatch = async (items, onProgress, chunkHash) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            const retireWorkerAfterTimeout = (worker, workerIndex, reason) => {
                ...
                worker.unref?.();
                logger.warn({ workerIndex, reason }, `Worker ${workerIndex} timed out; retiring without immediate terminate to avoid aborting native parser state.`);
            };
            ...
            const replaceWorker = async (workerIndex, mode = 'terminate', reason = 'replacing worker') => {
                ...
                try {
                    ...
                    logger.warn({ workerIndex, error: err instanceof Error ? err.message : String(err) }, `Worker ${workerIndex} replacement failed to come online; dropping slot.`);
                    return false;
                }
                if (stopped) {
                    await replacement.terminate().catch(() => undefined);
            ...
            const requeueRemainder = (job, excluded) => {
                ...
                if (excluded.length === 0) {
                    ...
                    if (firstPath !== undefined) {
                        ...
                        logger.warn({ startIndex: job.startIndex, firstPath, deaths }, `Conceptual job ${job.startIndex} died ${deaths} times unattributably; ` +
                        ...
                        logger.warn({ startIndex: job.startIndex, deaths }, `Conceptual job ${job.startIndex} died ${deaths} times unattributably with ` +
                    ...
                    idleTimer = setTimeout(() => {
                        if (!settled) {
                            ...
                            if (!stallCreditUsed && stallMs >= job.timeoutMs * STALL_CREDIT_FRACTION) {
                                ...
                                logger.warn({ workerIndex, stallMs: Math.round(stallMs), timeoutMs: job.timeoutMs }, `Worker ${workerIndex} idle timeout overlapped a main-thread stall (GC pressure); re-arming once instead of retiring.`);
                                resetIdleTimer();
                                return;
                            }
                            settled = true;
