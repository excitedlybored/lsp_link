...
import { extractTemplateArguments, stripTrailingCallSuffix, typeApplicationArguments, } from './utils/template-arguments.js';
...
export function extract(matches, filePath, provider) {
    ...
    for (let i = 0; i < scopeDrafts.length; i++) {
        ...
        if (d.parent === null && d.kind !== 'Module') {
            ...
            scopeDrafts[i] = makeDraft(d.id, moduleScope.id, d.kind, d.range, d.filePath, d.ownsReceivers, d.lexicalNames);
        }
    }
    ...
    const parsedImports = [];
    pass3CollectImports(partitioned.import_, positionIndex, filePath, parsedImports, provider, scopeTree);
...
function topicsOf(match) {
    const topics = new Set();
    for (const name of Object.keys(match)) {
        if (name.startsWith('@scope.'))
            topics.add('scope');
        else if (name.startsWith('@declaration.'))
            topics.add('declaration');
        else if (name.startsWith('@import.'))
            topics.add('import');
        else if (name.startsWith('@type-binding.'))
            topics.add('type-binding');
        else if (name.startsWith('@reference.'))
            topics.add('reference');
        else if (name.startsWith('@callable-flow.'))
            topics.add('callable-flow');
    }
    return topics;
}
...
function pass1BuildScopes(matches, filePath, provider) {
    ...
    for (const cand of candidates) {
        ...
        drafts.push(makeDraft(cand.id, parent, cand.kind, cand.range, filePath, provider.scopeOwnsReceivers?.(cand.match), parseScopeLexicalNames(cand.match)));
        stack.push(cand);
    }
    return drafts;
}
...
function makeDraft(id, parent, kind, range, filePath, ownsReceivers, lexicalNames) {
    return {
        id,
        parent,
        kind,
...
function parseScopeLexicalNames(match) {
...
function pass2AttachDeclarations(matches, drafts, positionIndex, localDefs, filePath, provider, scopeTree) {
    ...
    const firstDefByNodeId = new Map();
    for (const match of matches) {
...
function deriveDeclarationName(match, def) {
    const nameCap = match['@declaration.binding-name'] ??
 ...
 */
function runsOnlyWhenCalled(scopeTree, scopeId) {
...
function pass3CollectImports(matches, positionIndex, filePath, parsedImports, provider, scopeTree) {
...
function pass5CollectReferences(matches, positionIndex, filePath, referenceSites, provider, scopeTree) {
    for (const match of matches) {
        ...
        const typeArguments = kind === 'inherits' ? heritageTypeArguments(match, anchor, nameCap) : undefined;
 ...
 */
function heritageTypeArguments(match, anchor, nameCap) {
 ...
 */
function referenceTypeArguments(anchorText, baseName) {
    const text = stripTrailingCallSuffix(anchorText.trim());
    const opener = text.search(OPENING_BRACKET);
    if (opener === -1)
        return undefined;
    if (!text.slice(0, opener).trimEnd().endsWith(baseName))
        return undefined;
 ...
 */
function extractReceiverChain(match) {
...
function pass6CollectCallableFlows(matches, positionIndex, filePath, out) {
    for (const match of matches) {
        ...
        switch (kind) {
            case 'seed': {
                ...
                if (destination === undefined || target === undefined || !nonEmpty(targetName))
                    continue;
            ...
            case 'store': {
                ...
                if (source === undefined || pointer === undefined)
                    continue;
            ...
            case 'load': {
                const pointer = callableFlowOperand(match, 'pointer', positionIndex, filePath);
                const destination = callableFlowOperand(match, 'destination', positionIndex, filePath);
                if (pointer === undefined || destination === undefined)
                    continue;
            ...
            case 'argument': {
                const source = callableFlowOperand(match, 'source', positionIndex, filePath);
                const parameterIndex = parseNonNegativeInt(match['@callable-flow.parameter-index']?.text);
                if (source === undefined || parameterIndex === undefined)
                    continue;
                out.push({
                    kind,
            ...
            case 'invoke': {
                ...
                const inScope = positionIndex.atPosition(filePath, anchor.range.startLine, anchor.range.startCol);
                const invocationKind = parseCallableInvocationKind(match['@callable-flow.invocation-kind']?.text);
                if (callee === undefined || inScope === undefined || invocationKind === undefined)
                    continue;
...
function callableFlowKind(match) {
    return CALLABLE_FLOW_KINDS.find((kind) => match[`@callable-flow.${kind}`] !== undefined);
}
function callableFlowOperand(match, role, positionIndex, filePath) {
    const cap = match[`@callable-flow.${role}`];
    if (cap === undefined || !nonEmpty(cap.text))
        return undefined;
    ...
    if (indirection !== undefined && indirection > 16)
        return undefined;
    return {
        name: cap.text,
        inScope,
...
function callableFlowExpectedSignature(match) {
    ...
    const parameterTypeClasses = parseJsonParameterTypeClassesCapture(match['@callable-flow.expected-type-classes']);
...
function parseCallableOperandKind(text) {
    switch (text) {
        case 'binding':
...
function parseCallablePassingMode(text) {
    switch (text) {
        case 'value':
        case 'reference':
        case 'pointer':
...
function parseCallableInvocationKind(text) {
    switch (text) {
        case 'indirect':
        case 'member-pointer':
