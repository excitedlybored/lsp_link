 ...
 */
export function resolveRouteHandlerSymbols(model, extractedRoutes, decoratorRoutes, routeContext) {
    const out = new Map();
    ...
    const uniqueById = (defs) => {
        const byId = new Map(defs.map((def) => [def.nodeId, def]));
        return byId.size === 1 ? byId.values().next().value : undefined;
    };
    const routeCallables = (defs) => defs.filter((def) => def.type === 'Function' || def.type === 'Method');
    const exportedRouteCallables = (defs) => routeContext === undefined
        ? []
    ...
    const uniqueImport = (filePath, localName) => {
        const matches = (filesByPath.get(filePath)?.parsedImports ?? []).filter((parsedImport) => 'localName' in parsedImport &&
            parsedImport.localName === localName &&
            parsedImport.kind !== 'dynamic-unresolved');
        return matches.length === 1 ? matches[0] : undefined;
    };
    const importedTarget = (filePath, localName) => {
    ...
    const resolveDataRouteHandler = (filePath, designator) => {
        ...
        if (parts.length === 1) {
            ...
            return uniqueById(exportedRouteCallables(model.symbols.lookupExactAll(imported.targetFile, imported.parsedImport.importedName)))?.nodeId;
        }
        if (parts.length !== 2)
            return undefined;
        ...
        if (imported === undefined || imported.parsedImport.kind === 'wildcard')
            return undefined;
        if (imported.parsedImport.kind === 'namespace') {
            return uniqueById(exportedRouteCallables(model.symbols.lookupExactAll(imported.targetFile, member)))?.nodeId;
        }
        if (!('importedName' in imported.parsedImport))
            return undefined;
        if (imported.parsedImport.importedName === 'default')
            return undefined;
        const owner = uniqueById(model.symbols
    ...
    const dataHandlerByRoute = new Map();
    const dataHandlersByIdentity = new Map();
    for (const dr of decoratorRoutes) {
        if (dr.source !== DATA_ROUTE_TABLE_SOURCE || !dr.handlerName || !dr.routePath)
            continue;
    ...
    for (const dr of decoratorRoutes) {
        const handlerId = dr.source === DATA_ROUTE_TABLE_SOURCE
        ...
        if (dr.source === DATA_ROUTE_TABLE_SOURCE && handlerId === undefined)
            continue;
        if (dr.source === DATA_ROUTE_TABLE_SOURCE && dr.routePath) {
            const url = normalizeExtractedRoutePath(dr.routePath, dr.prefix ?? null);
            const key = routeNodeKey(normalizeRouteMethod(dr.httpMethod), url);
            const state = dataHandlersByIdentity.get(key);
            if (state === undefined || state.hasUnresolved || state.handlers.size !== 1)
                continue;
        }
