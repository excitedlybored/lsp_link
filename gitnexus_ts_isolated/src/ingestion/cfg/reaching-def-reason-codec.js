...
function isValidPair(defLine, useLine) {
 ...
 * the substrate).
 */
export function encodeReachingDefReasonPairs(name, pairs) {
 ...
 */
export function encodeReachingDefReason(name, defLine, useLine) {
 ...
 */
export function decodeReachingDefReason(reason) {
    const raw = typeof reason === 'string' ? reason : '';
    const sep = raw.indexOf(NAME_SEP);
    if (sep === -1)
        return { name: raw, pairs: [] };
    ...
    for (const chunk of body.split(PAIR_SEP)) {
        ...
        if (parts.length !== 2)
            continue; // malformed pair — drop it, keep the rest
        const defLine = Number(parts[0]);
        const useLine = Number(parts[1]);
        if (!isValidPair(defLine, useLine))
            continue;
