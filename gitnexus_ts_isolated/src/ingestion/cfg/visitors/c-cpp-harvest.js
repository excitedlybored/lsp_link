...
export class CCppHarvester extends ScopeTreeHarvester {
    ...
    declaratorName(node) {
        ...
        while (cur && hops-- > 0) {
            ...
            const next = cur.childForFieldName('declarator') ??
                cur.namedChildren.find((c) => c.type === 'identifier' ||
     ...
     */
    structuredBinding(declarator) {
        let cur = declarator ?? undefined;
        let hops = 4;
        while (cur && hops-- > 0) {
            if (cur.type === 'structured_binding_declarator')
                return cur;
            if (cur.type !== 'reference_declarator' && cur.type !== 'pointer_declarator')
                break;
            cur = cur.namedChildren.find((c) => c.type === 'structured_binding_declarator' ||
                c.type === 'reference_declarator' ||
                c.type === 'pointer_declarator');
        }
        return undefined;
    ...
    structuredBindingNames(sbd) {
        return sbd.namedChildren.filter((c) => c.type === 'identifier');
    }
    declareDeclarators(declNode, scope) {
        for (let i = 0; i < declNode.namedChildCount; i++) {
            ...
            if (d.type === 'init_declarator') {
                ...
                if (sbd) {
                    ...
                    for (const id of this.structuredBindingNames(sbd))
                        this.declare(id, 'var', scope);
                }
                else {
