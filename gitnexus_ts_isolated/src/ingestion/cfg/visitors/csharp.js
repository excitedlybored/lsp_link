...
class CsharpCfgWalk {
        ...
        return this.builder.withNesting(() => {
            ...
            for (let i = 0; i < stmts.length; i++) {
                ...
                if (this.isUsingLocalDecl(stmt)) {
                    ...
                    const acq = this.builder.newBlock(startLineOf(stmt), endLineOf(stmt), stmt.text, 'normal', this.harvest.facts(stmt));
                    if (entry === undefined)
                        entry = acq;
                    else
                        this.builder.connect(dangling, acq, 'seq');
    ...
    visitSwitch(stmt) {
        ...
        const entryOf = new Array(sections.length);
    ...
    sectionIsDefault(section) {
        return !section.namedChildren.some((c) => !this.isStatementLike(c) && c.type !== 'comment' && c.type !== 'when_clause');
    }
     ...
     */
    isUsingLocalDecl(stmt) {
        if (stmt.type !== 'local_declaration_statement')
            return false;
        for (let i = 0; i < stmt.childCount; i++) {
            if (stmt.child(i)?.type === 'using')
                return true;
        }
        return false;
    }
     ...
     */
    buildUsingDeclScope(declStmt, restStmts) {
        ...
        if (bodyRes)
            this.builder.connect(bodyRes.exits, disposeBlock, 'seq');
     ...
     */
    buildProtected(bodyNode, catchClauses, finallyBody, span) {
        ...
        if (finallyRes) {
            ...
            for (const e of catchExits)
                this.builder.edge(e, finallyRes.entry, 'seq');
     ...
     */
    buildProtectedSynthetic(bodyNode, span, text, finalizerFacts, factsNode) {
        void factsNode;
...
function buildFunctionCfg(fnNode, filePath) {
    try {
        if (!CSHARP_FUNCTION_TYPES.has(fnNode.type))
            return undefined;
        ...
        if (!body)
            return undefined; // abstract / partial / interface member — no body
...
/** The C# CFG visitor. */
export function createCsharpCfgVisitor() {
