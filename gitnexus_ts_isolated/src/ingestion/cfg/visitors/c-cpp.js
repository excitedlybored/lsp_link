...
class CCfgWalk {
    ...
    controlFlowTypes;
    constructor(builder, harvest, controlFlowTypes) {
    ...
    nearestContinueTarget() {
        for (let i = this.jumps.length - 1; i >= 0; i--) {
            if (this.jumps[i].kind === 'loop')
                return this.jumps[i].continueTo;
        }
        return undefined;
    }
    ...
    visitLabeled(stmt) {
        ...
        const body = stmt.namedChildren.find((c) => c.type !== 'statement_identifier' && c.type !== 'comment') ??
            null;
    ...
    finishGotos() {
        for (const [label, froms] of this.pendingGotos) {
            ...
            for (const from of froms)
                this.builder.edge(from, this.builder.exitIndex, 'seq');
        }
...
class CppCfgWalk extends CCfgWalk {
    ...
    visitForRange(stmt) {
        ...
        const header = this.builder.newBlock(startLineOf(stmt), startLineOf(stmt), this.forRangeHeaderText(stmt), 'normal', this.harvest.forRangeHeadFacts(stmt));
        const loopExit = this.builder.newBlock(endLineOf(stmt), endLineOf(stmt), '');
        this.jumps.push({ kind: 'loop', breakTo: loopExit, continueTo: header });
        const body = this.visitBody(this.bodyBlockOf(stmt));
        this.jumps.pop();
...
function buildFunctionCfg(fnNode, filePath, functionTypes, controlFlowTypes, WalkClass) {
    try {
        if (!functionTypes.has(fnNode.type))
            return undefined;
        ...
        if (!body || body.type !== 'compound_statement')
            return undefined; // declaration / no body
...
/** The C CFG visitor. */
export function createCCfgVisitor() {
    return {
        isFunction: (node) => C_FUNCTION_TYPES.has(node.type),
        buildFunctionCfg: (fnNode, filePath) => buildFunctionCfg(fnNode, filePath, C_FUNCTION_TYPES, C_CONTROL_FLOW_TYPES, CCfgWalk),
    };
}
/** The C++ CFG visitor (C core + exceptions + range-for + lambdas). */
export function createCppCfgVisitor() {
    return {
        isFunction: (node) => CPP_FUNCTION_TYPES.has(node.type),
        buildFunctionCfg: (fnNode, filePath) => buildFunctionCfg(fnNode, filePath, CPP_FUNCTION_TYPES, CPP_CONTROL_FLOW_TYPES, CppCfgWalk),
    };
}
