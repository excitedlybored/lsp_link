...
export class CsharpHarvester extends ScopeTreeHarvester {
    ...
    declareParams(fnNode) {
        const params = fnNode.childForFieldName('parameters') ??
            fnNode.namedChildren.find((c) => c.type === 'parameter_list' || c.type === 'implicit_parameter');
        if (!params)
            return;
    ...
    prescan(node, scope) {
        ...
        switch (t) {
            case 'local_declaration_statement': {
                ...
                if (decl)
                    this.declareVariableDeclaration(decl, childScope);
                break;
            }
            case 'foreach_statement': {
                ...
                if (left)
                    this.declareForeachTarget(left, childScope);
                break;
            }
    ...
    declareVariableDeclaration(declNode, scope) {
        for (let i = 0; i < declNode.namedChildCount; i++) {
            ...
            if (name) {
                ...
                if (tuple)
                    this.declareForeachTarget(tuple, scope);
            }
        }
    }
    /** Declare a `foreach` target — an identifier or a `tuple_pattern`. */
    declareForeachTarget(left, scope) {
    ...
    forEachHeadFacts(stmt) {
        ...
        if (left)
            this.defForeachTarget(left, acc);
        if (right)
    ...
    paramFacts() {
        const params = this.fnNode.childForFieldName('parameters') ??
            this.fnNode.namedChildren.find((c) => c.type === 'parameter_list' || c.type === 'implicit_parameter');
        if (!params)
            return undefined;
                    ...
                    for (let i = 0; i < decl.namedChildCount; i++) {
                        ...
                        if (!name) {
                            ...
                            if (tuple) {
                                ...
                                if (tupleInit)
                                    this.registerResultDefs(tupleInit, acc.defsSince(snap));
                            }
                            if (tupleInit)
                                this.walkValue(tupleInit, acc);
                            continue;
                        }
