 ...
 */
import { isExitReachableFromAllBlocks } from './post-dominators.js';
/**
 ...
 */
const SYNTHETIC_ESCAPE_KIND = 'cond-false';
function reach(start, adj, n) {
...
function computeReachability(cfg) {
...
export function computeScc(succ, n) {
    ...
    for (let root = 0; root < n; root++) {
        if (index[root] !== -1)
            continue;
        const work = [{ node: root, childIdx: 0 }];
        while (work.length > 0) {
 ...
 */
export function augmentForPostDom(cfg) {
    const n = cfg.blocks.length;
    ...
    for (const comp of members) {
        ...
        if (canReachExit[rep])
            continue; // SCC escapes to EXIT — nothing to bridge
    ...
    if (synthetic.length === 0)
        return cfg; // nothing bridgeable — gate will skip
 ...
 * the input unchanged.
 */
export function wasAugmented(cfg, view) {
    return view !== cfg;
}
