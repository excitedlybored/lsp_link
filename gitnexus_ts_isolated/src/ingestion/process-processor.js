...
outwardActionSites = []) => {
    ...
    const isSink = (nodeId) => sinkFunctions.has(nodeId);
    ...
    const tracesByTerminal = new Map();
    const rankedByInterest = (() => {
        const decorated = endpointDeduped.map((trace) => ({
            trace,
            sink: isSink(trace[trace.length - 1] ?? '') ? 1 : 0,
            key: traceOrderKey(trace),
        }));
        decorated.sort((a, b) => b.sink - a.sink || b.trace.length - a.trace.length || compareOrderKeys(a.key, b.key));
    ...
    for (const trace of rankedByInterest) {
        ...
        if (existing === undefined)
            tracesByTerminal.set(terminalId, [trace]);
        else
            existing.push(trace);
    }
    ...
    for (let round = 0; limitedTraces.length < cfg.maxProcesses; round++) {
        ...
        for (const traces of tracesByTerminal.values()) {
            ...
            addedAny = true;
            if (limitedTraces.length >= cfg.maxProcesses)
                break;
        }
        if (!addedAny)
            break;
...
const buildCallsGraph = (graph) => {
    ...
    graph.forEachRelationshipFields((sourceId, targetId, type, confidence) => {
        if (type !== 'CALLS' || confidence < MIN_TRACE_CONFIDENCE)
            return;
        const existing = adj.get(sourceId);
        if (existing === undefined)
            adj.set(sourceId, [targetId]);
        else
...
truncation) => {
    ...
    // `rankedByInterest`.
    const sorted = entryPointCandidates.sort((a, b) => b.score - a.score || (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
...
// nothing.
export const traceFromEntryPoint = (entryId, callsEdges, config, 
/**
...
truncation) => {
    ...
    if (stack.length > 0) {
        if (truncation !== undefined)
            truncation.walksCutByBudget++;
        logger.debug({ entryId, traceBudget, unexploredBranches: stack.length }, 'process-processor: trace budget exhausted; unexplored branches remain for this entry point');
    }
    return traces;
};
const CALLABLE_SINK_LABELS = new Set([
    'Function',
    'Method',
    'Constructor',
]);
 ...
 */
export function buildSinkFunctionSet(graph, sites) {
    const sinks = new Set();
    if (sites.length === 0)
        return sinks;
    const byFile = new Map();
    for (const node of graph.iterNodes()) {
        if (!CALLABLE_SINK_LABELS.has(node.label))
            continue;
        const props = node.properties;
        if (typeof props.filePath !== 'string')
            continue;
        if (typeof props.startLine !== 'number' || typeof props.endLine !== 'number')
            continue;
        const entry = { id: node.id, start: props.startLine, end: props.endLine };
        const list = byFile.get(props.filePath);
        if (list === undefined)
            byFile.set(props.filePath, [entry]);
        else
            list.push(entry);
    }
    for (const site of sites) {
        ...
        for (const c of candidates) {
            if (site.lineNumber < c.start || site.lineNumber > c.end)
                continue;
            if (best === undefined || c.end - c.start < best.end - best.start)
                best = c;
        }
        if (best !== undefined)
            sinks.add(best.id);
 ...
 */
const traceOrderKey = (trace) => trace.join('\u0000');
 ...
 * one.
 */
const sortByDepthThenPath = (traces) => {
 ...
 */
export const deduplicateTraces = (traces, 
