 ...
 */
const INJECTION_ANNOTATIONS = new Set(['@Autowired', '@Inject']);
...
const COLLECTION_WRAPPERS = new Set(['List', 'Set', 'Collection', 'Map']);
 ...
 */
function splitTopLevelGenericArgs(inner) {
    const args = [];
    let depth = 0;
    let segmentStart = 0;
 ...
 */
function parseElementTypeName(segment) {
    ...
    if (!JAVA_TYPE_NAME_PATTERN.test(element))
        return null;
    return element;
}
/**
 ...
 */
export function parseSpringCollectionType(rawDeclaredType) {
    ...
    if (openIndex === -1 || !normalized.endsWith('>'))
        return null;
...
export function parseSpringInjectionType(rawDeclaredType) {
    ...
    const normalized = rawDeclaredType.replace(/\s+/g, '').trim();
    if (!JAVA_TYPE_NAME_PATTERN.test(normalized))
        return null;
 ...
 */
export const springDiFieldMatcher = (node) => {
    ...
    if (!rawDeclaredType) {
        ...
        if (isDev) {
            logger.warn(`Spring DI: annotated field '${node.properties.name}' (${node.properties.filePath}) has no rawDeclaredType — extraction plumbing contract breach; skipping`);
        }
        return null;
    }
...
function isInjectionMatch(value) {
    if (value === null || typeof value !== 'object')
        return false;
    const match = value;
    const namedSelection = match.namedSelection;
    return (typeof match.targetTypeName === 'string' &&
...
function isProviderMatch(value) {
    if (value === null || typeof value !== 'object')
        return false;
    const provider = value;
    return (Array.isArray(provider.names) &&
