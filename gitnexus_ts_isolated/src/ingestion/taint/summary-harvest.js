...
import { sinkKindRank, sortSinkKinds } from './source-sink-config.js';
const EMPTY_FACTS = {
    paramCount: 0,
    paramToReturn: [],
    paramToCallArg: [],
...
const calleeTail = (callee) => callee === undefined ? undefined : (callee.split('.').pop() ?? callee);
/**
 ...
 */
export function harvestFunctionSummary(cfg, defUse, matches) {
    ...
    const sinkPosBySite = new Map(); // stmtKey → site → argPositions
    const sinkKindByEntry = new Map(); // stmtKey → site → kinds at any pos
    const sanitizerResultDefKinds = new Map(); // stmtKey → resultDef binding → kinds
    ...
    const paramReturn = new Map(); // param → neutralized intersection
    const paramReturnSeen = new Set();
    const paramCallArg = new Map();
    const sourceCallArg = new Map();
    ...
    const paramCallArgKinds = new Map();
    const sourceCallArgKinds = new Map();
    const intersectKinds = (store, key, incoming) => {
        const cur = store.get(key);
        if (cur === undefined)
            store.set(key, new Set(incoming));
        else
            for (const k of [...cur])
                if (!incoming.has(k))
                    cur.delete(k);
    };
    const paramSink = new Set();
    const paramSinkOut = [];
    const sourceReturn = new Set();
    // Caller-side call-result flows (#2084 review P1-1), deduped by a structural key.
    const callResults = new Map();
    const recordCallResult = (cr) => {
        const d = cr.dest;
        const destKey = d.to === 'sink'
            ? `sink:${d.sinkKind}`
            : d.to === 'return'
                ? 'return'
    ...
    for (const { idx } of paramBindings) {
        const paramIdx = paramIndexOf.get(idx);
    ...
    for (const block of cfg.blocks) {
        block.statements?.forEach((facts, stmtIdx) => {
            ...
            const point = { blockIndex: block.index, stmtIndex: stmtIdx, line: facts.line };
            facts.sites?.forEach((site, siteIndex) => {
                if (site.kind !== 'call' && site.kind !== 'new')
                    return;
                if (modeledSites.has(`${stmtKey}:${siteIndex}`))
                    return; // sink/sanitizer — modelled
                const tail = calleeTail(site.callee);
                if (tail === undefined)
                    return; // unresolvable callee — cannot compose
                ...
                if ((site.resultDefs?.length ?? 0) === 0) {
                    ...
                    while (cur?.parent) {
                        ...
                        else if (!modeledSites.has(ancKey) &&
        ...
        for (const fact of factsByDef.get(`${b}:${pointKey(t.point)}`) ?? []) {
            ...
            if (returnUseStmtKeys.has(useKey) && useStmt.uses.includes(b)) {
                if (t.originCallee !== undefined) {
                    ...
                    recordReturn(t.seedId, t.exclusions);
                else
                    sourceReturn.add('remote-input');
            }
            ...
            for (const d of [...useStmt.defs, ...(useStmt.mayDefs ?? [])]) {
                ...
                const exclusions = added && added.length > 0 ? new Set([...t.exclusions, ...added]) : t.exclusions;
                enqueue({
                    bindingIdx: d,
                    point: {
    ...
    const paramToReturn = [...paramReturn.entries()]
        .map(([param, kinds]) => ({
        param,
    ...
    const paramToSink = paramSinkOut.sort((a, b) => a.param - b.param || sinkKindRank(a.sinkKind) - sinkKindRank(b.sinkKind));
    const sourceToReturn = sourceReturn.size > 0 ? [{ sourceKind: 'remote-input' }] : [];
    const sourceToCallArg = [...sourceCallArg.entries()]
        .map(([key, edge]) => {
        const kinds = sourceCallArgKinds.get(key);
        return kinds && kinds.size > 0 ? { ...edge, neutralized: sortSinkKinds(kinds) } : edge;
    })
        .sort((a, b) => a.callLine - b.callLine ||
        a.argIndex - b.argIndex ||
        (a.calleeName ?? '').localeCompare(b.calleeName ?? ''));
...
function occurrencesInArgs(site, b) {
    const hits = [];
