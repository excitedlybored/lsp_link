 ...
 */
function packReturnBitset(returnFlowParams) {
    let bits = 0n;
 ...
 */
export function encodeCallSummary(returnFlowParams) {
 ...
 */
export function decodeCallSummary(reason) {
    ...
    const returnFlowParams = [];
    let bits = returnBits;
    let idx = 0;
    while (bits > 0n) {
        if ((bits & 1n) === 1n)
            returnFlowParams.push(idx);
        bits >>= 1n;
        idx++;
    }
