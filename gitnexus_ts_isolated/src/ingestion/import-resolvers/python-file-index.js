 ...
 */
export function importerDirOf(fromFile) {
    const norm = fromFile.replace(/\\/g, '/');
    const lastSlash = norm.lastIndexOf('/');
    return lastSlash === -1 ? '' : norm.slice(0, lastSlash);
}
export function importerAncestors(index, importerDir) {
 ...
 */
function buildImporterAncestors(importerDir) {
    const chain = [];
...
export const getPythonFileIndex = perFileSet((allFilePaths) => {
    ...
    const byInitParent = new Map();
    const dirPrefixes = new Set();
    const nestedDirNames = new Set();
    for (const raw of allFilePaths) {
        ...
        if (base === '__init__.py' && lastSlash >= 0) {
            ...
            if (parentName) {
                ...
                if (ib === undefined)
                    byInitParent.set(initKey, [entry]);
                else
                    ib.push(entry);
            }
        }
        ...
        for (let i = lastSlash; i >= 0; i--) {
            ...
            if (dirPrefixes.has(dirPrefix))
                break;
            dirPrefixes.add(dirPrefix);
            const parentSlash = i > 0 ? norm.lastIndexOf('/', i - 1) : -1;
            if (parentSlash > 0)
                nestedDirNames.add(norm.slice(parentSlash + 1, i));
        }
    }
    return {
        normSet,
        ...
        dirPrefixes,
        nestedDirNames,
        ancestorsByDir: new Map(),
        bareImportPrefixesByDir: new Map(),
    };
});
/**
 ...
 */
export function importerBarePrefixes(index, importerDir) {
 ...
 */
function buildImporterBarePrefixes(importerDir) {
    const prefixes = [];
    let dir = importerDir;
 ...
 */
export function pythonSegmentAbsent(index, segment) {
    if (segment === '' || segment.includes('\\'))
        return false;
    if (index.byBasename.has(`${segment}.py`))
        return false;
    if (index.byInitParent.has(`${segment}/__init__.py`))
        return false;
    return true;
}
