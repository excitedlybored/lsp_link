...
export function buildSuffixIndex(normalizedFileList, allFileList, options) {
    ...
    const getExactMap = () => {
        ...
        for (let i = 0; i < normalizedFileList.length; i++) {
            ...
            if (!built.has(normalized))
                built.set(normalized, original);
        }
        exactMap = built;
        return built;
    };
    ...
    return {
        get: (suffix) => getExactMap().get(suffix),
        getInsensitive: (suffix) => getLowerMap().get(suffix.toLowerCase()),
