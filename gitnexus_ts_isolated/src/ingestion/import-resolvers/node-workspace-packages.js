 ...
 */
export function nodePackageNameOf(specifier) {
...
export function owningPackage(filePath, packages) {
    if (!packages)
        return null;
    let best = null;
    for (const pkg of packages.byName.values()) {
        const inside = pkg.dir === '' || filePath.startsWith(`${pkg.dir}/`);
        if (inside && (best === null || pkg.dir.length > best.dir.length))
            best = pkg;
    }
    return best;
}
 ...
 */
export function resolveNodeWorkspaceImport(specifier, packages, allFiles) {
    if (!packages)
        return null;
 ...
 */
export function matchSubpathMap(map, specifier) {
    ...
    const patterns = [...map.entries()]
        ...
        .filter(({ prefix, suffix }) => specifier.startsWith(prefix) &&
 ...
 */
export function substituteStar(target, stem) {
...
function entryStemsFor(pkg, subpath) {
    if (subpath === '')
        return pkg.entries;
...
async function loadWorkspaceScope(repoRoot) {
    ...
    if (Array.isArray(lerna?.packages)) {
        patterns.push(...lerna.packages.filter((w) => typeof w === 'string'));
    }
...
async function readYamlPackages(filePath) {
    let raw;
    try {
 ...
 */
export async function loadNodeWorkspacePackages(repoRoot) {
    ...
    while (queue.length > 0) {
        if (dirsScanned >= SCAN_MAX_DIRS) {
            logger.warn(`[node] package.json scan of ${repoRoot} hit the ${SCAN_MAX_DIRS}-directory cap; workspace packages below it will not resolve`);
            break;
        }
        ...
        for (const entry of entries) {
            if (entry.isDirectory()) {
                if (isHardcodedIgnoredDirectory(entry.name))
                    continue;
            ...
            if (pkg !== null && !byName.has(pkg.name))
                byName.set(pkg.name, pkg.package);
        }
    }
    return byName.size === 0 ? null : { byName };
}
async function readManifest(manifestPath, repoRoot, dir) {
    let parsed;
    try {
    ...
    const rebase = (raw) => joinRepoPath(packageDir, stripEntryPrefixes(raw));
 ...
 */
function collectExports(node, subpaths, rootStems, rebase, currentSubpath = '') {
    ...
    for (const [key, value] of Object.entries(node)) {
        if (key.startsWith('.')) {
            ...
            collectExports(value, subpaths, rootStems, rebase, key === '.' ? '' : key.replace(/^\.\//, ''));
        }
        else {
...
function collectImports(node, out, rebase, currentKey = null) {
    if (typeof node === 'string') {
        if (currentKey === null)
            return;
    ...
    if (Array.isArray(node)) {
        for (const element of node)
            collectImports(element, out, rebase, currentKey);
        return;
    }
    if (node === null || typeof node !== 'object')
        return;
...
function stripEntryPrefixes(entry) {
...
function repoRelativeDir(repoRoot, dir) {
    const rel = path.relative(repoRoot, dir).split(path.sep).join('/');
    return rel === '.' ? '' : rel;
}
function joinRepoPath(dir, rest) {
