...
function topLevelAssignment(argument) {
    const stack = [];
    let quote = null;
    let escaped = false;
...
export function parseSpringAnnotationArguments(annotationText) {
    const open = annotationText.indexOf('(');
    if (open === -1)
        return [];
    const close = annotationText.lastIndexOf(')');
    if (close < open || annotationText.slice(close + 1).trim().length > 0)
        return null;
    const body = annotationText.slice(open + 1, close).trim();
    if (body.length === 0)
        return [];
    const rawArguments = splitTopLevel(body, ',');
    if (rawArguments === null || rawArguments.some((argument) => argument.length === 0))
        return null;
    const parsed = [];
...
export function parseStaticStringLiteral(value) {
    const trimmed = value.trim();
    if (trimmed.startsWith('"""')) {
        if (trimmed.length < 6 || !trimmed.endsWith('"""'))
            return null;
        const raw = trimmed.slice(3, -3);
        if (raw.includes('"""') || /\$(?:\{|[A-Za-z_])/.test(raw))
            return null;
        return raw;
    }
...
export function parseStaticStringValues(value) {
    const trimmed = value.trim();
    const pair = (trimmed.startsWith('{') && trimmed.endsWith('}')) ||
...
export function parseStaticClassLiteral(value) {
...
export function normalizeSpringBeanType(rawType) {
    ...
    let normalized = rawType
        ...
        .replace(/\s+/g, '')
        .replace(/^kotlin\.collections\.Mutable(List|Set|Collection|Map)(?=<|$)/, 'kotlin.collections.$1')
