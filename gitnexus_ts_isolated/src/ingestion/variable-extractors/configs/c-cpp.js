 ...
 */
function findStructuredBindingDeclarator(node) {
    for (let i = 0; i < node.namedChildCount; i++) {
        const child = node.namedChild(i);
        if (child?.type !== 'init_declarator')
            continue;
        ...
        if (declarator?.type === 'reference_declarator') {
            const inner = declarator.namedChildren.find((c) => c.type === 'structured_binding_declarator');
            if (inner)
                return inner;
        }
    }
 ...
 */
function extractCVarNames(node) {
