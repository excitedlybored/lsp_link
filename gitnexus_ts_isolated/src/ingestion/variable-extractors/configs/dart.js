...
function extractDartVarNames(node) {
 ...
 */
function scanLeadingSiblings(node) {
    const out = [];
...
function hasReadonlyModifier(node) {
    for (const sib of scanLeadingSiblings(node)) {
        if (sib.type === 'const_builtin' || sib.type === 'final_builtin')
            return true;
    }
    return false;
}
