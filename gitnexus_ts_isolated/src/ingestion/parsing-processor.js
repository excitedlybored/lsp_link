...
function compareParsedNodeSourceOrder(left, right) {
...
function nodesInSourceOrder(nodes) {
...
chunkHash) => {
    ...
    for (const [lang, c] of cfgSkipped) {
        if (c.tooDeeplyNested > 0 || c.buildError > 0) {
            logger.warn(`  CFG functions skipped (${lang}): ${c.tooDeeplyNested} too-deeply-nested, ` +
