...
import { intersectSpringHttpMethods, springAnnotationHttpMethods, isRouteMemberKey, findEnclosingType, unquoteSpringLiteral, } from './spring-shared.js';
/**
...
export function extractSpringRoutes(tree, filePath, lineOffset = 0) {
    ...
    const classHttpMethodsById = new Map();
    ...
    const routes = [];
    const httpMethodsByAnnotationId = new Map();
    for (const match of matches) {
        const caps = {};
        for (const { name, node } of match.captures) {
            caps[name] = node;
        ...
        const httpMethods = intersectSpringHttpMethods(enclosingClass ? (classHttpMethodsById.get(enclosingClass.id) ?? ['*']) : ['*'], methodMethods);
    ...
    for (const match of TYPE_DECLARATION_QUERY.matches(tree.rootNode)) {
        ...
        for (const methodNode of directMethods(typeNode)) {
            ...
            for (const annotationNode of declarationAnnotations(methodNode)) {
                ...
                if (annotationHasRouteMember(annotationNode))
                    continue;
...
function annotationHasRouteMember(ann) {
    const args = ann.childForFieldName('arguments');
    if (!args)
        return false;
    for (const child of args.namedChildren) {
...
function typeRequestMethods(typeNode) {
    const mappings = declarationAnnotations(typeNode).filter((ann) => annotationName(ann) === 'RequestMapping');
...
function annotationRoutePathsOrDefault(ann) {
