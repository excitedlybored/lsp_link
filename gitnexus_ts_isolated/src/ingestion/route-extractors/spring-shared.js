 ...
 */
function parseRequestMethodValues(value) {
    let trimmed = '';
    ...
    if (body.length === 0)
        return [];
    if (!hasOpeningBrace && body.includes(','))
        return null;
    const methods = [];
    const parts = body.split(',');
    if (hasOpeningBrace && parts[parts.length - 1].trim() === '')
        parts.pop();
    for (const rawPart of parts) {
        const part = rawPart.trim();
        const match = /^(?:(?:[A-Za-z_$][A-Za-z0-9_$]*\.)*RequestMethod\.)?(GET|HEAD|POST|PUT|PATCH|DELETE|OPTIONS|TRACE)$/.exec(part);
        if (!match)
            return null;
 ...
 */
export function springAnnotationHttpMethods(annotationName, annotationText) {
...
export function intersectSpringHttpMethods(classMethods, methodMethods) {
    if (classMethods.length === 0 || methodMethods.length === 0)
        return [];
    if (classMethods.includes('*'))
        return methodMethods;
    if (methodMethods.includes('*'))
        return classMethods;
