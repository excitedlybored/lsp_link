...
function joinRouteName(prefix, name) {
    if (!name)
        return null;
    return prefix ? `${prefix}${name}` : name;
}
function routeNameBaseFromPath(routePath) {
...
function appendResourceActionName(base, action) {
    if (!base)
        return null;
...
export function extractLaravelRoutes(tree, filePath) {
    ...
    function emitRoute(httpMethod, argsNode, lineNumber, groupStack, chainAttrs) {
        ...
        if (ROUTE_RESOURCE_METHODS.has(httpMethod)) {
            ...
            const routeNameBase = routeName ?? joinRouteName(effective.namePrefix, routeNameBaseFromPath(routePath));
        ...
        if (chain) {
            if (chain.terminalMethod === 'group') {
                ...
                for (const attr of chain.attributes) {
                    ...
                    if (attr.method === 'name')
                        groupCtx.namePrefix = extractFirstStringArg(attr.argsNode);
                    if (attr.method === 'controller')
            ...
            const chainedRouteIndex = chain.attributes.findIndex((attr) => ROUTE_HTTP_METHODS.has(attr.method) || ROUTE_RESOURCE_METHODS.has(attr.method));
            if (chainedRouteIndex >= 0) {
                ...
                emitRoute(routeAttr.method, routeAttr.argsNode, node.startPosition.row, groupSnapshot, routeAttrs);
                continue;
            }
        }
