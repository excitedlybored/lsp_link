 ...
 */
const PATH_IDENTIFIERS = new Set([
    'pathname',
    'pathName',
    'urlPath',
    'routePath',
...
const RAW_URL_RECEIVERS = new Set(['req', 'request']);
/**
...
function buildConstantMap(root) {
    const direct = new Map(); // name -> literal
    const alias = new Map(); // name -> other name
...
function plainLiteralValue(node) {
...
function isPathExpression(node) {
    if (node.type === 'identifier')
        return PATH_IDENTIFIERS.has(node.text);
    if (node.type === 'member_expression') {
        const property = node.childForFieldName('property');
        if (property === null)
            return false;
        if (PATH_IDENTIFIERS.has(property.text))
            return true;
 ...
 * kept for symmetry.
 */
function isNegatedContext(node) {
    let negations = 0;
    let current = node;
...
function isMethodExpression(node) {
    if (node.type === 'identifier')
        return node.text === 'method' || node.text === 'httpMethod';
    if (node.type === 'member_expression') {
        const property = node.childForFieldName('property');
 ...
 */
function verbFromComparison(node) {
 ...
 */
function governingVerbs(comparison) {
    let current = comparison;
    let parent = current.parent;
    while (parent !== null && !FUNCTION_NODE_TYPES.has(parent.type)) {
        if (parent.type === 'binary_expression' &&
            parent.childForFieldName('operator')?.text === '&&') {
            const sibling = parent.childForFieldName('left')?.id === current.id
 ...
 */
function findVerbsInSubtree(node, negated = false) {
    if (isNegation(node)) {
    ...
    if (node.type === 'ternary_expression')
        return verbsFromTernary(node, negated);
    if (isDisjunction(node))
        return verbsFromDisjunction(node, negated);
    const direct = verbFromComparison(node);
    if (direct !== null)
        return negated ? [] : [direct];
 ...
 */
function verbsFromDisjunction(node, negated) {
    if (negated)
        return [];
 ...
 */
function verbsFromTernary(node, negated) {
    if (negated)
        return [];
    ...
    if (alternative.type === 'false') {
        return intersectVerbs(findVerbsInSubtree(condition, false), findVerbsInSubtree(consequence, false));
    }
    if (consequence.type === 'false') {
        return intersectVerbs(findVerbsInSubtree(condition, true), findVerbsInSubtree(alternative, false));
    }
    return [];
}
/**
 ...
 */
function enclosingHandlerName(node) {
...
function enclosingScopeId(node) {
 ...
 */
function enclosingScopeIds(node) {
    const ids = [];
    for (let fn = enclosingFunction(node); fn !== null; fn = enclosingFunction(fn))
        ids.push(fn.id);
 ...
 */
function bindingKey(scopeId, name) {
    return `${scopeId}\u0000${name}`;
}
/**
...
function patternNames(pattern, out = []) {
    ...
    if (pattern.type === 'pair_pattern' || pattern.type === 'assignment_pattern') {
        ...
        if (bound !== null)
            patternNames(bound, out);
        return out;
    }
    for (const child of pattern.namedChildren)
        patternNames(child, out);
    return out;
}
/**
 ...
 */
export function regexToRoutePath(source) {
 ...
 * comparison.
 */
export function extractDispatchGuardRoutes(tree, filePath, lineOffset = 0) {
    ...
    const visit = (node) => {
        if (node.type === 'binary_expression')
            collectFromComparison(node, found, constants);
        else if (node.type === 'call_expression')
            collectFromRegexDispatch(node, found, regexes, matches);
        else if (node.type === 'switch_statement')
            collectFromSwitch(node, found, constants);
        else if (node.type === 'variable_declarator')
            noteDeclaration(node, matches);
 ...
 */
function pushPerVerb(out, verbs, route) {
    if (verbs.length === 0) {
 ...
 */
function collectFromSwitch(node, out, constants) {
    ...
    if (discriminant === null || !isPathExpression(discriminant))
        return;
 ...
 */
function noteDeclaration(node, matches) {
    const name = node.childForFieldName('name');
    if (name === null)
        return;
 ...
 */
const REBINDABLE_TARGETS = new Set([
    'identifier',
    'array_pattern',
    'object_pattern',
]);
function noteReassignment(node, matches) {
    const left = node.childForFieldName('left');
    if (left === null || !REBINDABLE_TARGETS.has(left.type))
        return;
    ...
    for (const scopeId of enclosingScopeIds(node)) {
        for (const name of names)
            matches.bindings.set(bindingKey(scopeId, name), null);
    }
}
/**
 ...
 */
function buildRegexConstantMap(root) {
...
function regexPatternOf(node, regexes) {
    const expr = unparenthesize(node);
    if (expr === null)
        return null;
    if (expr.type === 'regex')
        return expr.childForFieldName('pattern')?.text ?? null;
    if (expr.type === 'identifier')
        return regexes.get(expr.text) ?? null;
    return null;
}
/**
...
function boundDeclaratorName(call) {
    const parent = call.parent;
    if (parent === null || parent.type !== 'variable_declarator')
        return null;
    if (parent.childForFieldName('value')?.id !== call.id)
        return null;
 ...
 */
function collectFromMatchBindings(root, matchBindings, out) {
    ...
    for (const binding of matchBindings.values())
        if (binding !== null)
            liveNames.add(binding.name);
    if (liveNames.size === 0)
        return;
    ...
    for (const [key, binding] of matchBindings) {
        if (binding === null || tested.has(key))
            continue;
        out.push({
            url: binding.url,
            verb: null,
 ...
 */
function isTruthinessPosition(node) {
    let current = node;
    let parent = current.parent;
 ...
 */
function dedupeWithinFile(routes) {
    const seen = new Set();
    const out = [];
 ...
 */
export function reconcileDispatchGuardRoutes(routes) {
    const verbedUrls = new Set(routes
        .filter((r) => r.source === DISPATCH_GUARD_SOURCE && r.httpMethod !== '')
        .map((r) => r.routePath));
    if (verbedUrls.size === 0)
        return [...routes];
    return routes.filter((r) => !(r.source === DISPATCH_GUARD_SOURCE && r.httpMethod === '' && verbedUrls.has(r.routePath)));
}
