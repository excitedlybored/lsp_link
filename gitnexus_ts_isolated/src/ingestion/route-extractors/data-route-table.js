...
const ROUTE_TOKEN_HINTS = ['path', 'method', 'handler'];
...
function decodeJavaScriptStringLiteral(raw) {
    if (raw.length < 2)
        return null;
...
function plainString(node) {
    ...
    if (node.type === 'template_string' &&
        node.namedChildren.every((child) => child.type === 'string_fragment' || child.type === 'escape_sequence')) {
        return decodeJavaScriptStringLiteral(node.text);
    }
    return null;
}
...
function containsExecutingExpression(node) {
    if (EXECUTING_VALUE_NODES.has(node.type))
        return true;
    if (node.type === 'unary_expression' && node.text.trimStart().startsWith('delete '))
        return true;
    return node.namedChildren.some(containsExecutingExpression);
}
function routeFromObject(node) {
    ...
    for (const child of node.namedChildren) {
        ...
        if (key === null)
            return null;
        if (!ROUTE_TOKEN_HINTS.includes(key)) {
            if (containsExecutingExpression(valueNode))
                return null;
            continue;
        }
        if (values.has(key))
    ...
    if (pathNode === undefined || methodNode === undefined || handlerNode === undefined)
        return null;
    const path = plainString(pathNode);
    const rawMethod = plainString(methodNode);
    const handler = handlerName(handlerNode);
    if (path === null || !path.startsWith('/') || rawMethod === null || handler === null)
        return null;
...
function staticRouteIdentity(node) {
    const values = new Map();
    ...
    const pathNode = values.get('path');
    const methodNode = values.get('method');
    if (pathNode === undefined || methodNode === undefined || !values.has('handler'))
        return null;
    if (lastPath < lastUnknownProperty || lastMethod < lastUnknownProperty)
        return null;
    const path = plainString(pathNode);
    const rawMethod = plainString(methodNode);
    if (path === null || !path.startsWith('/') || rawMethod === null)
        return null;
    const method = rawMethod.toUpperCase();
    return HTTP_METHODS.has(method) ? `${method}\0${path}` : null;
}
function lexicalContainer(node) {
    let current = node;
...
function entryMemberField(node, entryName) {
...
function referencesEntry(node, entryName) {
    if (node.type === 'identifier' && node.text === entryName)
        return true;
...
function isRequestField(node, field, bindingCounts) {
    if (node.type === 'parenthesized_expression') {
        const nested = node.namedChildren[0];
    ...
    return (property?.type === 'property_identifier' &&
...
function comparisonFields(condition, entryName, bindingCounts) {
    ...
    if (operator === '&&') {
        ...
        if (!leftEvidence.valid || !rightEvidence.valid)
            return { valid: false, fields: new Set() };
        return {
            valid: true,
    ...
    if ((leftField === null) === (rightField === null))
        return { valid: false, fields: new Set() };
    if ((leftField !== null && referencesEntry(right, entryName)) ||
    ...
    const matchesRequest = (field === 'path' || field === 'method') &&
...
function directlyCallsHandler(node, entryName) {
    let found = false;
    const visit = (child) => {
        if (found || (child !== node && NESTED_EXECUTABLES.has(child.type)))
            return;
...
function hasOnlyIngressReferences(scope) {
    let valid = true;
    const visit = (node) => {
        if (!valid)
            return;
...
function hasOnlyDispatchEntryReferences(node, entryName) {
    let valid = true;
    const visit = (child) => {
        if (!valid)
            return;
...
function hasProviderDispatch(body, entryName, bindingCounts) {
    if (!hasOnlyDispatchEntryReferences(body, entryName))
        return false;
    let found = false;
    const visit = (node) => {
        if (found || (node !== body && NESTED_EXECUTABLES.has(node.type)))
            return;
...
function isStaticallyFalse(node) {
    if (node.type === 'parenthesized_expression') {
        const nested = node.namedChildren[0];
...
function definitelyTerminates(node) {
...
function hasDispatchLoop(scope, tableName, bindingCounts) {
    let found = false;
    const visit = (node) => {
        if (found)
            return;
        if (node !== scope && NESTED_EXECUTABLES.has(node.type))
            return;
...
function collectPatternNames(node, add) {
    ...
    for (const child of node.namedChildren)
        collectPatternNames(child, add);
}
function collectBindingCounts(root) {
...
function isConstDeclarator(node) {
    ...
    return (declaration?.type === 'lexical_declaration' &&
        declaration.children.some((child) => child.type === 'const'));
}
function hasOnlyDispatchReferences(scope, declarator, tableName) {
    ...
    const visit = (node) => {
        ...
        if (node.type === 'identifier' && node.text === tableName) {
            if (declarationName?.id === node.id)
                return;
            const parent = node.parent;
            const allowedForOf = parent?.type === 'for_in_statement' &&
                parent.childForFieldName('right')?.id === node.id &&
                parent.children.some((child) => child.type === 'of');
            const lengthMember = parent?.type === 'member_expression' &&
...
function collectHandlerReferences(root) {
...
function isUnsafeHandlerReference(reference, handlerRoot, routeEntry) {
    ...
    while (parent !== null) {
        ...
        if (reference.type === 'identifier') {
            ...
            if (parent.type === 'call_expression') {
                ...
                const callsOwnerMember = (fn?.type === 'member_expression' || fn?.type === 'subscript_expression') &&
                    identifierName(owner) === handlerRoot &&
                    containsNode(owner ?? null, reference);
                if (containsNode(args, reference) || callsOwnerMember)
                    return true;
            }
        }
        current = parent;
...
function hasStableHandlerBinding(references, handlerRoot, routeEntry) {
    return !(references.get(handlerRoot) ?? []).some((reference) => isUnsafeHandlerReference(reference, handlerRoot, routeEntry));
}
...
function isRouteHandlerDesignatorReference(node, routeEntry) {
    let designator = node;
...
export function scanDataRouteTables(tree) {
    const source = tree.rootNode.text;
    if (!ROUTE_TOKEN_HINTS.every((token) => source.includes(token)))
        return [];
    const routes = [];
    ...
    visit(tree.rootNode);
    if (hasUnknownTableEntries)
        return [];
    return routes.filter((route) => !blockedIdentities.has(`${route.method}\0${route.path}`));
}
export function extractDataRouteTableRoutes(tree, filePath, lineOffset = 0) {
