 ...
 */
export function rankAndCap(counts, cap) {
    const ranked = [...counts.entries()].sort(([aName, aCount], [bName, bCount]) => bCount - aCount || compareCodeUnits(aName, bName));
 ...
 */
export function lookupCount(counts, name) {
    if (counts === undefined || name.length === 0)
        return undefined;
