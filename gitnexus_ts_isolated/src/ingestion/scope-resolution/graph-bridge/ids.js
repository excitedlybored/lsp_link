...
import { AMBIGUOUS_POSITION, localNameKey, positionKey, qualifiedKey, simpleKey, } from '../graph-bridge/node-lookup.js';
import { isOverloadableCallable, isPositionQualifiedLocalLabel, } from '../../utils/callable-labels.js';
...
function rangeContainsPoint(range, at) {
    if (at.startLine < range.startLine || at.startLine > range.endLine)
        return false;
    if (at.startLine === range.startLine && at.startCol < range.startCol)
        return false;
    if (at.startLine === range.endLine && at.startCol > range.endCol)
        return false;
    return true;
}
/**
 ...
 */
const isCallableDef = (d) => isOverloadableCallable(d.type);
/**
 ...
 */
function scopeIsCallableBody(range, def) {
...
function pickCallerCallableDef(scope, scopes, atRange) {
    if (atRange !== undefined) {
        for (const childId of scopes.scopeTree.getChildren(scope.id)) {
            ...
            if (child === undefined)
                continue;
            if (!rangeContainsPoint(child.range, atRange))
                continue;
            const childCallable = child.ownedDefs.find(isCallableDef);
            if (childCallable === undefined)
                continue;
 ...
 */
function defStartLine(nodeId, filePath) {
 ...
 */
function simpleNameOf(qualifiedName) {
 ...
 */
function siblingCallableLabel(label) {
    if (label === 'Method')
        return 'Function';
    if (label === 'Function')
        return 'Method';
...
export function resolveDefGraphId(filePath, def, nodeLookup) {
    ...
    if (def.type !== undefined) {
        ...
        if (line !== undefined && isPositionQualifiedLocalLabel(def.type)) {
            ...
            if (posHit !== undefined && posHit !== AMBIGUOUS_POSITION)
                return posHit;
        ...
        const nameForms = nsPrefix !== undefined && nsPrefix.length > 0 ? [`${nsPrefix}.${qn}`, qn] : [qn];
 ...
 */
export function resolveCallerGraphId(startScope, scopes, nodeLookup, atRange) {
