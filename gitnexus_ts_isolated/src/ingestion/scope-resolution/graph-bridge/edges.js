 ...
 */
function isPhantomCalleeRead(site, targetDef) {
    if (site.kind !== 'read' || site.inCalleePosition !== true)
        return false;
 ...
 */
export function tryEmitEdge(graph, scopes, nodeLookup, site, targetDef, reason, seen, confidence = 0.85, collapseByCallerTarget = false, calleeCapture) {
    ...
    if (isPhantomCalleeRead(site, targetDef))
        return false;
    ...
    if (calleeCapture !== undefined && edgeType === 'CALLS') {
        calleeCapture.sink.add(calleeCapture.filePath, site.atRange.startLine, site.atRange.startCol, targetGraphId);
    }
 ...
 */
export function tryEmitEdgeWithExplicitTargetId(graph, scopes, nodeLookup, site, targetGraphId, reason, seen, confidence = 0.85, collapseByCallerTarget = false, calleeCapture) {
    const callerGraphId = resolveCallerGraphId(site.inScope, scopes, nodeLookup, site.atRange);
    const edgeType = mapReferenceKindToEdgeType(site.kind);
    if (callerGraphId === undefined)
        return false;
