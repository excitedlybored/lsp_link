 ...
 */
class ScopeByKeyView {
    idByKey;
    scopeTree;
    constructor(idByKey, scopeTree) {
 ...
 */
export function buildWorkspaceResolutionIndex(parsedFiles, scopeTree) {
    const classScopeIdByDefId = new Map();
    const classScopeIdToDefId = new Map();
    const moduleScopeIdByFile = new Map();
    const exportedCallableByName = new Map();
    // Back-compat (no scopeTree): keep the direct Scope-object maps.
    const classScopeByDefIdDirect = scopeTree === undefined ? new Map() : undefined;
    const moduleScopeByFileDirect = scopeTree === undefined ? new Map() : undefined;
    for (const parsed of parsedFiles) {
        ...
        if (moduleScope !== undefined) {
            ...
            for (const [name, refs] of moduleScope.bindings) {
                if (exportedCallableByName.has(name))
                    continue;
    ...
    const classScopeByDefId = scopeTree === undefined
        ? classScopeByDefIdDirect
        : new ScopeByKeyView(classScopeIdByDefId, scopeTree);
    const moduleScopeByFile = scopeTree === undefined
