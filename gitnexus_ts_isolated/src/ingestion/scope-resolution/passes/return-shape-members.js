...
import { findCallableBindingInScope, findClassBindingInScope, findReceiverTypeBinding, } from '../scope/walkers.js';
 ...
 */
function idNamesMember(id, owner, member) {
 ...
 * BINDS the name.
 */
function parameterCellKey(scope, name) {
    return `${scope}\u0000${name}`;
}
/**
 ...
 */
function parameterProducerFor(startScope, name, parameterProducers, indexes) {
    let cursor = startScope;
    while (cursor !== null) {
 ...
 */
function buildParameterProducers(indexes, parsedFiles) {
    ...
    for (const parsed of parsedFiles) {
        for (const flow of parsed.callableFlowSites ?? []) {
            ...
            if (calleeDef?.filePath === undefined)
                continue;
            const binding = formals.get(`${calleeDef.filePath}\u0000${callee}\u0000${flow.parameterIndex}`);
            if (binding === undefined)
                continue;
            const cell = parameterCellKey(binding.inScope, binding.name);
            if (conflicted.has(cell))
                continue;
            const producer = findReceiverTypeBinding(flow.source.inScope, flow.source.name, indexes)?.rawName;
            if (producer === undefined || producer.length === 0)
                continue;
...
export function emitReturnShapeMemberAccesses(graph, indexes, parsedFiles, nodeLookup, 
/** Sites a precise pass already owns — never re-resolved here. */
skipSites, propertyNameIndex, 
    ...
    for (const parsed of parsedFiles) {
        for (const site of parsed.referenceSites) {
            ...
            if (producerFile === undefined) {
                ...
                if (ownerType === undefined || ownerType.length === 0)
                    continue;
            ...
            if (producerFile === undefined)
                continue;
            if (!ownFilePaths.has(producerFile))
                continue;
            const candidates = propertyNameIndex.get(site.name);
            if (candidates === undefined)
                continue;
            const owned = candidates.filter((c) => c.filePath === producerFile && idNamesMember(c.id, producer, site.name));
