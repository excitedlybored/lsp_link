...
import { resolveCallerGraphId, resolveDefGraphId, simpleQualifiedName, } from '../graph-bridge/ids.js';
...
export function callableFlowSiteKey(filePath, range) {
 ...
 */
export function collectDeferredIndirectSites(parsedFiles, scopes) {
    const out = new Set();
...
export function emitCallableValueFlow(input) {
    const facts = [];
    const invokes = [];
    ...
    const warnOverflow = (key, count, context, cap = MAX_CALLABLE_VALUE_TARGETS) => {
        ...
        addWorkItem(() => {
            ...
            if (targets.overflow || targets.targets.size > MAX_CALLABLE_VALUE_TARGETS) {
                if (!dynamicOverflow.has(callKey)) {
                    ...
                    warnOverflow(`invoke:${callKey}`, Math.max(targets.targets.size, MAX_CALLABLE_VALUE_TARGETS + 1), `site:${callKey}`);
                    notify('call', callKey);
                }
                return;
            }
    ...
    for (const fact of facts) {
        ...
        addWorkItem(() => {
            ...
            const callOverflow = dynamicOverflow.has(callKey) || targetIds.size > MAX_CALLABLE_VALUE_TARGETS;
            if (callOverflow) {
    ...
    for (const invoke of invokes) {
        ...
        for (const target of targets.values()) {
            if (tryEmitEdgeWithExplicitTargetId(input.graph, input.scopes, input.nodeLookup, { inScope: invoke.site.inScope, atRange: invoke.site.callSite, kind: 'call' }, target.id, 'callable-value-flow', seen, confidence, input.collapseByCallerTarget === true, { sink: input.calleeIds, filePath: invoke.filePath })) {
                emitted++;
            }
        }
    }
 ...
 */
export function buildGraphTargetIndex(scopes, nodeLookup, providerTarget, graph) {
    const out = new Map();
    ...
    for (const def of scopes.defs.byId.values()) {
        ...
        const id = anchored?.length === 1 ? anchored[0] : resolveDefGraphId(def.filePath, def, nodeLookup);
        if (id === undefined)
            continue;
...
function indexFormalsByGraphId(parsedFiles, scopes, nodeLookup, targetIndexes, aliasesByTargetId) {
    ...
    for (const parsed of parsedFiles) {
        for (const site of parsed.callableFlowSites ?? []) {
            if (site.kind !== 'formal')
                continue;
            const resolvedCaller = resolveCallerGraphId(site.binding.inScope, scopes, nodeLookup, site.ownerRange);
...
function narrowIndexedFormals(formals, callSignature) {
    ...
    if (owners.length < 2)
        return formals;
    const narrowed = narrowOverloadCandidates(owners, callSignature.parameterCount, callSignature.parameterTypes, { argumentTypeClasses: callSignature.parameterTypeClasses });
...
function expandMemberTargets(invoke, targets, scopes, graphTargets, targetIndexes, graph) {
    ...
    if (typeRef === undefined)
        return new Map(targets);
    const receiverClass = resolveInheritanceBaseInScope(invoke.site.receiver.inScope, typeRef, scopes);
    if (receiverClass === undefined)
        return new Map(targets);
...
function narrowTargetCandidates(candidates, expected, graph) {
    if (candidates.length === 0)
        return [];
    const narrowedDefs = narrowOverloadCandidates(candidates.map((candidate) => candidate.def), expected?.parameterCount, expected?.parameterTypes);
