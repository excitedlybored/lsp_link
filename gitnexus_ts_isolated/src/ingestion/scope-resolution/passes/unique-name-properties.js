...
export function buildPropertyNameIndex(graph) {
    ...
    for (const node of graph.iterNodes()) {
        ...
        if (existing.some((c) => c.id === node.id))
            continue;
 ...
 * can disambiguate.
 */
function candidatesForLanguage(all, ownFilePaths) {
    const mine = [];
    for (const candidate of all) {
        if (!ownFilePaths.has(candidate.filePath))
            continue;
        if (mine.length >= MAX_TRACKED_CANDIDATES)
 ...
 */
function buildDirectImportMap(indexes, finalized) {
    ...
    for (const [scopeId, edges] of finalized.imports) {
        ...
        for (const edge of edges) {
            if (edge.targetFile === null || edge.targetFile === fromFile)
                continue;
...
function simpleNameOfGraphId(graphId) {
 ...
 */
function narrowToSingleCandidate(candidatesIn, readingFile, importedFiles, 
...
ownerName) {
    ...
    if (ownerName !== undefined && ownerName.length > 0) {
        ...
        if (own.length === 1)
            return { id: own[0].id, tier: 'own-return-shape' };
    }
...
export function emitUniqueNamePropertyAccesses(graph, indexes, parsedFiles, nodeLookup, 
...
reportOnly = false) {
    ...
    const directImports = finalized === undefined
        ? new Map()
    ...
    const crossLanguageAnchors = new Map();
    const seen = new Set();
        ...
        for (const site of parsed.referenceSites) {
            ...
            if (candidates === undefined) {
                ...
                if (!crossLanguageAnchors.has(site.name)) {
                    crossLanguageAnchors.set(site.name, new Set(allWithName.map((c) => languageOf(c.filePath))));
                }
                continue;
            }
            ...
            if (callerGraphId === undefined)
                continue;
            const choice = narrowToSingleCandidate(candidates, parsed.filePath, directImports.get(parsed.filePath), simpleNameOfGraphId(callerGraphId));
            if (choice === null) {
                ambiguous++;
                ambiguousNames.add(site.name);
                continue;
            ...
            if (choice.tier !== 'workspace-unique')
                narrowed++;
            // A property reading itself is not a fact about anything.
            if (callerGraphId === targetId)
                continue;
