 ...
 */
import { lookupBindingsAt, namesAtScope, moduleScopeIdOf, namespaceTypeBindingFor, } from '../scope/walkers.js';
/**
...
export function followChainPostFinalize(start, fromScopeId, scopes) {
    let current = start;
    ...
    for (let depth = 0; depth < RECHAIN_MAX_DEPTH; depth++) {
        ...
        if (next === undefined) {
            ...
            if (nsHit !== undefined && nsHit !== current)
                next = nsHit;
        }
        if (next === undefined) {
            const ws = scopes.workspaceTypeBindings?.get(current.rawName);
            if (ws !== undefined && ws !== current)
                next = ws;
        }
        if (next === undefined)
            return current;
