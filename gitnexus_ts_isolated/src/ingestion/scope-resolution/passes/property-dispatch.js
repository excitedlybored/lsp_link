...
export function emitPropertyDispatchCalls(graph, scopes, parsedFiles, nodeLookup, calleeIdSink) {
    const seen = new Set();
    let usesEmitted = 0;
    ...
    for (const parsed of parsedFiles) {
        for (const site of parsed.referenceSites) {
            if (site.kind !== 'value-ref')
                continue;
            const def = findCallableBindingInScope(site.inScope, site.name, scopes);
            if (def === undefined)
                continue;
            const ok = tryEmitEdge(graph, scopes, nodeLookup, site, def, 'scope-resolution: value-ref', seen);
            if (ok)
                usesEmitted++;
            if (site.propertyKey === undefined)
                continue;
    ...
    if (registrations.size > 0) {
        for (const parsed of parsedFiles) {
            const calleeCapture = calleeIdSink !== undefined ? { sink: calleeIdSink, filePath: parsed.filePath } : undefined;
            for (const site of parsed.referenceSites) {
                if (site.kind !== 'call' || site.callForm !== 'member')
                    continue;
                const defs = registrations.get(site.name);
                if (defs === undefined)
                    continue;
                for (const def of defs.values()) {
                    const ok = tryEmitEdge(graph, scopes, nodeLookup, site, def, 'property-dispatch', seen, PROPERTY_DISPATCH_CONFIDENCE, false, calleeCapture);
                    if (ok)
                        callsEmitted++;
                }
            }
