...
function pairwiseCompare(a, b) {
    ...
    for (let i = 0; i < len; i++) {
        if (a[i] < b[i])
            ...
            bBetter = true;
        if (aBetter && bBetter)
            return 0; // incomparable — early exit
    }
    if (aBetter && !bBetter)
        return -1;
    if (bBetter && !aBetter)
 ...
 */
export function isOverloadAmbiguousAfterNormalization(candidates, argCount) {
    ...
    if (compareUpTo === 0)
        return false;
    if (first.length < compareUpTo)
        return false;
    for (let i = 1; i < candidates.length; i++) {
        const p = candidates[i].parameterTypes;
        if (p === undefined)
            return false;
        if (p.length < compareUpTo)
            return false;
        ...
        if (argCount === undefined && p.length !== first.length)
            return false;
    }
    return true;
}
