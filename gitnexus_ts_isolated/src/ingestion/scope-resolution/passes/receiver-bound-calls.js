...
import { bindsTypeParameter, findClassBindingInScope, findEnclosingClassDef, isReceiverOwnedButUnbound, findExportedDef, findOwnedMember, findReceiverTypeBinding, findValueBindingInScope, isClassLike, isNamespaceNameShadowed, resolveClassBindingForName, } from '../scope/walkers.js';
import { tryEmitEdge, tryEmitEdgeWithExplicitTargetId, } from '../graph-bridge/edges.js';
import { resolveCompoundReceiverClass, resolveCompoundReceiverTyped, } from '../passes/compound-receiver.js';
import { erasedTypeApplication, typeApplicationArguments } from '../../utils/template-arguments.js';
import { heritageTypeArgumentsKey, stepHeritageInstantiation, } from '../utils/generic-instantiation.js';
 ...
 * production caller.
 */
export function classifyReceiverOrigin(decoded, inScope, receiverName, scopes, options = {}) {
    ...
    if (binding !== undefined) {
        ...
        if (resolveClassBindingForName(binding.declaredAtScope, binding.rawName, scopes, strip) !==
            undefined) {
            return 'in-program';
        }
    ...
    if (findClassBindingInScope(inScope, base, scopes, strip) !== undefined)
        return 'in-program';
    ...
    // same population.
    if (findValueBindingInScope(inScope, base, scopes) !== undefined)
        return 'in-program';
...
export function emitReceiverBoundCalls(graph, scopes, parsedFiles, nodeLookup, handledSites, provider, index, model, options = {}) {
    ...
    const graphIdToClassDefs = new Map();
    ...
    const classGraphIdByDefId = new Map();
    /**
    ...
    for (const relType of ['IMPLEMENTS', 'EXTENDS']) {
        for (const rel of graph.iterRelationshipsByType(relType)) {
            ...
            if (superDefs === undefined || subDefs === undefined)
                continue;
     ...
     */
    const groundTypeArgument = (name, scopeId) => {
        const def = scopeId === undefined ? undefined : resolveClassBindingForName(scopeId, name, scopes);
        return {
    ...
    receiverTypeSpelling) => {
        ...
        if (subtypesBySupertypeDefId.get(ownerDef.nodeId) === undefined)
            return 0;
        const receiverTypeArguments = receiverTypeSpelling === undefined
            ? undefined
        ...
        const resolveSupertypeArgument = (name) => groundTypeArgument(name, site.inScope);
        const targetByMemberId = new Map();
        const bestIncomingCount = new Map([[ownerDef.nodeId, 0]]);
        const queue = [
            {
        ...
        while (head < queue.length) {
            ...
            // before #2912.
            const superGraphId = current.typeArguments === undefined ? undefined : classGraphIdByDefId.get(current.typeId);
            for (const subDef of subtypesBySupertypeDefId.get(current.typeId) ?? []) {
                ...
                if (previousIncomingCount !== undefined &&
                ...
                let subtypeArguments;
                if (superGraphId !== undefined) {
                    const subGraphId = classGraphIdByDefId.get(subDef.nodeId);
                    const heritageArguments = subGraphId === undefined
                        ? undefined
                        : options.heritageTypeArguments?.get(heritageTypeArgumentsKey(subGraphId, superGraphId));
        ...
        const targets = [...targetByMemberId.values()]
            .sort((left, right) => left.syntheticOwnerPriority - right.syntheticOwnerPriority ||
        ...
        for (const implMember of targets) {
            const ok = tryEmitEdge(graph, scopes, nodeLookup, site, implMember, 'interface-dispatch', seen, confidence, collapse, calleeCapture);
            if (ok)
                n++;
        }
        return n;
    ...
    for (const parsed of parsedFiles) {
        ...
        for (const site of parsed.referenceSites) {
            ...
            if (isSuper) {
                ...
                if (enclosingClass !== undefined) {
                    ...
                    if (memberDef !== undefined) {
                        ...
                        const ok = tryEmitEdge(graph, scopes, nodeLookup, site, memberDef, 'global', seen, 0.85, collapse, calleeCapture);
                        if (ok)
                            emitted++;
            ...
            if (receiverName.includes('.') ||
                ...
                const resolved = resolveCompoundReceiverTyped(receiverName, site.inScope, scopes, index, 
                ...
                if (resolved !== undefined && currentClass !== undefined) {
                    ...
                    if (memberDef !== undefined) {
                        ...
                        const ok = tryEmitEdge(graph, scopes, nodeLookup, site, memberDef, memberDef.filePath !== parsed.filePath ? 'import-resolved' : 'global', seen, 0.85, collapse, calleeCapture);
                        if (ok)
                            emitted++;
                        ...
                        emitted += emitInterfaceDispatchFor(currentClass, memberName, memberDef, site, 0.85, calleeCapture, resolved.declaredSpelling);
                        // Always mark handled when the site was resolved, even
                        // if the edge was deduplicated (collapse mode), so
                        // `emitReferencesViaLookup` doesn't re-emit from the
                        // reference index.
            ...
            if (provider.resolveThisViaEnclosingClass === true && receiverName === 'this') {
                ...
                if (enclosingClass !== undefined) {
                    ...
                    if (languageResolution?.kind === 'resolved') {
                        ...
                        const ok = tryEmitEdge(graph, scopes, nodeLookup, site, memberDef, reason, seen, confidence, collapse, calleeCapture);
                        if (ok)
                            emitted++;
                        handledSites.add(siteKey);
                        continue;
            ...
            if (classDef !== undefined) {
                ...
                for (const ownerId of chain) {
                    ...
                    if (picked !== undefined) {
                        ...
                        if (narrowOverloadCandidates([memberDef], site.arity, site.argumentTypes).length === 0) {
                            memberDef = undefined;
                            break;
                        }
                        break;
            ...
            if (typeRef !== undefined && chainHead !== undefined && !namespaceTargets.has(chainHead)) {
                ...
                let resolved = resolveCompoundReceiverTyped(typeRef.rawName, typeRef.declaredAtScope, scopes, index, fileCompoundOpts);
                if (resolved === undefined && !typeRef.rawName.includes('(')) {
                    resolved = resolveCompoundReceiverTyped(typeRef.rawName + '()', typeRef.declaredAtScope, scopes, index, fileCompoundOpts);
                }
                ...
                if (resolved !== undefined && ownerDef !== undefined) {
                    ...
                    if (memberDef !== undefined) {
                        ...
                        emitted += emitInterfaceDispatchFor(ownerDef, memberName, memberDef, site, 0.85, calleeCapture, resolved.declaredSpelling);
                        // Always mark handled when the site was resolved, even
                        // if the edge was deduplicated (collapse mode), so
                        // `emitReferencesViaLookup` doesn't re-emit from the
                        // reference index.
            ...
            if (typeRef !== undefined && !typeRef.rawName.includes('.')) {
                ...
                let ownerDef = resolveClassBindingForName(site.inScope, typeApplication ?? typeRef.rawName, scopes);
                ...
                if (ownerDef === undefined && typeApplication === undefined) {
                    ...
                    if (qnameIds.length === 1) {
                        ...
                        if (qdef !== undefined && isClassLike(qdef.type))
                            ownerDef = qdef;
                    }
                }
                ...
                if (ownerDef === undefined && typeApplication === undefined) {
                    ownerDef = resolveCompoundReceiverClass(receiverName, site.inScope, scopes, index, 
                ...
                if (ownerDef !== undefined) {
                    ...
                    if (memberDef !== undefined) {
                        ...
                        emitted += emitInterfaceDispatchFor(ownerDef, memberName, memberDef, site, confidence, calleeCapture, typeApplication ?? typeRef.rawName);
                        // Always mark handled when the site was resolved, even
                        // if the edge was deduplicated (collapse mode), so
                        // `emitReferencesViaLookup` doesn't re-emit from the
                        // reference index.
                ...
                if (picked !== undefined) {
                    ...
                    const reason = site.kind === 'write' || site.kind === 'read'
                        ...
                        : picked.filePath !== parsed.filePath
                            ...
                            : 'global';
                    const confidence = site.kind === 'write' || site.kind === 'read' ? 1.0 : 0.85;
                    const ok = tryEmitEdgeWithExplicitTargetId(graph, scopes, nodeLookup, site, picked.nodeId, reason, seen, confidence, collapse, calleeCapture);
                    if (ok)
                        emitted++;
                    handledSites.add(siteKey);
                    continue;
            ...
            if (compoundReceiverUnresolved) {
                ...
                if (staticMemberReceiver !== undefined &&
                    ...
                    const receiverClass = declaringScope === undefined || fieldDeclaredType === undefined
                        ? undefined
                        : resolveClassBindingForName(declaringScope, fieldDeclaredType, scopes, provider.stripTypePreservingDecoration);
                    if (receiverClass !== undefined) {
                        ...
                        if (memberDef !== undefined) {
                            ...
                            emitted += emitInterfaceDispatchFor(receiverClass, memberName, memberDef, site, confidence, calleeCapture, fieldDeclaredType);
                            handledSites.add(siteKey);
                            continue;
                        }
                    }
