...
export function emitImportedValueReferences(graph, indexes, parsedFiles, nodeLookup, 
    ...
    for (const parsed of parsedFiles) {
        for (const site of parsed.referenceSites) {
            ...
            if (def.filePath === parsed.filePath)
                continue;
            if (tryEmitEdge(graph, indexes, nodeLookup, site, def, 'import-resolved', seen, IMPORTED_VALUE_CONFIDENCE)) {
                emitted++;
            }
        }
    }
