 ...
 */
import { erasedTypeApplication, matchingOpenParen, stripTemplateArguments, } from '../../utils/template-arguments.js';
import { decodeReceiverChain } from '../../utils/receiver-chain-codec.js';
import { findClassBindingInScope, resolveClassBindingForName, findEnclosingClassDef, findExportedDef, findExportedDefByName, findReceiverTypeBinding, isClassLike, isNamespaceNameShadowed, } from '../scope/walkers.js';
...
function resolveConstructionExpressionClass(fnExpr, inScope, scopes, index, options) {
    ...
    if (lastDot !== -1) {
        ...
        if (namespaceFiles.length > 0) {
            if (isNamespaceNameShadowed(namespaceName, inScope, scopes))
                return undefined;
            const namespaceMatches = namespaceFiles
                .map((targetFile) => findExportedDef(targetFile, exportedName, index))
                .filter((def) => def !== undefined && isClassLike(def.type));
    ...
    if (qualifiedIds.length === 1) {
        ...
        if (qualified !== undefined && isClassLike(qualified.type))
            return qualified;
    }
 ...
 */
function classOfDeclaredType(typeRef, scopes, stripDecoration, recordReceiverType) {
    ...
    const def = resolveClassBindingForName(typeRef.declaredAtScope, spelling, scopes, stripDecoration);
 ...
 */
function noteReceiverType(record, spelling, def) {
    if (def !== undefined)
        record?.(spelling, def.nodeId);
    return def;
}
/**
 ...
 */
function classOfReturnType(retType, scopes, record) {
    const def = findClassBindingInScope(retType.declaredAtScope, retType.rawName, scopes);
    if (def === undefined || record === undefined)
        return def;
...
function typeOfMemberOnClass(owner, memberName, scopes, index, options) {
    ...
    for (const ownerId of ownerChain) {
        ...
        if (memberType !== undefined) {
            const def = classOfDeclaredType(memberType, scopes, options.stripTypePreservingDecoration, options.recordReceiverType);
        ...
        if (classScope !== undefined && options.hoistTypeBindingsToModule === true) {
            ...
            while (curId !== null) {
                ...
                if (hoisted !== undefined) {
                    ...
                    const def = classOfDeclaredType(hoisted, scopes, options.stripTypePreservingDecoration, options.recordReceiverType);
 ...
 */
export function foldReceiverChain(chain, inScope, scopes, index, options = {}) {
    ...
    for (const step of chain.steps) {
        ...
        if (step.kind === 'index') {
            ...
            const elementClass = findClassBindingInScope(scopeForLookup, element, scopes, options.stripTypePreservingDecoration);
 ...
 */
export function resolveCompoundReceiverTyped(receiverText, inScope, scopes, index, options = {}) {
    let spelling;
    let spellingDefId;
...
export function resolveCompoundReceiverClass(receiverText, inScope, scopes, index, options = {}, depth = 0) {
    ...
    if (!workingText.includes('.') && !workingText.includes('(')) {
        ...
        if (tb !== undefined) {
            ...
            const viaTb = classOfDeclaredType(tb, scopes, options.stripTypePreservingDecoration, options.recordReceiverType);
            if (viaTb !== undefined)
                return viaTb;
 ...
 */
export function stripCastWrappers(text) {
    let castType;
    let workingText = text;
    let peels = 0;
    while (true) {
