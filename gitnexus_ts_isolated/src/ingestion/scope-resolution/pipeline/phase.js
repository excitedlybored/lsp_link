...
export function selectScopeSourcePathsToRead(provider, primaryFilePaths, preExtractedByPath) {
    ...
    const needsAllSourceText = hasPostExtractHooks && provider.postExtractSourceTextPolicy !== 'uncached-files';
...
export const scopeResolutionPhase = {
    ...
    async execute(ctx, deps) {
        ...
        for (const f of scannedFiles) {
            ...
            if (resolver?.languageProvider.parseStrategy !== 'standalone' &&
        ...
        // to.
        const sharedPropertyNameIndex = totalScopeFiles > 0 ? buildPropertyNameIndex(ctx.graph) : undefined;
        ...
        const crossLanguageAnchorsByName = new Map();
        let ambiguousPropertyReads = 0;
        const ambiguousPropertyNames = new Set();
        let pdgEmitManifest;
        ...
        try {
            for (const [lang, provider] of SCOPE_RESOLVERS) {
                ...
                if (provider.collectScopeContextPaths !== undefined) {
                    ...
                    await loadStoreFor(scopeFilePaths);
                    const pathsToRead = selectScopeSourcePathsToRead(provider, primaryFilePaths, preExtractedByPath);
                ...
                for (const n of stats.uniqueNamePropertyAmbiguousNames)
                    ambiguousPropertyNames.add(n);
            ...
            if (crossLanguagePropertyReads > 0) {
                const sample = Array.from(crossLanguageAnchorsByName)
                    ...
                    .join(', ');
                logger.warn(`[scope-resolution] ${crossLanguagePropertyReads} property read/write site(s) name a field ` +
