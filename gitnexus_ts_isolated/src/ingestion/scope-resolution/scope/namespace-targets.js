 ...
 */
export function collectNamespaceTargets(parsed, scopes, options) {
    const out = new Map();
    const moduleEdges = scopes.imports.get(parsed.moduleScope);
    if (moduleEdges === undefined)
        return out;
    const addTarget = (key, targetFile) => {
        let targets = out.get(key);
        if (targets === undefined) {
            targets = [];
            out.set(key, targets);
        }
        if (!targets.includes(targetFile))
            targets.push(targetFile);
    };
    const moduleFileExists = options?.moduleFileExists ?? (() => false);
    for (const edge of moduleEdges) {
        if (edge.targetFile === null || edge.kind !== 'namespace')
            continue;
        ...
        for (const [spelling, targetFile] of spellings)
            addTarget(spelling, targetFile);
    }
    return out;
}
