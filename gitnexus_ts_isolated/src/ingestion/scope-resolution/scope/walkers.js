 ...
 */
export function isReceiverOwnedButUnbound(startScope, receiverName, scopes) {
 ...
 * this walk wants.
 */
export function isNamespaceNameShadowed(namespaceName, inScope, scopes) {
 ...
 */
export function findAllClassBindingsInScope(startScope, name, scopes) {
 ...
 */
function declarationOpenedScope(def, scope) {
    if (scope.kind === 'Module')
        return false;
...
function typeParameterNamesInScope(scopeId, scopes) {
    ...
    for (let i = chain.length - 1; i >= 0; i -= 1) {
        ...
        for (const def of scope?.ownedDefs ?? []) {
            if (def.typeParameters === undefined)
                continue;
            if (scope === undefined || !declarationOpenedScope(def, scope))
                continue;
 ...
 */
export function bindsTypeParameter(scopeId, name, scopes) {
    if (name.length === 0)
        return false;
 ...
 */
export function resolveClassBindingForName(scopeId, rawClassName, scopes, 
/**
 ...
 */
export function findShapeBindingInScope(startScope, receiverName, scopes) {
 ...
 */
export function findAllCallableBindingCandidatesInScope(startScope, callableName, scopes) {
    let currentId = startScope;
    const visited = new Set();
    while (currentId !== null) {
        if (visited.has(currentId))
...
export function findAllCallableBindingsInScope(startScope, callableName, scopes) {
 ...
 * detection.
 */
export function findCallableBindingsAndAdlBlocker(startScope, name, scopes) {
    let currentId = startScope;
    const visited = new Set();
    while (currentId !== null) {
        if (visited.has(currentId))
 ...
 */
export function tagNamespacePrefixes(parsed, options = {}) {
