...
export const buildCommunityProjection = (knowledgeGraph) => {
    ...
    knowledgeGraph.forEachRelationshipFields((sourceId, targetId, type, confidence) => {
        if (!isClusteringRelationship(type) || sourceId === targetId)
            return;
        if (isLarge && confidence < MIN_CONFIDENCE_LARGE)
            return;
...
const runCommunityEngine = async (graph, projection, engineRequested, options, onProgress) => {
    ...
    onProgress?.(`Experimental ${engineRequested} community engine requested — unsupported, and its ` +
        'communities will not match the Graphology default.', 32);
    try {
...
const isIntegerPartition = (partition) => {
    for (let index = 0; index < partition.length; index++) {
        if (!Number.isSafeInteger(partition[index]))
            return false;
    }
    return true;
};
 ...
 * Graphology.
 */
export const buildIcebugWorkerSource = (moduleSpecifier) => `
