...
const lookupInEnv = (env, varName, callNode, patternOverrides, enclosingFunctionFinder, extractFunctionNameHook, filePath, thisBoundaryNodeTypes) => {
 ...
 */
const findEnclosingClassName = (node, thisBoundaryNodeTypes) => {
 ...
 */
const substituteThisReceiver = (item, node, thisBoundaryNodeTypes) => {
...
export const buildTypeEnv = (tree, language, options) => {
    ...
    return {
        lookup: (varName, callNode) => lookupInEnv(env, varName, callNode, patternOverrides, options?.enclosingFunctionFinder, extractFuncNameHook, options?.filePath, config.thisBoundaryNodeTypes),
