...
import { extractAnnotations, findVisibility, hasKeyword, hasModifier, typeFromField, } from './helpers.js';
...
function isInsideKotlinCompanion(node) {
    for (let cur = node.parent; cur !== null; cur = cur.parent) {
        if (cur.type === 'class_body')
            return cur.parent?.type === 'companion_object';
