 ...
 */
export const SINGLE_ARG_CONTAINERS = [
    'list',
    'List',
    'set',
    'Set',
...
export const MAPPING_CONTAINERS = [
    'dict',
    'Dict',
    'Mapping',
    'MutableMapping',
...
const SINGLE_ARG_CONTAINER_RE = new RegExp(`^${QUALIFIER}(?:${SINGLE_ARG_CONTAINERS.join('|')})\\[([^,\\]]+)\\]$`);
const MAPPING_CONTAINER_RE = new RegExp(`^${QUALIFIER}(?:${MAPPING_CONTAINERS.join('|')})\\[[^,\\]]+,\\s*([^\\]]+)\\]$`);
/**
 ...
 */
function isNotAUserGenericBase(base) {
 ...
 */
export const NOT_A_USER_GENERIC_SPELLINGS = [
...
const NOT_A_USER_GENERIC = new Set(NOT_A_USER_GENERIC_SPELLINGS.map((name) => name.toLowerCase()));
/**
