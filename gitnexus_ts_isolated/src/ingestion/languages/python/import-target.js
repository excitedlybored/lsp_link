...
import { getPythonFileIndex, importerAncestors, importerDirOf, } from '../../import-resolvers/python-file-index.js';
...
export function resolvePythonImportTarget(parsedImport, workspaceIndex) {
    ...
    if (submoduleTarget !== null &&
        ...
        // base-package lookup.
        const packageTarget = resolvePythonImportTarget({ ...parsedImport, targetIncludesImportedName: true }, workspaceIndex);
 ...
 * per pass.
 */
function pythonFileExportsName(targetFile, importedName, parsedFiles) {
 ...
 */
const parsedFileByPath = perFileSet((parsedFiles) => {
...
function resolveAbsoluteFromFiles(pathLike, allFilePaths, fromFile) {
    ...
    for (const cand of moduleCandidates ?? []) {
        if (cand.norm.endsWith(suffixFile))
            matches.push(cand);
    }
    ...
    for (const cand of packageCandidates ?? []) {
        if (cand.norm.endsWith(suffixPkg))
            matches.push(cand);
    }
    if (matches.length === 0)
        return null;
...
function hasRepoCandidate(leadingSegment, allFilePaths, fromFile) {
    ...
    const index = getPythonFileIndex(allFilePaths);
    if (index.normSet.has(rootFile) || index.normSet.has(initFile))
        return true;
    if (index.dirPrefixes.has(prefix))
        return true;
    ...
    if (!index.nestedDirNames.has(leadingSegment))
        return false;
    for (const ancestor of importerAncestors(index, importerDirOf(fromFile))) {
        if (index.dirPrefixes.has(`${ancestor}/${prefix}`))
            return true;
    }
    return false;
}
function pythonImportedSubmoduleTarget(parsedImport) {
    if (parsedImport.kind !== 'named' && parsedImport.kind !== 'alias')
        return null;
    if (parsedImport.targetIncludesImportedName === true)
        return null;
 ...
 */
export function isPythonImportedModule(parsedImport, targetFile, fromFile) {
    const submoduleTarget = pythonImportedSubmoduleTarget(parsedImport);
    if (submoduleTarget === null)
        return false;
    ...
    return (normalizedTarget === moduleFile ||
 ...
 */
export function pythonNamespaceReceiverPaths(edge, moduleFileExists) {
    const segments = edge.importPath.split('.');
    if (segments.length < 2)
        return undefined;
    if (segments[0] !== edge.localName)
        return undefined;
    const out = [[edge.importPath, edge.targetFile]];
    ...
    for (let i = 1; i < segments.length; i++) {
        ...
        if (moduleFileExists(packageFile))
            out.push([spelling, packageFile]);
