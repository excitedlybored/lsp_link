 ...
 */
function constructorCallTypeName(right, receiverName) {
 ...
 */
export function synthesizeConstructorFieldTypeBindings(fnNode) {
    if (fnNode.childForFieldName('name')?.text !== '__init__')
        return [];
    if (findEnclosingClassDefinition(fnNode) === null)
        return [];
    if (hasDecorator(fnNode, 'staticmethod') || hasDecorator(fnNode, 'classmethod'))
        return [];
    ...
    for (let i = 0; i < parameters.namedChildCount; i++) {
        ...
        if (name !== null && annotation !== null)
            parameterTypes.set(name, annotation.text);
    }
        ...
        if (node.type === 'assignment') {
            ...
            if (left?.type === 'attribute') {
                ...
                if (object?.type === 'identifier' && object.text === receiverName && field !== null) {
                    ...
                    const parameterType = right?.type === 'identifier' ? parameterTypes.get(right.text) : undefined;
                    ...
                    const constructedType = explicitType === null && parameterType === undefined
                        ...
                        if (existing === undefined || tier >= existing.tier) {
                            candidates.set(field.text, {
                                ...
                                match: {
                                    ...
                                    '@type-binding.type': syntheticCapture('@type-binding.type', explicitType ?? right ?? field, typeName),
                                    ...
                                    ...(explicit
                                        ...
                                        : inferredFromConstructor
                                            ? {
                                                '@type-binding.constructor': syntheticCapture('@type-binding.constructor', right ?? field, '1'),
                                            }
                                            : {
                                                '@type-binding.parameter': syntheticCapture('@type-binding.parameter', right ?? field, '1'),
                                            }),
                                    '@type-binding.instance-field': syntheticCapture('@type-binding.instance-field', node, '1'),
                                },
                            });
                        }
                    }
