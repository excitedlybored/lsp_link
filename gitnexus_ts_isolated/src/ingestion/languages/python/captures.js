...
import { synthesizeConstructorFieldTypeBindings, synthesizeReceiverTypeBinding, } from './receiver-binding.js';
        ...
        if (grouped['@scope.function'] !== undefined) {
            ...
            if (fnNode !== null) {
                ...
                if (parameterNames.length > 0) {
                    grouped['@scope.lexical-names'] = syntheticCapture('@scope.lexical-names', fnNode, JSON.stringify(parameterNames));
                }
 ...
 * `inherits` kind here.
 */
function synthesizePythonInheritanceReferences(root) {
    const out = [];
    walkNamedTree(root, (node) => {
        if (node.type !== 'class_definition')
            return;
        const superclasses = node.childForFieldName('superclasses');
        if (superclasses === null || superclasses.type !== 'argument_list')
            return;
 ...
 */
function pythonBaseLookupNameNode(base) {
    switch (base.type) {
        case 'identifier':
            return base;
        case 'attribute': {
