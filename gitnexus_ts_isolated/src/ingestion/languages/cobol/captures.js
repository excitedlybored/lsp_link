...
export function emitCobolScopeCaptures(sourceText, _filePath, _cachedTree) {
    ...
    const out = [];
    const procedurePointers = new Set(extracted.dataItems
    ...
    for (const line of cleaned.split(/\r?\n/)) {
        const declaration = line.match(/^\s*\d{1,2}\s+([A-Z][A-Z0-9-]*).*\b(?:PROCEDURE|FUNCTION)-POINTER\b/i);
        if (declaration !== null)
            procedurePointers.add(declaration[1].toUpperCase());
    }
    ...
    for (const program of extracted.programs) {
        for (let index = 0; index < (program.procedureUsing?.length ?? 0); index++) {
            ...
            out.push({
                ...
                '@callable-flow.binding': capture('@callable-flow.binding', rangeOf(program.startLine, 0, program.startLine, parameter.length), parameter),
                '@callable-flow.parameter-index': capture('@callable-flow.parameter-index', ownerRange, String(index)),
                '@callable-flow.passing-mode': capture('@callable-flow.passing-mode', ownerRange, 'reference'),
            });
        }
    }
    for (let index = 0; index < lines.length; index++) {
        ...
        const entry = code.match(/\bSET\s+([A-Z0-9][A-Z0-9-]*)\s+TO\s+ENTRY\s+(?:"([^"]+)"|'([^']+)')/i);
        if (entry !== null) {
            ...
            if (procedurePointers.has(destination.toUpperCase())) {
                out.push({
                    ...
                    '@callable-flow.destination': capture('@callable-flow.destination', lineRange, destination),
    ...
    for (const call of extracted.calls) {
        ...
        for (let index = 0; index < (call.parameters?.length ?? 0); index++) {
            ...
            out.push({
                ...
                '@callable-flow.parameter-index': capture('@callable-flow.parameter-index', lineRange, String(index)),
                ...(!call.isQuoted && procedurePointers.has(call.target.toUpperCase())
                    ? {}
                    : {
                        '@callable-flow.direct-callee-name': capture('@callable-flow.direct-callee-name', lineRange, call.target),
                    }),
            });
        }
        if (!call.isQuoted && procedurePointers.has(call.target.toUpperCase())) {
            out.push({
                ...
                '@callable-flow.invocation-kind': capture('@callable-flow.invocation-kind', lineRange, 'indirect'),
                '@callable-flow.arity': capture('@callable-flow.arity', lineRange, String(call.parameters?.length ?? 0)),
            });
        }
    }
    return out;
 ...
 */
function findProgramIdNameColumn(lines, lineNum) {
