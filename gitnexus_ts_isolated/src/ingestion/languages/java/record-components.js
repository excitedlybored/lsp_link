...
function recordComponents(recordNode) {
    const parameters = recordNode.childForFieldName('parameters');
    if (parameters === null)
        return [];
    return parameters.namedChildren.filter((node) => node !== null && (node.type === 'formal_parameter' || node.type === 'spread_parameter'));
}
/**
 ...
 */
function isRecordComponentName(node) {
    return (node !== null &&
        node !== undefined &&
...
function recordComponentNameNode(component) {
    const name = component.type === 'formal_parameter'
 ...
 */
const explicitZeroArgAccessorNamesCache = new WeakMap();
function explicitZeroArgAccessorNames(recordNode) {
...
function computeExplicitZeroArgAccessorNames(recordNode) {
    const names = new Set();
    const body = recordNode.childForFieldName('body');
    if (body === null)
        return names;
    for (const node of body.namedChildren) {
        if (node === null || node.type !== 'method_declaration')
            continue;
        const name = node.childForFieldName('name')?.text;
        const parameters = node.childForFieldName('parameters');
        const parameterCount = parameters?.namedChildren.filter((parameter) => parameter !== null &&
            (parameter.type === 'formal_parameter' || parameter.type === 'spread_parameter')).length ?? 0;
        if (name !== undefined && parameterCount === 0)
            names.add(name);
    }
    return names;
}
function recordComponentReturnType(component) {
    const typeNode = component.childForFieldName('type') ??
        (component.type === 'spread_parameter'
            ? component.namedChildren.find((node) => node?.type !== 'modifiers' && node?.type !== 'variable_declarator')
            : undefined);
...
function implicitAccessorInfo(component, context) {
...
export function synthesizeJavaRecordComponentAccessorCaptures(rootNode) {
    const captures = [];
    for (const recordNode of rootNode.descendantsOfType('record_declaration')) {
        ...
        for (const component of recordComponents(recordNode)) {
            ...
            captures.push({
                ...
                '@declaration.parameter-count': syntheticCapture('@declaration.parameter-count', component, '0'),
                '@declaration.required-parameter-count': syntheticCapture('@declaration.required-parameter-count', component, '0'),
                '@declaration.return-type': syntheticCapture('@declaration.return-type', component, returnType),
            });
        }
    }
    return captures;
 ...
 */
export function shouldSkipJavaRecordComponentDefinition(captureMap) {
    ...
    if (!isRecordComponentName(nameNode))
        return true;
