import { materializeClassAnnotationFacts, recordClassAnnotationCapture, } from '../../frameworks/spring/bean-candidates.js';
import { javaLocalTypeDeclarationContainer, nodeIfType, nodeToCapture, synthesizeJavaTypeIdentity, syntheticCapture, } from '../../utils/ast-helpers.js';
...
import { setJavaClassAnnotationFacts, setJavaSpringAopFacts, setJavaSpringConfigConsumerFacts, setJavaSpringConditionalFacts, setJavaSpringDiFacts, setJavaSpringNonHttpHandlerFacts, } from './capture-side-channel.js';
...
import { captureJavaSpringDiClassFact } from './spring-di.js';
import { synthesizeReceiverChainCapture } from '../../utils/receiver-chain-captures.js';
import { captureJavaSpringAopFacts } from './spring-aop.js';
import { captureJavaSpringConditionalFacts, } from './spring-conditionals.js';
import { captureJavaSpringNonHttpHandlerFacts, } from './spring-non-http-handlers.js';
...
function javaLocalTypeVisibilityScope(node) {
...
export function emitJavaScopeCaptures(sourceText, filePath, cachedTree) {
    let tree = cachedTree;
    if (tree === undefined) {
    ...
    const springDiFacts = [];
    const springNonHttpHandlerFacts = [];
    ...
    for (const m of rawMatches) {
        ...
        if (springDiClassNode !== null && !springDiClassNodeIds.has(springDiClassNode.id)) {
            ...
            springConditionalFacts.push(...captureJavaSpringConditionalFacts(springDiClassNode, filePath));
            springNonHttpHandlerFacts.push(...captureJavaSpringNonHttpHandlerFacts(springDiClassNode, filePath));
        ...
        const localTypeIdentity = typeDeclaration === undefined ? undefined : synthesizeJavaTypeIdentity(typeDeclaration);
        if (localTypeIdentity?.bindingName !== undefined &&
            grouped['@declaration.name'] !== undefined &&
            typeDeclaration !== undefined) {
            grouped['@declaration.binding-name'] = grouped['@declaration.name'];
            grouped['@declaration.name'] = syntheticCapture('@declaration.name', typeDeclaration, localTypeIdentity.name);
            const visibilityScope = javaLocalTypeVisibilityScope(typeDeclaration);
            if (visibilityScope !== undefined)
                out.push(visibilityScope);
        }
        ...
        if (grouped['@reference.receiver'] !== undefined) {
            ...
            if (simpleName !== undefined) {
                grouped['@reference.receiver'] = syntheticCapture('@reference.receiver', receiverNode, simpleName);
            }
        }
        ...
        if (callTag !== undefined && grouped['@reference.arity'] === undefined) {
            ...
            const callNode = nodeIfType(nodeMap[callTag], 'method_invocation', 'object_creation_expression');
            if (callNode !== null) {
                ...
                const args = argList === null
                    ? []
                    : argList.namedChildren.filter((c) => c !== null && c.type !== 'block_comment' && c.type !== 'line_comment');
    ...
    setJavaSpringConfigConsumerFacts(filePath, captureJavaSpringConfigConsumerFacts(tree.rootNode, filePath));
 ...
 */
function synthesizeJavaAnonymousClassDeclarations(rootNode) {
    const out = [];
    for (const oce of rootNode.descendantsOfType('object_creation_expression')) {
        ...
        const baseSimpleName = constructedType !== null && constructedType !== undefined
    ...
    for (const constant of rootNode.descendantsOfType('enum_constant')) {
        ...
        if (bodiedIdentity !== undefined && isBodied) {
            out.push({
                ...
                '@declaration.is-synthetic': syntheticCapture('@declaration.is-synthetic', bodyNode, 'true'),
            });
...
function javaEnclosingEnumNameOf(constant) {
    let cursor = constant.parent;
    while (cursor !== null) {
 ...
 */
function synthesizeJavaExplicitConstructorReferences(root) {
    const out = [];
    const stack = [root];
...
function emitJavaExplicitConstructorRef(out, node) {
 ...
 */
function synthesizeJavaInheritanceReferences(root) {
    const out = [];
    const stack = [root];
    while (stack.length > 0) {
        const node = stack.pop();
        ...
        if (node.type === 'class_declaration' ||
            ...
            if (interfaces !== null) {
                for (const typeList of interfaces.namedChildren) {
                    if (typeList === null || typeList.type !== 'type_list')
                        continue;
            ...
            for (let i = 0; i < node.namedChildCount; i++) {
                ...
                if (extendsInterfaces === null || extendsInterfaces.type !== 'extends_interfaces')
                    continue;
...
function emitJavaInheritanceBase(out, base) {
    if (base === null)
        return;
    const nameNode = javaBaseLookupNameNode(base);
    if (nameNode === null)
