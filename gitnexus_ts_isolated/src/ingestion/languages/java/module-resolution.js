 ...
 */
export function buildJavaPackageIndex(parsedFiles, packageOf) {
    ...
    for (const parsed of parsedFiles) {
        ...
        if (typeName !== null) {
            ...
            if (!types.has(typeName))
                types.set(typeName, filePath);
        }
        const files = filesByPackage.get(packageName);
        if (files === undefined)
            filesByPackage.set(packageName, [filePath]);
        else
            files.push(filePath);
    }
 ...
 */
export function resolveJavaModule(targetRaw, index) {
    if (targetRaw === '')
        return null;
 ...
 */
function resolveTypeName(qualified, index) {
...
function baseTypeName(filePath) {
