 ...
 */
export function populateCppRangeBindings(parsedFiles, _indexes, ctx) {
        ...
        if (cachedTree !== undefined) {
            ...
            try {
                ...
                if (err instanceof ParseTimeoutError) {
                    logger.warn({ file: parsed.filePath }, 'cpp range-binding: parse timed out, skipping file');
                    continue;
                }
                throw err;
            }
 ...
 * extraction.
 */
function buildParamTemplateMap(rootNode) {
    const map = new Map();
 ...
 */
function extractLoopVarNames(declarator) {
 ...
 */
function extractSourceVarName(right) {
 ...
 */
function extractCppElementType(rawType) {
