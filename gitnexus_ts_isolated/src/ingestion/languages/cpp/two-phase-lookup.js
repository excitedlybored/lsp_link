 ...
 */
const dependentPackBaseClassesByFile = new Map();
/**
...
export function markCppDependentPackBase(filePath, className) {
...
export function collectCppTwoPhaseSideChannel(filePath) {
    const perFile = dependentBasesByFile.get(filePath);
    const dependentBases = [];
    if (perFile !== undefined) {
...
export function applyCppTwoPhaseSideChannel(filePath, data) {
...
export function populateCppDependentBases(parsedFiles) {
    if (dependentBasesByFile.size === 0 && dependentPackBaseClassesByFile.size === 0)
        return;
    ...
    for (const parsed of parsedFiles)
        parsedByFile.set(parsed.filePath, parsed);
 ...
 */
export function isCppDependentBaseMember(callerScopeId, candidateDef, scopes) {
    ...
    if (bases.has('*pack-expansion*')) {
        if (candidateDef.ownerId !== undefined)
            return candidateDef.ownerId !== enclosing.nodeId;
        if (candidateDef.type !== 'Method' && candidateDef.type !== 'Constructor')
            return false;
...
function getQualifiedParentName(qualifiedName) {
...
function getQualifiedSimpleName(qualifiedName) {
    if (qualifiedName === undefined)
        return undefined;
    const lastDot = qualifiedName.lastIndexOf('.');
