...
import { expandCppWildcardNames, isFileLocal, clearFileLocalNames, populateCppAnonymousNamespaceScopes, populateCppNonGloballyVisible, isCppDefGloballyVisible, } from './file-local-linkage.js';
import { populateCppDependentBases, clearCppDependentBases, isCppDependentBaseMember, } from './two-phase-lookup.js';
 ...
 */
const augmentedFilePathsFor = perFileSet((allFilePaths) => perFileSet((headerPaths) => {
...
export const cppScopeResolver = {
    ...
    resolveImportTarget: (targetRaw, fromFile, allFilePaths, resolutionConfig) => {
        ...
        if (headerPaths !== undefined && headerPaths.size > 0) {
            return resolveCppImportTarget(targetRaw, fromFile, augmentedFilePathsFor(allFilePaths)(headerPaths));
        }
        return resolveCppImportTarget(targetRaw, fromFile, allFilePaths);
    },
    expandsWildcardTo: (targetModuleScope, parsedFiles) => expandCppWildcardNames(targetModuleScope, parsedFiles),
    ...
    isSuperReceiverInContext: (text, callerScope, scopes) => {
        ...
        if (lastDoubleColon >= 0)
            lhs = lhs.slice(lastDoubleColon + 2).trim();
        if (lhs.length === 0)
            return false;
        ...
        if (lhsDef.nodeId === enclosing.nodeId)
            return false;
    ...
    isFileLocalDef: (def) => {
        ...
        if (isFileLocal(def.filePath, simple))
            return true;
        ...
        if (!isCppDefGloballyVisible(def.filePath, def.nodeId))
            return true;
        return false;
    },
    ...
    hasFileLocalCallableLinkage: (def) => {
        ...
        if (def.type === 'Method' || def.type === 'Constructor')
            return false;
        const simple = def.qualifiedName?.split('.').pop() ?? def.qualifiedName ?? '';
    ...
    isCallableVisibleFromCaller: ({ candidate, callerScope, scopes }) => {
        if (callerScope === undefined || scopes === undefined)
            return true;
    ...
    resolveAdlCandidates: (site, callerParsed, scopes, parsedFiles) => {
        ...
        if (usingNamedHits.length === 0)
            return adlHits;
        if (adlHits === undefined || adlHits.length === 0)
            return usingNamedHits;
        const merged = [];
        const seen = new Set();
    ...
    resolveQualifiedReceiverMember: (receiverName, memberName, _callerScope, scopes, parsedFiles, callsite) => resolveCppQualifiedNamespaceMember(receiverName, memberName, parsedFiles, scopes, callsite),
};
