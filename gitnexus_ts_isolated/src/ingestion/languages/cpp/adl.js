 ...
 */
const argInfoSiteKeysByFile = new Map();
const noAdlSiteKeysByFile = new Map();
...
function pushFileSiteKey(idx, filePath, key) {
...
function buildAdlIndex(scopes, parsedFiles) {
    const idx = {
        ...
        for (const scope of parsed.scopes) {
            ...
            if (isCppInlineNamespaceScope(scope.id)) {
                ...
                if (parentScope !== undefined && parentScope.kind === 'Namespace') {
                    ...
                    if (parentQName !== '' && parentQName !== qName)
                        keys.push(parentQName);
                }
            }
            for (const def of scope.ownedDefs) {
                if (def.type === 'Function' || def.type === 'Method') {
                    ...
                    if (qName !== '')
                        pushNested(idx.nsFunctionsByQName, qName, sn, def);
                }
                if (!isAdlCallableType(def.type))
                    continue;
                const s = seq++;
                idx.seqByNodeId.set(def.nodeId, s);
                const sn = adlSimpleName(def);
                for (const key of keys)
                    pushNested(idx.nsCandidates, key, sn, def);
            }
        }
        ...
        for (const scope of parsed.scopes) {
            ...
            for (const childScope of childrenByParent.get(scope.id) ?? []) {
                ...
                for (const def of childScope.ownedDefs) {
                    ...
                    idx.seqByNodeId.set(def.nodeId, s);
                    const sn = adlSimpleName(def);
                    for (const key of classNsKeys)
                        pushNested(idx.friendCandidates, key, sn, def);
                }
            }
    ...
    if (process.env.NODE_ENV !== 'production' && process.env.VALIDATE_SEMANTIC_MODEL !== '0') {
        ...
        if (missing.length > 0) {
            throw new Error(`[cpp-adl] seq-coverage invariant violated: ${missing.length} candidate def(s) ` +
 ...
 * and its unit test.
 */
export function validateAdlSeqCoverage(idx) {
        ...
        for (const bySimple of buckets.values()) {
            for (const defs of bySimple.values()) {
                for (const def of defs) {
                    if (!idx.seqByNodeId.has(def.nodeId))
...
function ensureAdlIndex(scopes, parsedFiles) {
...
export function markCppAdlSiteArgs(filePath, line, col, args) {
...
export function markCppAdlSiteNoAdl(filePath, line, col) {
    const key = siteKey(filePath, line, col);
    // Lockstep with `noAdlSiteKeysByFile`: index the key only on first insert.
    if (!noAdlSites.has(key))
 ...
 */
export function collectCppAdlSideChannel(filePath) {
    const args = [];
...
export function applyCppAdlSideChannel(filePath, data) {
...
export function clearCppAdlState() {
    argInfoBySite.clear();
    noAdlSites.clear();
 ...
 */
export function populateCppAssociatedNamespaces(parsed) {
    const scopesById = new Map();
    for (const scope of parsed.scopes)
 ...
 */
export function pickCppAdlCandidates(site, callerParsed, scopes, parsedFiles) {
    ...
    for (const arg of args) {
        ...
        if (arg.functionRefText !== undefined) {
            collectFunctionTypeAssociatedNamespaces(arg.functionRefText, scopes, idx, associatedNamespaces);
        }
    }
...
function collectAssociatedNamespacesForAdlArg(arg, scopes, idx, associatedNamespaces) {
...
function addAssociatedNamespaceForClassName(simpleClassName, scopes, idx, associatedNamespaces) {
    if (simpleClassName.length === 0)
        return;
    const classLookup = findCppClassDefBySimpleName(idx, simpleClassName);
    if (classLookup === undefined)
        return;
    ...
    for (const ancestorDefId of scopes.methodDispatch.mroFor(classDef.nodeId)) {
        ...
        if (ancestorNsQName !== undefined)
            associatedNamespaces.add(ancestorNsQName);
    }
}
...
function computeEnclosingNamespaceQName(classScope, scopesById) {
    let parentId = classScope.parent;
    while (parentId !== null) {
 ...
 *  module scope). */
function computeNamespaceQName(nsScope, scopesById) {
    const segments = [];
    let currentId = nsScope.parent;
    let current = nsScope;
...
function findCppClassDefBySimpleName(idx, simpleName) {
 ...
 */
function collectFunctionTypeAssociatedNamespaces(refText, scopes, idx, out) {
    ...
    if (colonIdx !== -1) {
        ...
        if (matches !== undefined) {
            for (const def of matches)
                collectAssociatedNamespacesForFunctionDef(def, scopes, idx, out);
        }
        return;
    }
...
function collectAssociatedNamespacesForFunctionDef(def, scopes, idx, out) {
...
function collectAssociatedNamespacesForFunctionTypeText(typeText, scopes, idx, out) {
