 ...
 */
export function interpretCppImport(captures) {
 ...
 */
export function interpretCppTypeBinding(captures) {
    ...
    const effectiveType = captures['@type-binding.field'] === undefined ? type : cppQualifiedTail(type);
    ...
    const declaredSpelling = cppPointerSpelling(captures, effectiveType, name) ??
 ...
 */
function cppQualifiedTail(text) {
    let angleDepth = 0;
    let lastSeparator = -1;
...
const squeezeWhitespace = (text) => text.replace(/\s+/g, '');
/**
 ...
 */
function cppPointerSpelling(captures, typeText, nameText) {
    ...
    for (const anchor of CPP_WHOLE_DECLARATION_ANCHORS) {
        ...
        if (squeezeWhitespace(text) === target)
...
const CPP_SPECIFIER_RE = /\b(const|volatile|restrict|static|extern|inline|mutable|constexpr|consteval)\b/g;
/** Remove C++ declaration specifiers from `text` and trim the result. */
export function stripCppSpecifiers(text) {
