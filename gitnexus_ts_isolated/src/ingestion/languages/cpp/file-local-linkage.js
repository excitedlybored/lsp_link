...
export function markFileLocal(filePath, name) {
...
export function applyCppFileLocalSideChannel(filePath, data) {
    for (const name of data.fileLocalNames)
        markFileLocal(filePath, name);
    if (data.anonymousNamespaceRanges.length > 0) {
        ...
        for (const r of data.anonymousNamespaceRanges)
            set.add(r);
    }
}
/** Clear tracked file-local names (call at start of each resolution pass). */
export function clearFileLocalNames() {
...
export function populateCppNonGloballyVisible(parsed) {
    ...
    for (const scope of parsed.scopes) {
        if (scope.kind !== 'Namespace' && scope.kind !== 'Class')
            continue;
 ...
 */
export function isCppDefGloballyVisible(filePath, nodeId) {
 ...
 */
const moduleScopeIndex = perFileSet((parsedFiles) => {
    const index = new Map();
...
export function expandCppWildcardNames(targetModuleScope, parsedFiles) {
    ...
    for (const def of target.localDefs) {
        ...
        const ownerIsAnonymousNamespace = ownerScope !== undefined &&
