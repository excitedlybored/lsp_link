...
const C_CALLABLE_CAPTURE_OPTIONS = {
    ...
    parameterPassingMode: (parameter) => containsNodeType(parameter, 'pointer_declarator') ? 'pointer' : 'value',
    expectedSignature: (container, destination) => functionDeclaratorSignature(destination) ?? functionDeclaratorSignature(container),
};
...
function functionDeclaratorSignature(node) {
    ...
    if (parameters === null || parameters === undefined)
        return undefined;
    const parameterNodes = parameters.namedChildren.filter((child) => child !== null && child.type === 'parameter_declaration');
    ...
    const hasEllipsis = parameters.children.some((child) => child.type === 'variadic_parameter' ||
        child.type === '...' ||
    ...
    if (isVoidOnly)
        return { parameterCount: 0, parameterTypes: [] };
    const parameterTypes = parameterNodes.map((parameter) => parameter.childForFieldName('type')?.text ?? 'unknown');
    if (hasEllipsis)
        parameterTypes.push('...');
    return {
...
function containsNodeType(root, type) {
    return findDescendantOfType(root, type) !== null;
}
function findDescendantOfType(root, type) {
    const stack = [root];
