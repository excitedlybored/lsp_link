 ...
 * the tree.
 */
const moduleScopeNamesByRoot = new WeakMap();
 ...
 */
const exportAliasesByRoot = new WeakMap();
...
function declaresName(subtree, name) {
    const stack = [subtree];
    while (stack.length > 0) {
        ...
        if (n !== subtree && BINDING_SCOPE_NODE_TYPES.has(n.type))
            continue;
 ...
 */
function isExportsObjectExpression(node) {
    if (node.type === 'identifier') {
        return node.text === 'exports' && !isShadowedByLocalBinding(node, 'exports');
    }
    if (node.type !== 'member_expression')
        return false;
    return (node.childForFieldName('object')?.text === 'module' &&
    ...
    for (const child of root.namedChildren) {
        ...
        for (const declarator of child.namedChildren) {
            ...
            if (name === null || name.type !== 'identifier' || value === null)
                continue;
            if (isExportsObjectExpression(value))
...
const isCommonJsByRoot = new WeakMap();
 ...
 */
function isCommonJsModule(root, filePath) {
 ...
 */
export function isModuleLevelThisAssignment(assignment) {
    const left = assignment.type === 'assignment_expression' ? assignment.childForFieldName('left') : null;
    if (left === null || left.type !== 'member_expression')
        return false;
    if (left.childForFieldName('object')?.type !== 'this')
        return false;
    const right = assignment.childForFieldName('right');
    if (right === null || !CALLABLE_VALUE_TYPES.has(right.type))
        return false;
    for (let anc = assignment.parent; anc !== null; anc = anc.parent) {
        if (THIS_BINDING_NODE_TYPES.has(anc.type))
            return false;
    }
    return true;
}
export function isModuleLevelThisExport(assignment, root, filePath) {
    const left = assignment.type === 'assignment_expression' ? assignment.childForFieldName('left') : null;
    if (left === null || left.type !== 'member_expression')
        return false;
    if (left.childForFieldName('object')?.type !== 'this')
...
export function moduleLevelThisExportName(assignment, root, filePath) {
    if (!isModuleLevelThisExport(assignment, root, filePath))
        return null;
...
function collectDeclaredNames(node, out) {
    ...
    if (VARIABLE_DECLARATION_TYPES.has(node.type)) {
        for (const declarator of node.namedChildren) {
            ...
            if (name === null || name.type !== 'identifier')
                continue;
            const value = declarator.childForFieldName('value');
            if (value !== null && CALLABLE_VALUE_TYPES.has(value.type))
                out.add(name.text);
        }
        return;
    }
    ...
    if (node.type === 'export_statement') {
        ...
        if (declaration !== null)
            collectDeclaredNames(declaration, out);
    }
}
/** The set of names declared at module scope (top level) of `root`. */
function moduleScopeDeclaredNames(root) {
 ...
 */
function cjsExportedPropertyName(node, root) {
    const assignment = node.parent;
    if (assignment === null || assignment.type !== 'assignment_expression')
        return null;
    if (assignment.childForFieldName('right')?.id !== node.id)
        return null;
    ...
    if (receiver === null)
        return null;
    if (isExportsObjectExpression(receiver))
 ...
 */
export function cjsExportedNameFor(node, root) {
 ...
 */
export function isUndeclarableThisMemberValue(node, root) {
    const assignment = node.parent;
    if (assignment === null || assignment.type !== 'assignment_expression')
        return false;
    if (assignment.childForFieldName('right')?.id !== node.id)
 ...
 */
export function isUnexportedMemberAssignmentValue(node, root) {
    const assignment = node.parent;
    if (assignment === null || assignment.type !== 'assignment_expression')
        return false;
    if (assignment.childForFieldName('right')?.id !== node.id)
    ...
    if (left === null || left.type !== 'member_expression')
        return false;
    if (left.childForFieldName('object')?.type !== 'identifier')
        return false;
 ...
 */
export function isShadowedCjsExportAssignment(node, root) {
 ...
 */
export function cjsExportedName(value, root) {
    ...
    if (assignment === null || assignment.type !== 'assignment_expression')
        return null;
    if (assignment.childForFieldName('right')?.id !== value.id)
        return null;
...
function isCjsDefaultExportAssignmentNode(assignment) {
    const right = assignment.childForFieldName('right');
    if (right === null || !CALLABLE_VALUE_TYPES.has(right.type))
        return false;
    const left = assignment.childForFieldName('left');
 ...
 */
export function defaultExportNameCollides(assignment, root, derivedName) {
    if (!isCjsDefaultExportAssignmentNode(assignment))
        return false;
 ...
 */
export function isShadowedCjsExportAssignmentNode(node, root) {
