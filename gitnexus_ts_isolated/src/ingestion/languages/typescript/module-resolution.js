 ...
 */
import { matchSubpathMap, nodePackageNameOf, owningPackage, resolveNodeWorkspaceImport, substituteStar, } from '../../import-resolvers/node-workspace-packages.js';
import { resolveFile } from './file-candidates.js';
import { tsconfigFor } from './tsconfig.js';
/**
 * Resolve one specifier to a repo file, or `null` when nothing in the repo
 * declares it.
 */
export function resolveTsModule(specifier, ctx) {
    if (specifier === '')
        return null;
    // 1. A path specifier.
    ...
    const viaWorkspace = resolveNodeWorkspaceImport(specifier, ctx.workspacePackages, ctx.allFilePaths);
    if (viaWorkspace !== null)
        return viaWorkspace;
 ...
 */
function resolveViaPaths(specifier, paths, allFiles) {
    const matches = [];
    ...
    matches.sort((a, b) => Number(a.stem !== null) - Number(b.stem !== null) || b.prefixLength - a.prefixLength);
...
function resolveSubpathImport(specifier, ctx) {
