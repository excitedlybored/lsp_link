 ...
 */
export function tsconfigFor(index, filePath) {
    if (index === null)
        return null;
...
export async function loadTsconfigIndex(repoRoot) {
 ...
 */
async function readCompilerOptions(absPath, depth, repoRootHint) {
        ...
        if (opts.paths !== null && typeof opts.paths === 'object' && !Array.isArray(opts.paths)) {
            ...
            for (const [pattern, targets] of Object.entries(opts.paths)) {
                ...
                if (asStrings.length > 0)
                    own.paths.push({ pattern, targets: asStrings });
            }
        }
    }
...
async function readExtended(value, fromDir, depth, repoRootHint) {
 ...
 * to invented ones.
 */
async function resolveExtendsTarget(spec, fromDir) {
...
function* nodeModulesChain(fromDir) {
    let dir = fromDir;
    for (;;) {
...
async function firstReadableConfig(base) {
...
async function findTsconfigFiles(repoRoot) {
    const found = [];
...
function parseJsonc(raw) {
    const withoutComments = raw
        .replace(/\\"|"(?:\\"|[^"])*"|(\/\/.*$)|(\/\*[\s\S]*?\*\/)/gm, (match, line, block) => line !== undefined || block !== undefined ? '' : match)
        .replace(/,(\s*[}\]])/g, '$1');
