 ...
 */
export function synthesizeCjsReExports(root, out) {
    ...
    for (const child of root.namedChildren) {
        ...
        for (const declarator of child.namedChildren) {
            ...
            if (arg === undefined || arg === null || arg.type !== 'string')
                continue;
    ...
    if (namespaceSources.size === 0 && namedSources.size === 0)
        return;
    for (const child of root.namedChildren) {
        ...
        if (receiver === null || exposed === null || exposed.type !== 'property_identifier')
            continue;
        const isExportsReceiver = (receiver.type === 'identifier' && receiver.text === 'exports') ||
        ...
        if (right.type === 'member_expression') {
            ...
            if (ns === null || member === null || ns.type !== 'identifier')
                continue;
 ...
 */
export function synthesizeCjsDefaultExport(root, filePath, out) {
    for (const child of root.namedChildren) {
        if (child.type !== 'expression_statement')
            continue;
        const assignment = child.namedChild(0);
        ...
        if (left === null || left.type !== 'member_expression')
            continue;
        if (left.childForFieldName('object')?.text !== 'module')
            continue;
        if (left.childForFieldName('property')?.text !== 'exports')
            continue;
        ...
        if (ownName === null && defaultExportNameCollides(assignment, root, name))
            continue;
...
export function synthesizeCjsModuleExports(root, filePath, out) {
