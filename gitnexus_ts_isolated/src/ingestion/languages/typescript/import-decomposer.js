 ...
 */
function mayHaveTypeKeyword(stmtNode) {
        ...
        if (child.type === 'named_imports') {
            for (let j = 0; j < child.namedChildCount; j++) {
                ...
                if (spec === null || spec.type !== 'import_specifier')
                    continue;
                const decomposed = decomposeNamedSpecifier(spec, source, stmtNode, statementTypeOnly, mayHaveType);
                if (decomposed !== null)
                    out.push(decomposed);
            }
            continue;
 ...
 */
function decomposeNamedSpecifier(spec, source, stmtNode, statementTypeOnly, mayHaveType) {
    ...
    if (exportClause !== null) {
        ...
        for (let i = 0; i < exportClause.namedChildCount; i++) {
            ...
            if (spec === null || spec.type !== 'export_specifier')
                continue;
            const decomposed = decomposeReexportSpecifier(spec, source, stmtNode, statementTypeOnly, mayHaveType);
            if (decomposed !== null)
                out.push(decomposed);
        }
        return out;
...
function decomposeReexportSpecifier(spec, source, stmtNode, statementTypeOnly, mayHaveType) {
    const nameNode = spec.childForFieldName('name');
    const aliasNode = spec.childForFieldName('alias');
    if (nameNode === null)
        return null;
