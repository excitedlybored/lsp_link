 ...
 */
export function resolveFile(stem, allFiles) {
    if (stem === '')
        return null;
    if (allFiles.has(stem))
        return stem;
    ...
    if (tsEquivalents !== undefined) {
        ...
        for (const candidate of tsEquivalents) {
            if (allFiles.has(stripped + candidate))
                return stripped + candidate;
        }
    }
    for (const candidate of TS_EXTENSIONS) {
        if (allFiles.has(stem + candidate))
            return stem + candidate;
    }
    for (const candidate of TS_EXTENSIONS) {
        if (allFiles.has(`${stem}/index${candidate}`))
            return `${stem}/index${candidate}`;
    }
    return null;
}
