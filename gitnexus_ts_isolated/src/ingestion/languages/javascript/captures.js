...
const JS_CLASS_FIELD_DEFINITION_TYPES = new Set(['field_definition']);
...
import { isShadowedCjsExportAssignment, isUnexportedMemberAssignmentValue, isUndeclarableThisMemberValue, } from '../typescript/cjs-export-assignment.js';
...
const JS_CALLABLE_CAPTURE_OPTIONS = {
    functionNodeTypes: new Set(FUNCTION_NODE_TYPES),
...
function pickFirstNode(groupedNodes, tags) {
    for (const tag of tags) {
...
const JSDOC_TYPE_REFERENCE_RE = /^[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*(?:\s*<[\w$.,<>\s]*>)?(?:\s*\|\s*[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*(?:\s*<[\w$.,<>\s]*>)?)*$/;
/**
 ...
 */
function jsDocFieldTypeSpelling(rawType) {
    const spelling = rawType
        .trim()
        .replace(/^[?!]+/, '')
        .trim();
    if (spelling === '' || spelling.length > JSDOC_TYPE_MAX_LENGTH)
        return null;
    if (reducesToContainedType(spelling))
        return null;
    if (!JSDOC_TYPE_REFERENCE_RE.test(spelling))
        return null;
    return spelling;
}
/**
 ...
 */
function jsDocBindableFieldName(node) {
    if (isStaticClassFieldBinding(node, JS_CLASS_FIELD_DEFINITION_TYPES))
        return null;
 ...
 */
function emitJsDocFieldBinding(docComment, nameNode, out) {
...
function synthesizeJsDocBindings(root, out) {
    ...
    for (;;) {
        ...
        if (!isFnDecl && !isMethodDef && !isLexDecl && !isFieldDef)
            continue;
        ...
        if (isFieldDef && fieldNameNode === null)
            continue;
 ...
 */
function synthesizeJsInheritanceReferences(root, out) {
    const stack = [root];
    for (;;) {
        const node = stack.pop();
        if (node === undefined)
...
function terminalJsHeritageNameNode(node) {
    switch (node.type) {
        case 'identifier':
...
export function emitJsScopeCaptures(sourceText, filePath, cachedTree) {
    ...
    for (const m of rawMatches) {
        ...
        if (isStaticClassFieldBinding(groupedNodes['@type-binding.constructor'], JS_CLASS_FIELD_DEFINITION_TYPES)) {
            continue;
        }
