 ...
 */
export function expandGoDotImports(parsedFiles, indexes) {
        ...
        for (const targetFile of wildcardTargets) {
            ...
            for (const [name, refs] of targetBindings) {
                ...
                for (const ref of refs) {
                    if (ref.origin !== 'local')
                        continue;
                    if (entries.some((e) => e.def.nodeId === ref.def.nodeId))
                        continue;
...
export function expandGoWildcardNames(targetModuleScope, parsedFiles) {
