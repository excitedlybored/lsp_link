 ...
 */
export function populateGoOwners(parsed) {
...
export function populateGoWorkspaceOwners(parsedFiles, ctx) {
    ...
    for (const parsed of parsedFiles) {
        ...
        if (pkgName === null) {
            ...
            skippedCount += 1;
            if (skippedSample.length < SKIPPED_SAMPLE_CAP)
                skippedSample.push(parsed.filePath);
            continue;
        }
    ...
    if (skippedCount > 0) {
        logger.warn({ skippedFiles: skippedCount, sample: skippedSample }, 'go: files with no resolvable package clause were excluded from method-owner ' +
        ...
        for (const parsed of parsedFiles) {
            for (const scope of parsed.scopes) {
                if (scope.kind !== 'Function')
                    continue;
                const methodDefs = scope.ownedDefs.filter((d) => d.type === 'Method' && d.ownerId === undefined);
                if (methodDefs.length === 0)
                    continue;
                // Find the self typeBinding in this Function scope.
                let receiverType;
                let receiverKind = 'value';
                ...
                if (owner !== undefined) {
                    for (const def of methodDefs) {
                        ...
                        def.goReceiverKind = receiverKind;
