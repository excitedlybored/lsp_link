...
import { matchingGoDelimiter, readGoTypeParameters, splitTopLevelGoList, } from './generic-type-parameters.js';
...
function detectGoInterfaceImplementationsFromIndexes(indexes) {
    ...
    for (const iface of indexes.interfaces) {
        ...
        const undecidedStructIds = new Set();
        for (const candidateSet of [required, ...instantiatedMethodSetsFor(iface, required, indexes)]) {
            for (const structId of candidateStructIds) {
                ...
                const verdict = methodSetSatisfies(pointerSet, candidateSet, indexes.signatureContextByDefId);
                if (verdict !== 'satisfied') {
                    ...
                    if (verdict === 'undecided')
                        undecidedStructIds.add(structId);
                    continue;
                }
...
function methodSetSatisfies(actual, required, signatureContextByDefId) {
    ...
    for (const [name, requiredOverloads] of required) {
        ...
        for (const requiredMethod of requiredOverloads) {
            ...
            const verdict = compatibleMethodVerdict(actualOverloads, requiredMethod, signatureContextByDefId);
...
function compatibleMethodVerdict(actualOverloads, requiredMethod, signatureContextByDefId) {
...
function signaturesCompatible(actual, required, signatureContextByDefId) {
    ...
    if (!countsCompatible(actual.parameterCount, required.parameterCount) ||
    ...
    if (parameters === 'unsatisfied')
        return 'unsatisfied';
    const returns = returnTypeVerdict(actual.returnType, required.returnType, actualContext, requiredContext);
    if (returns === 'unsatisfied')
        return 'unsatisfied';
...
function parameterTypesVerdict(actualDef, requiredDef, actualContext, requiredContext) {
    ...
    if (actual === undefined || required === undefined) {
        ...
        if (actualDef.parameterCount === 0 && requiredDef.parameterCount === 0)
            return 'satisfied';
...
function returnTypeVerdict(actual, required, actualContext, requiredContext) {
    if (required === undefined)
        return actual === undefined ? 'satisfied' : 'unsatisfied';
    if (actual === undefined)
        return 'unsatisfied';
...
function typeVerdict(actual, required, actualContext, requiredContext) {
    ...
    if (actualType === undefined || requiredType === undefined)
        return 'undecided';
...
const normalizedTypesByContext = new WeakMap();
function normalizeSignatureType(typeName, context) {
    ...
    if (normalized.has(compact))
        return normalized.get(compact);
...
function externalPackageQualifier(importPath) {
...
function goImportToken(localName, importPath) {
