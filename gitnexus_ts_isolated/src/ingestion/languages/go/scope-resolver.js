...
import { goArityCompatibility, goMergeBindings, populateGoPackageSiblings, resolveGoImportTarget, mirrorGoNamespaceTypeBindings, } from './index.js';
...
export const goScopeResolver = {
    language: SupportedLanguages.Go,
    languageProvider: goProvider,
    importEdgeReason: 'go-scope: import',
    loadResolutionConfig: (repoPath) => loadGoModulePath(repoPath),
    resolveImportTarget: (targetRaw, fromFile, allFilePaths, resolutionConfig) => resolveGoImportTarget(targetRaw, fromFile, allFilePaths, resolutionConfig),
    expandsWildcardTo: (targetModuleScope, parsedFiles) => expandGoWildcardNames(targetModuleScope, parsedFiles),
    ...
    stripTypePreservingDecoration: (typeName) => typeName.startsWith('*') ? typeName.slice(1).trim() : undefined,
    ...
    populateOwners: (parsed) => populateGoOwners(parsed),
