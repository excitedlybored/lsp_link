...
export function synthesizeGoTypeBindings(rootNode) {
    const out = [];
    for (const node of rootNode.descendantsOfType('var_spec')) {
        const name = node.childForFieldName('name');
        const value = node.childForFieldName('value');
        if (name?.type !== 'identifier' || value === null)
            continue;
        ...
        out.push({
            ...
            '@type-binding.type': syntheticCapture('@type-binding.type', typeNode, extractSimpleTypeNameText(typeNode)),
        });
    }
    for (const node of rootNode.descendantsOfType('short_var_declaration')) {
        ...
        if (lhsIds.length >= 2 && rhsExprs.length >= 2) {
            for (let i = 0; i < Math.min(lhsIds.length, rhsExprs.length); i++) {
                ...
                out.push({
                    '@type-binding.multi-assign': syntheticCapture('@type-binding.multi-assign', node, lhsId.text),
                    '@type-binding.name': syntheticCapture('@type-binding.name', lhsId, lhsId.text),
                    '@type-binding.type': syntheticCapture('@type-binding.type', typeNode ?? rhsExpr, typeName),
                });
            }
                    ...
                    if (typeArg !== undefined) {
                        ...
                        if (nameNodes.length > 0) {
                            out.push({
                                ...
                                '@type-binding.name': syntheticCapture('@type-binding.name', nameNodes[0], nameNodes[0].text),
                ...
                if (fn?.type === 'identifier' && fn.text === 'make' && args !== null) {
                    ...
                    if (sliceOrMap !== undefined) {
                        ...
                        if (sliceOrMap.type === 'slice_type') {
                            ...
                            const typeChildren = sliceOrMap.namedChildren.filter((c) => ['type_identifier', 'qualified_type'].includes(c.type));
                        ...
                        if (typeName !== '') {
                            ...
                            if (nameNodes.length > 0) {
                                out.push({
                                    ...
                                    '@type-binding.name': syntheticCapture('@type-binding.name', nameNodes[0], nameNodes[0].text),
                                    '@type-binding.type': syntheticCapture('@type-binding.type', sliceOrMap, typeName),
                                });
                            }
                        }
                    }
...
function synthesizeElementAccessBindings(rootNode, out) {
        ...
        for (const expr of right.namedChildren) {
            ...
            if (expr.type === 'call_expression') {
                ...
                if (fn?.type === 'identifier' && (fn.text === 'make' || fn.text === 'new')) {
                    const args = expr.childForFieldName('arguments');
                    const typeNode = args?.namedChildren.find((c) => ['type_identifier', 'qualified_type', 'slice_type', 'map_type'].includes(c.type));
                    if (typeNode) {
                        if (typeNode.type === 'slice_type') {
                            const elem = typeNode.namedChildren.find((c) => ['type_identifier', 'qualified_type'].includes(c.type));
                            if (elem)
                                typeName = extractSimpleTypeNameText(elem);
                        }
                        else if (typeNode.type === 'map_type') {
                            const tc = typeNode.namedChildren.filter((c) => ['type_identifier', 'qualified_type'].includes(c.type));
                            typeName = tc[1]
    ...
    for (const rangeClause of rootNode.descendantsOfType('range_clause')) {
        ...
        if (right === null || right.namedChildren.length === 0)
            continue;
...
export function extractSimpleTypeNameText(node) {
...
function extractCompositeLiteralTypeNode(expr) {
...
function extractTypeNode(expr) {
    ...
    if (expr.type === 'call_expression') {
        ...
        if (fn?.type === 'identifier' && fn.text === 'new') {
            const args = expr.childForFieldName('arguments');
            return (args?.namedChildren.find((c) => ['type_identifier', 'qualified_type', 'pointer_type'].includes(c.type)) ?? null);
        }
        if (fn?.type === 'identifier' && fn.text === 'make') {
            const args = expr.childForFieldName('arguments');
            const container = args?.namedChildren.find((c) => ['slice_type', 'map_type'].includes(c.type));
            if (container?.type === 'slice_type') {
                return (container.namedChildren.find((c) => ['type_identifier', 'qualified_type'].includes(c.type)) ?? null);
            }
            if (container?.type === 'map_type') {
                const typeChildren = container.namedChildren.filter((c) => ['type_identifier', 'qualified_type'].includes(c.type));
