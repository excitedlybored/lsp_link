import { buildPackageDirIndex, filesDirectlyInPkgDir, sortedRootFiles, } from '../../import-resolvers/package-dir-index.js';
 ...
 */
export function resolveGoImportTarget(targetRaw, _fromFile, allFilePaths, resolutionConfig) {
    if (!targetRaw)
        return null;
    const goModule = resolutionConfig;
    ...
    if (goModule != null) {
        const ownedByModule = targetRaw === goModule.modulePath || targetRaw.startsWith(`${goModule.modulePath}/`);
        if (!ownedByModule)
            return null;
        const relativePkg = targetRaw === goModule.modulePath ? '' : targetRaw.slice(goModule.modulePath.length + 1); // e.g. "internal/models"
        const files = relativePkg === ''
...
function isGoPackageFile(normalized) {
 ...
 */
const getGoPackageIndex = perFileSet((allFilePaths) => buildPackageDirIndex(allFilePaths, isGoPackageFile));
function findRootPackageFiles(allFilePaths) {
    return sortedRootFiles(getGoPackageIndex(allFilePaths));
}
function findAllFilesInPkgDir(allFilePaths, pkgPath) {
