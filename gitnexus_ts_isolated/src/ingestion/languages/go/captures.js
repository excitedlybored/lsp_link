import { nodeToCapture, syntheticCapture, walkNamedTree, } from '../../utils/ast-helpers.js';
 ...
 */
const CALLEE_POSITION_MARKER = Object.freeze({
 ...
 */
const EMBEDDED_POINTER_MARKER = Object.freeze({
...
const GO_CALLABLE_CAPTURE_OPTIONS = {
    ...
    extractAssignment: (node) => {
        ...
        if (left.type !== 'expression_list' || right.type !== 'expression_list')
            return undefined;
        const destinations = left.namedChildren.filter((child) => child !== null);
        const sources = right.namedChildren.filter((child) => child !== null);
 ...
 */
function isCalleeOfMemberCall(node) {
...
export function emitGoScopeCaptures(sourceText, _filePath, cachedTree) {
        ...
        if (declAnchorNode !== undefined) {
            ...
            if (fnNode !== null) {
                ...
                if (arity.returnType !== undefined) {
                    grouped['@declaration.return-type'] = syntheticCapture('@declaration.return-type', fnNode, arity.returnType);
                }
            }
    ...
    for (const match of out) {
        if (match['@declaration.field'] === undefined)
            continue;
 ...
 */
function synthesizeGoInheritanceReferences(root) {
    const out = [];
 ...
 */
function emitGoEmbedInheritance(baseNode, out, embeddedAsPointer) {
    if (baseNode === null)
        return;
 ...
 * with the legacy leg.
 */
function goEmbedBaseNameNode(node) {
...
function normalizeGenericConstructorCapture(nodeMap, grouped) {
    const typeNode = grouped['@type-binding.constructor'] !== undefined ? nodeMap['@type-binding.type'] : undefined;
    if (typeNode !== undefined && typeNode.type === 'generic_type') {
        const base = typeNode.childForFieldName('type');
        if (base !== null) {
            grouped['@type-binding.type'] = syntheticCapture('@type-binding.type', base, extractSimpleTypeNameText(base));
        }
    }
    const referenceNode = grouped['@reference.call.constructor'] !== undefined ? nodeMap['@reference.name'] : undefined;
    if (referenceNode !== undefined && referenceNode.type === 'generic_type') {
        const base = referenceNode.childForFieldName('type');
        if (base !== null) {
            grouped['@reference.name'] = syntheticCapture('@reference.name', base, extractSimpleTypeNameText(base));
        }
    }
}
function isRawMultiAssignTypeBinding(nodeMap) {
    ...
    if (rhs === null)
        return false;
    return (lhs.namedChildren.filter((c) => c.type === 'identifier').length >= 2 &&
