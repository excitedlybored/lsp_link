...
const PHP_CALLABLE_CAPTURE_OPTIONS = {
    ...
    extractCallableReference: (node) => {
        if (node.type !== 'function_call_expression')
            return undefined;
        const args = node.childForFieldName('arguments');
        const isFirstClass = args?.namedChildren.some((child) => child !== null && child.type === 'variadic_placeholder') === true;
        const target = node.childForFieldName('function');
        if (!isFirstClass || target === null || target.type === 'variable_name')
            return undefined;
...
export function emitPhpScopeCaptures(sourceText, _filePath, cachedTree) {
    ...
    for (const id of walked.docPropertyAnchorIds)
        typedPropertyAnchorIds.add(id);
    for (const m of rawMatches) {
        ...
        if (callTag !== undefined) {
            ...
            if (argumentsNode?.namedChildren.some((child) => child !== null && child.type === 'variadic_placeholder') === true) {
        ...
        if (callTag !== undefined && grouped['@reference.arity'] === undefined) {
            const callNode = nodeIfType(nodeMap[callTag], 'function_call_expression', 'member_call_expression', 'nullsafe_member_call_expression', 'scoped_call_expression', 'object_creation_expression');
            if (callNode !== null) {
 ...
 */
function synthesizePhpTreeWalkCaptures(root) {
    const inheritance = [];
    const docProperties = [];
    const docPropertyAnchorIds = new Set();
...
function emitPhpHeritageReferences(node, out) {
    if (node.type === 'class_declaration') {
        ...
        if (ifaceClause !== null)
            emitPhpBaseNames(ifaceClause, out);
    }
 ...
 */
function emitPhpTraitUses(node, out) {
    const body = node.childForFieldName('body');
    if (body === null || body.type !== 'declaration_list')
        return;
 ...
 */
function emitPhpBaseNames(clause, out) {
    for (let i = 0; i < clause.namedChildCount; i++) {
        const base = clause.namedChild(i);
        if (base === null)
            continue;
        if (base.type !== 'name' && base.type !== 'qualified_name')
            continue;
 ...
 * element type.
 */
function precedingCommentSiblings(node) {
    const comments = [];
 ...
 */
function nearestPrecedingCommentMatch(node, re) {
...
function extractPropertyElementType(propDecl) {
    ...
    if (varTag !== null)
        return normalizePhpDocType(varTag[1]);
 ...
 */
const PHPDOC_PHANTOM_CONTAINER_BASES = new Set(['list']);
/**
 ...
 */
function erasePhpDocTypeArguments(text) {
    let out = '';
    let depth = 0;
 ...
 */
function phpDocPropertyFieldType(rawType) {
    ...
    if (erased.endsWith('[]'))
        return null;
    if (PHPDOC_PHANTOM_CONTAINER_BASES.has(erased.toLowerCase()))
        return null;
    return erased;
}
/**
...
function emitPhpDocPropertyBinding(node, matches, anchorIds) {
    ...
    matches.push({
        ...
        '@declaration.name': syntheticCapture('@declaration.name', varNameNode, varNameNode.text.replace(/^\$/, '')),
    });
}
/**
 ...
 */
function findPhpDocVarTag(propDecl) {
