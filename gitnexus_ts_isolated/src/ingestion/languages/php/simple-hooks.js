 ...
 */
export function phpBindingScopeFor(decl, innermost, tree) {
    ...
    if (decl['@declaration.property'] !== undefined && innermost.kind === 'Function') {
        ...
        if (cur !== undefined && cur.kind === 'Class')
            return cur.id;
    }
 ...
 */
export function phpImportOwningScope(_imp, innermost, _tree) {
 ...
 *   - non-Function scopes
 */
export function phpReceiverBinding(functionScope) {
    if (functionScope.kind !== 'Function')
        return null;
    return (functionScope.typeBindings.get('$this') ?? functionScope.typeBindings.get('parent') ?? null);
}
