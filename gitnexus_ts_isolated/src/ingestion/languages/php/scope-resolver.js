...
import { findReceiverTypeBinding, populateClassOwnedMembers, } from '../../scope-resolution/scope/walkers.js';
...
function phpIsCallableVisibleFromCaller(ctx) {
    ...
    const candQualified = candidate.qualifiedName === undefined
        ? ''
    ...
    if (candQualified === '')
        return false;
    return callerParsed.parsedImports.some((imp) => imp.kind === 'named' &&
 ...
 */
function buildPhpExtendsOnlyMro(graph, parsedFiles, nodeLookup) {
    return buildMro(graph, parsedFiles, nodeLookup, defaultLinearize);
}
function buildPhpMro(graph, parsedFiles, nodeLookup) {
    ...
    for (const parsed of parsedFiles) {
        for (const def of parsed.localDefs) {
            if (def.type !== 'Class' && def.type !== 'Trait')
                continue;
    ...
    const traitDefIds = new Set();
    for (const parsed of parsedFiles) {
        for (const def of parsed.localDefs) {
            if (def.type === 'Trait')
                traitDefIds.add(def.nodeId);
        }
    }
    ...
    for (const rel of graph.iterRelationshipsByType('IMPLEMENTS')) {
        ...
        if (targetDefId === undefined)
            continue;
        if (!traitDefIds.has(targetDefId))
            continue;
        ...
        if (!list.includes(targetDefId))
            list.push(targetDefId);
    }
 ...
 */
function collectTransitiveTraits(seeds, directTraitUse) {
    const out = [];
    const seen = new Set();
    const queue = [...seeds];
 ...
 */
function phpEmitUnresolvedReceiverEdges(graph, scopes, parsedFiles, nodeLookup, handledSites, model) {
    let emitted = 0;
    const seen = new Set();
    for (const parsed of parsedFiles) {
        for (const site of parsed.referenceSites) {
            ...
            const candidates = allCandidates.filter((c) => {
                if (c.qualifiedName === undefined)
                    return false;
                if (seen2.has(c.nodeId))
                    return false;
                seen2.add(c.nodeId);
                return true;
            });
            if (candidates.length !== 1)
                continue; // ambiguous or missing — skip
...
const phpScopeResolver = {
    language: SupportedLanguages.PHP,
    languageProvider: phpProvider,
    importEdgeReason: 'php-scope: use',
    resolveImportTarget: (targetRaw, fromFile, allFilePaths, resolutionConfig, context) => resolvePhpImportTargetInternal(targetRaw, fromFile, allFilePaths, resolutionConfig, context),
    ...
    buildExtendsOnlyMro: (graph, parsedFiles, nodeLookup) => buildPhpExtendsOnlyMro(graph, parsedFiles, nodeLookup),
