 ...
 */
import { nodeToCapture, syntheticCapture, walkNamedTree, findChild, } from '../../utils/ast-helpers.js';
 ...
 */
export function dartScopeOwnsReceivers(match) {
    ...
    if (!Array.isArray(parsed))
        return undefined;
    const names = parsed.filter((name) => typeof name === 'string' && name.length > 0);
...
const DART_CALLABLE_CAPTURE_OPTIONS = {
    ...
    callSiteNode: (node) => dartCallableCallee(node) ?? undefined,
    callableProtocolMethods: new Set(['call']),
};
function dartLexicalFunctionOwner(input) {
    let node = input;
    while (node !== null) {
        if (node.type === 'function_signature' || node.type === 'function_expression')
            return node;
...
export function emitDartScopeCaptures(sourceText, _filePath, cachedTree) {
    ...
    const memo = { fieldsByClassBody: new Map(), shadowsByBody: new Map() };
    ...
    for (const match of getDartScopeQuery().matches(root)) {
        ...
        if (declTag !== undefined) {
            ...
            if (seenFnDeclNodes.has(declKey))
                continue; // dedup named-ctor double-match
            ...
            if (bodyNode !== null) {
                ...
                for (const cm of synthesizeDartReceiverBinding(declNode, bodyNode))
                    out.push(cm);
            }
            for (const cm of synthesizeDartSignatureBindings(declNode, bodyNode))
                out.push(cm);
            continue;
        }
        ...
        if (grouped['@declaration.property'] !== undefined &&
            ...
            if (fieldType !== null) {
                grouped['@declaration.field-type'] = syntheticCapture('@declaration.field-type', propNode, fieldType);
            }
            // Structural receiver chain for a call whose receiver is itself an
            // expression, so resolution can type it by folding over structure
            // instead of re-parsing the receiver's source text. Self-gating: a
...
function dartCallableCallee(selector) {
    if (selector.namedChild(0)?.type !== 'argument_part')
        return null;
 ...
 */
function findFunctionBody(declNode) {
    ...
    const node = declNode.parent !== null && declNode.parent.type === 'method_signature'
        ? declNode.parent
        : declNode;
...
function spanCapture(name, startNode, endNode) {
    return {
        name,
        range: {
...
function attachArityMetadata(grouped, declNode) {
    const meta = computeDartArityMetadata(declNode);
    if (meta.parameterCount !== undefined) {
        grouped['@declaration.parameter-count'] = syntheticCapture('@declaration.parameter-count', declNode, String(meta.parameterCount));
    }
    if (meta.requiredParameterCount !== undefined) {
        grouped['@declaration.required-parameter-count'] = syntheticCapture('@declaration.required-parameter-count', declNode, String(meta.requiredParameterCount));
    }
    if (meta.parameterTypes !== undefined) {
        grouped['@declaration.parameter-types'] = syntheticCapture('@declaration.parameter-types', declNode, JSON.stringify(meta.parameterTypes));
    }
}
/** The declared type of a class field (`Address address = …` → `Address`). */
function extractFieldType(declNode) {
...
function countArgs(argPart) {
    ...
    for (let i = 0; i < args.namedChildCount; i++) {
        ...
        if (c !== null && (c.type === 'argument' || c.type === 'named_argument'))
            n++;
    }
    return n;
}
...
function computeReceiverText(nameSelector) {
    const selectors = [];
    ...
    if (head === null)
        return null;
    if (head.type !== 'identifier' && head.type !== 'this' && head.type !== 'super')
        return null;
    selectors.reverse();
    let text = head.text;
...
function emitSelectorReference(selector, out, seenReadSpans) {
    ...
    if (inner.type === 'argument_part') {
        ...
        if (prev.type === 'identifier') {
            ...
            if (DART_BUILT_INS.has(name))
                return; // legacy suppresses built-in-named calls
        ...
        if (prev.type === 'selector') {
            ...
            if (ASSIGNABLE_SELECTORS.has(prevInner.type)) {
                ...
                if (nameId === null)
                    return;
                if (DART_BUILT_INS.has(nameId.text))
                    return; // legacy suppresses built-in-named calls
    ...
    if (ASSIGNABLE_SELECTORS.has(inner.type)) {
        ...
        const isCall = next !== null && next.type === 'selector' && next.namedChild(0)?.type === 'argument_part';
        if (isCall)
            return;
 ...
 */
function emitCascadeReference(cascade, out) {
    ...
    for (let i = 0; i < cascade.namedChildCount; i++) {
        ...
        if (c.type === 'cascade_selector')
            selectorNode = c;
        else if (c.type === 'argument_part')
            argPart = c;
    }
    if (selectorNode === null || argPart === null)
        return;
    const nameId = selectorName(selectorNode);
    if (nameId === null || DART_BUILT_INS.has(nameId.text))
        return;
 ...
 * than guessed at.
 */
function isDirectConstruction(node) {
...
function findDirectCallValue(initVarDef) {
 ...
 */
function dartFieldConstructorCallee(nameNode) {
    const value = nameNode.nextNamedSibling;
    return isDirectConstruction(value) ? value : null;
}
function emitVarTypeBinding(initVarDef, out) {
 ...
 */
function emitDartFieldAssignmentBindings(classNode, out, memo) {
    ...
    for (const member of members) {
        if (member === null || member.type !== 'function_body')
            continue;
        ...
        if (signature === null || signature.type !== 'method_signature')
            continue;
        if (hasKeyword(signature, 'static'))
            continue;
        ...
        const shadowsOf = () => dartBodyShadows(member, memo);
        walkNamedTree(member, (node) => {
            if (node.type !== 'assignment_expression')
                return;
            const target = node.namedChild(0);
            if (target === null || target.type !== 'assignable_expression')
                return;
            ...
            if (first.type === 'identifier' && target.namedChildCount === 1) {
                ...
                if (!fields.has(first.text) || shadowsOf().has(first.text))
                    return;
                fieldNameNode = first;
            }
            else if (first.type === 'this') {
                const selector = target.namedChild(1);
                if (selector === null || selector.type !== 'unconditional_assignable_selector')
                    return;
            ...
            if (!isDirectConstruction(callee))
                return;
            out.push({
                '@type-binding.constructor': nodeToCapture('@type-binding.constructor', node),
                '@type-binding.dart-field': syntheticCapture('@type-binding.dart-field', fieldNameNode, '1'),
                '@type-binding.name': syntheticCapture('@type-binding.name', fieldNameNode, fieldNameNode.text),
                '@type-binding.type': syntheticCapture('@type-binding.type', callee, callee.text),
            });
        });
    }
}
const nodeSpanKey = (node) => `${node.startIndex}:${node.endIndex}`;
/**
 ...
 */
function dartClassFieldNames(classBody, memo) {
    ...
    for (const member of classBody.namedChildren) {
        if (member === null || member.type !== 'declaration')
            continue;
        ...
        for (const init of list.namedChildren) {
            ...
            if (nameNode !== null && nameNode.type === 'identifier')
                fields.add(nameNode.text);
        }
    }
    memo.fieldsByClassBody.set(key, fields);
    return fields;
}
function dartBodyShadows(bodyNode, memo) {
 ...
 * order-stable.
 */
function dartShadowedFieldsCapture(bodyNode, memo) {
    ...
    if (classBody === null || classBody.type !== 'class_body')
        return undefined;
 ...
 */
function collectDartBodyShadows(bodyNode) {
 ...
 */
function addDartBinderName(node, out) {
    switch (node.type) {
        ...
        case 'initialized_identifier': {
            ...
            if (first !== null && first.type === 'identifier')
                out.add(first.text);
            return;
        }
        // ── Dart 3 patterns ──────────────────────────────────────────────────────
...
function dartParameterName(param) {
...
function emitHeritage(classNode, out) {
...
function emitExtensionImplementsHeritage(extensionNode, out) {
...
function emitHeritageMarkers(container, kind, className, out) {
    for (let i = 0; i < container.namedChildCount; i++) {
        ...
        const fields = args === undefined || args.includes(':')
...
function typeArgumentsAfter(container, index) {
