...
export function dartBindingScopeFor(decl, innermost, tree) {
    ...
    if (decl['@declaration.function'] !== undefined ||
        decl['@declaration.method'] !== undefined ||
        decl['@declaration.constructor'] !== undefined) {
        if (innermost.kind === 'Function' && innermost.parent !== null)
            return innermost.parent;
    }
    return null;
}
export function dartImportOwningScope(_imp, innermost, _tree) {
    return innermost.kind === 'Module' ? innermost.id : null;
}
export function dartReceiverBinding(functionScope) {
