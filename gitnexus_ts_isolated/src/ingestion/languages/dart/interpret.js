...
export function interpretDartImport(captures) {
...
const BARE_CONTAINER_TYPES = new Set([
    'Future',
    'FutureOr',
    'List',
    'Iterable',
...
export function normalizeDartType(text) {
    let s = text.trim();
    ...
    if (gen !== null && gen[1] !== undefined)
        s = gen[1].trim().replace(/\?+$/, '');
...
export function interpretDartTypeBinding(captures) {
