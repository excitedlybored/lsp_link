 ...
 */
function pickClassByName(name, preferredFile, defsByName) {
    ...
    if (sameFile !== undefined)
        return sameFile.graphId;
    if (cands.length === 1)
        return cands[0].graphId;
 ...
 */
function emitDartHeritageEdges(graph, parsedFiles, nodeLookup, recordTypeArguments) {
    ...
    for (const parsed of parsedFiles) {
        for (const imp of parsed.parsedImports) {
            ...
            if (childId === undefined || baseId === undefined || childId === baseId)
                continue;
            ...
            if (rawTypeArguments !== undefined) {
                ...
                if (typeArguments !== undefined)
                    recordTypeArguments?.(childId, baseId, typeArguments);
            }
 ...
 * dispatch purposes).
 */
function buildDartMro(graph, parsedFiles, nodeLookup) {
...
export const dartScopeResolver = {
    language: SupportedLanguages.Dart,
    languageProvider: dartProvider,
    importEdgeReason: 'dart-scope: import',
    resolveImportTarget: (targetRaw, fromFile, allFilePaths) => resolveDartImportTarget(targetRaw, fromFile, allFilePaths),
    ...
    expandsWildcardTo: (targetModuleScope, parsedFiles) => expandDartWildcardNames(targetModuleScope, parsedFiles),
    ...
    emitHeritageEdges: (graph, parsedFiles, nodeLookup, _scopes, recordTypeArguments) => emitDartHeritageEdges(graph, parsedFiles, nodeLookup, recordTypeArguments),
