...
function findEnclosingTypeDeclaration(node) {
    let cur = node.parent;
    while (cur !== null) {
        if (TYPE_DECL_TYPES.has(cur.type))
            return cur;
        cur = cur.parent;
    }
    return null;
...
function firstSuperType(typeNode) {
...
function buildReceiverMatch(anchor, name, typeText) {
    return {
 ...
 */
export function synthesizeDartReceiverBinding(declNode, bodyNode) {
    if (dartMethodConfig.isStatic(declNode))
        return [];
    const enclosingType = findEnclosingTypeDeclaration(declNode);
    if (enclosingType === null)
        return []; // top-level function — no receiver
    const typeName = enclosingTypeName(enclosingType);
    if (typeName === null)
        return [];
    const out = [buildReceiverMatch(bodyNode, 'this', typeName)];
