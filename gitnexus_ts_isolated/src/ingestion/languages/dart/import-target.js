...
const getDartFileIndex = perFileSet((allFilePaths) => {
...
function findByPathSuffix(allFilePaths, candidate) {
    const bucket = getDartFileIndex(allFilePaths).byBasename.get(candidate.slice(candidate.lastIndexOf('/') + 1));
    if (bucket === undefined)
        return null;
    const suffix = '/' + candidate;
    for (const fp of bucket) {
        if (fp === candidate || fp.endsWith(suffix))
            return fp;
    }
    return null;
}
...
function resolveRelative(rel, fromFile, allFilePaths) {
...
export function resolveDartImportTarget(targetRaw, fromFile, allFilePaths) {
    if (targetRaw.startsWith(DART_HERITAGE_PREFIX))
        return null;
    // `targetRaw` already arrives quote-stripped from `interpretDartImport`.
    if (targetRaw === '')
        return null;
    // Dart SDK imports never resolve to a repo file.
    if (targetRaw.startsWith('dart:'))
        return null;
