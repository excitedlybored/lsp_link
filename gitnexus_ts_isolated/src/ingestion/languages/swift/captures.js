...
import { findEnclosingTypeDeclaration, synthesizeSwiftReceiverBinding, } from './receiver-binding.js';
 ...
 */
function swiftEnclosingTypeName(node) {
...
const SWIFT_CALLABLE_CAPTURE_OPTIONS = {
    ...
    extractFunctionParameters: (node) => node.namedChildren.filter((child) => child !== null && child.type === 'parameter'),
};
    ...
    for (const m of rawMatches) {
        ...
        if (grouped['@declaration.qualified_name'] === undefined) {
            ...
            if (ownerTag !== undefined && declaredName !== undefined && declNode != null) {
                ...
                if (owner !== null) {
                    grouped['@declaration.qualified_name'] = syntheticCapture('@declaration.qualified_name', declNode, `${owner}.${declaredName}`);
                }
            }
        }
