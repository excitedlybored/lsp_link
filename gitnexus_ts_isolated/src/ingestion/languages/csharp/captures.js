...
const CSHARP_CALLABLE_CAPTURE_OPTIONS = {
    ...
    extractAssignment: (node) => {
        ...
        if (destination === null || source === null || source.id === destination.id)
            return undefined;
...
export function emitCsharpScopeCaptures(sourceText, _filePath, cachedTree) {
    ...
    for (const m of rawMatches) {
        ...
        if (grouped['@reference.call.constructor.qualified'] !== undefined &&
            ...
            if (nameNode !== null) {
                ...
                if (qText.length > 0 && qText !== nameNode.text) {
                    grouped['@reference.qualified-name'] = syntheticCapture('@reference.qualified-name', qNode, qText);
                }
            }
        }
        ...
        if (callTag !== undefined && grouped['@reference.arity'] === undefined) {
            const callNode = nodeIfType(nodeMap[callTag], 'invocation_expression', 'object_creation_expression');
            if (callNode !== null) {
        ...
        if (grouped['@declaration.class'] !== undefined ||
            ...
            const typeNode = nodeIfType(nodeMap['@declaration.class'] ?? nodeMap['@declaration.record'], 'class_declaration', 'record_declaration');
            if (typeNode !== null) {
 ...
 */
function synthesizeCsharpConstructorInitializerReferences(root) {
    const out = [];
 ...
 */
function synthesizeCsharpInheritanceReferences(root) {
    const out = [];
    walkNamedTree(root, (node) => {
