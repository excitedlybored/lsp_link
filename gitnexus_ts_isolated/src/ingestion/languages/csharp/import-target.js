...
import { buildPackageDirIndex, firstFileDirectlyInPkgDir, } from '../../import-resolvers/package-dir-index.js';
 ...
 */
const getCsharpDirIndex = perFileSet((allFilePaths) => buildPackageDirIndex(allFilePaths, (normalized) => normalized.endsWith('.cs')));
export function resolveCsharpImportTarget(parsedImport, workspaceIndex) {
    ...
    if (csharpConfigs.length > 0) {
        ...
        const fromCsproj = resolveCSharpImportInternal(targetRaw, [...csharpConfigs], ctx.allFilePaths, index, evidence);
 ...
 */
function resolveDirectMatch(ws, dirs, pathLike) {
 ...
 */
function resolveByProgressiveStripping(ws, dirs, pathLike) {
