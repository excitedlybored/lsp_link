...
export function populateCsharpNamespaceSiblings(parsedFiles, indexes, inputs) {
    ...
    const workspaceTypeBindings = indexes.workspaceTypeBindings;
    ...
    const namespaceTypeBindings = indexes.namespaceTypeBindings;
    for (const [nsName, bucket] of buckets) {
        if (nsName === '')
            continue; // global → workspaceTypeBindings (above)
        ...
        for (const scopeInfo of bucket.scopes) {
            ...
            for (const [boundName, typeRef] of scopeInfo.scope.typeBindings) {
                ...
                // First-wins, matching the old per-importer `has(boundName)` skip.
                if (!nsMap.has(boundName))
                    nsMap.set(boundName, typeRef);
            }
        }
    }
    ...
    const accessibleNamespacesByScope = indexes.accessibleNamespacesByScope;
    for (const parsed of parsedFiles) {
        const moduleScope = parsed.scopes.find((s) => s.kind === 'Module');
        if (moduleScope === undefined)
            continue;
    ...
    const namespaceFqn = indexes.namespaceFqnBindings;
    for (const [nsName, bucket] of buckets) {
