 ...
 */
import { SupportedLanguages, getLanguageFromFilename } from '../../../../_shared/index.js';
...
const VUE_SCOPE_CONTEXT_LANGUAGES = new Set([
    SupportedLanguages.Vue,
    SupportedLanguages.TypeScript,
    SupportedLanguages.JavaScript,
]);
function isVueScopeContextLanguage(lang) {
    return lang !== null && VUE_SCOPE_CONTEXT_LANGUAGES.has(lang);
}
const vueScopeResolver = {
     ...
     */
    collectScopeContextPaths({ primaryFilePaths, preExtractedByPath, entryFileContents, allScannedPaths, resolutionConfig, }) {
        const resolveTargets = (targetRaw, fromFile) => {
            const resolved = vueScopeResolver.resolveImportTarget(targetRaw, fromFile, allScannedPaths, resolutionConfig);
            if (resolved === null)
                return [];
        ...
        while (queue.length > 0) {
            ...
            if (parsed === undefined) {
                ...
                if (source !== undefined) {
                    ...
                    if (parsed !== undefined)
                        fallbackParsed.set(current, parsed);
                }
            }
            if (parsed === undefined)
                continue;
            for (const parsedImport of parsed.parsedImports) {
                if (parsedImport.targetRaw.trim().length === 0)
                    continue;
                for (const targetPath of resolveTargets(parsedImport.targetRaw, current)) {
                    if (!allScannedPaths.has(targetPath))
                        continue;
                    if (!isVueScopeContextLanguage(getLanguageFromFilename(targetPath)))
                        continue;
    ...
    emitPostResolutionEdges(graph, parsedFiles, nodeLookup, indexes, ctx) {
        for (const parsedFile of parsedFiles) {
            if (!parsedFile.filePath.endsWith('.vue'))
                continue;
            ...
            for (const [scopeId, edges] of indexes.imports) {
                ...
                if (scope?.filePath !== parsedFile.filePath)
                    continue;
            ...
            const { templateComponents, nativeEventHandlers, componentEventBindings, scriptEmitCalls, templateAttributeBindings, } = extractVueTemplateEdgeData(content, { sourceKind: 'full-sfc' });
