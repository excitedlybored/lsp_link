...
export function buildKotlinPackageIndex(parsedFiles, packageOf) {
    ...
    for (const parsed of parsedFiles) {
        ...
        if (packageFiles === undefined)
            filesByPackage.set(packageName, [parsed.filePath]);
        else
            packageFiles.push(parsed.filePath);
        const moduleScope = parsed.scopes.find((scope) => scope.id === parsed.moduleScope && scope.kind === 'Module') ??
        ...
        for (const [name, refs] of moduleScope.bindings) {
            ...
            if (files === undefined)
                declarations.set(name, [parsed.filePath]);
            else if (!files.includes(parsed.filePath))
...
export function resolveKotlinModule(targetRaw, index) {
    if (targetRaw === '')
        return null;
...
function resolveTopLevelDeclaration(qualifiedName, index) {
...
function declarationFiles(files) {
