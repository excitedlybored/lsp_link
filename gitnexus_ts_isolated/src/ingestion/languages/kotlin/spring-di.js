...
function annotationFact(annotation) {
...
export function kotlinSpringAnnotationFacts(node) {
    const facts = [];
    for (const child of node.namedChildren) {
        if (child.type !== 'modifiers' && child.type !== 'parameter_modifiers')
            continue;
        facts.push(...annotationsFromModifierContainer(child));
    }
    return facts;
}
function directTypeNode(node) {
    return node.namedChildren.find((child) => KOTLIN_TYPE_NODES.has(child.type));
}
function kotlinBeanFactoryReturnType(functionNode) {
    const parametersIndex = functionNode.namedChildren.findIndex((child) => child.type === 'function_value_parameters');
    ...
    if (body === undefined)
        return undefined;
    const call = body.type === 'call_expression' ? body : firstDescendantOfType(body, 'call_expression');
...
function functionDependencies(callable) {
    const parameters = callable.namedChildren.find((child) => child.type === 'function_value_parameters');
...
function primaryConstructorDependencies(constructor) {
    const dependencies = [];
...
function propertyDependency(property) {
 ...
 * post-resolution.
 */
export function captureKotlinSpringDiClassFact(classNode, filePath) {
    if (!isKotlinBeanCandidateClass(classNode))
        return null;
    ...
    const primaryConstructor = classNode.namedChildren.find((child) => child.type === 'primary_constructor');
    const secondaryConstructors = body?.namedChildren.filter((child) => child.type === 'secondary_constructor') ?? [];
    const constructorCount = (primaryConstructor === undefined ? 0 : 1) + secondaryConstructors.length;
    if (primaryConstructor !== undefined) {
        const annotations = kotlinSpringAnnotationFacts(primaryConstructor);
        const implicitConstructor = constructorCount === 1 &&
    ...
    if (body !== undefined) {
        for (const member of body.namedChildren) {
            if (member.type === 'property_declaration') {
                ...
                const annotations = kotlinSpringAnnotationFacts(member);
                const name = member.namedChildren.find((child) => child.type === 'simple_identifier')?.text.trim() ??
                    '<method>';
                const factoryAnnotations = annotations.filter((annotation) => annotation.useSiteTarget === undefined);
...
function isApplicableInjectionAnnotation(annotation, site) {
...
function isApplicableQualifierAnnotation(annotation, site) {
    if (annotation.useSiteTarget === undefined)
        return true;
    if (site.kind === 'property') {
        return (annotation.useSiteTarget === 'field' ||
            annotation.useSiteTarget === 'param' ||
            annotation.useSiteTarget === 'setparam');
    }
    return annotation.useSiteTarget === 'param';
}
function isApplicableFactoryQualifierAnnotation(annotation) {
    return annotation.useSiteTarget === undefined || annotation.useSiteTarget === 'param';
}
function parseKotlinSpringInjectionType(rawType) {
