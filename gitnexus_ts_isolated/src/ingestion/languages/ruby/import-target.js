 ...
 */
function resolveRelative(targetRaw, fromDir, allFilePaths) {
    ...
    if (allFilePaths.has(indexFile))
        return indexFile;
    // The path might already include .rb extension
    if (resolvedPath.endsWith('.rb') && allFilePaths.has(resolvedPath))
        return resolvedPath;
    return null;
}
/**
 ...
 */
function resolveBare(targetRaw, allFilePaths) {
