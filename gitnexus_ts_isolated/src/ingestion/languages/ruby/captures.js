...
const RUBY_CALLABLE_CAPTURE_OPTIONS = {
    ...
    extractCallableReference: (node) => {
        ...
        if (method?.text !== 'method' && method?.text !== 'public_method')
            return undefined;
        const args = node.childForFieldName('arguments');
        const symbol = args?.namedChildren.find((child) => child !== null && child.type === 'simple_symbol');
        if (symbol === undefined)
            return undefined;
 ...
 */
function isRubyInstanceIvarWrite(ivarNode) {
    if (ivarNode === undefined)
        return false;
...
export function emitRubyScopeCaptures(sourceText, _filePath, cachedTree) {
    ...
    for (const m of rawMatches) {
        ...
        if (grouped['@type-binding.ivar-field'] !== undefined &&
                        ...
                        if (argList !== null) {
                            for (let ai = 0; ai < argList.namedChildCount; ai++) {
                                ...
                                if (arg !== null && arg.type === 'simple_symbol') {
                                    ...
                                    out.push({
                                        '@declaration.property': syntheticCapture('@declaration.property', arg, propName),
    ...
    for (const assignNode of tree.rootNode.descendantsOfType('assignment')) {
        ...
        if (recvNode === null || methodNode === null)
            continue;
        ...
        out.push({
            '@type-binding.call-return': syntheticCapture('@type-binding.call-return', assignNode, assignNode.text),
        ...
        if (paramMatch) {
            ...
            if (methodNode !== null && paramName && typeName) {
                out.push({
                    ...
                    '@type-binding.type': syntheticCapture('@type-binding.type', methodNode, normalizeYardType(typeName)),
                });
            }
        }
                        ...
                        for (let ai = 0; ai < argList.namedChildCount; ai++) {
                            ...
                            if (arg !== null && arg.type === 'simple_symbol') {
                                ...
                                out.push({
                                    ...
                                    '@type-binding.type': syntheticCapture('@type-binding.type', attrNode, normalizeYardType(typeName)),
                                });
                            }
                        }
                    }
    ...
    for (const methodNode of [
        ...
        if (lastChild.type === 'call') {
            ...
            if (recv !== null &&
                ...
                out.push({
                    '@type-binding.return': syntheticCapture('@type-binding.return', methodNode, `constructor-return: ${recv.text}.new`),
                    '@type-binding.name': syntheticCapture('@type-binding.name', methodNode, methodName),
...
function computeRubyDeclarationArity(fnNode) {
    const params = fnNode.childForFieldName('parameters');
    if (params === null)
        return { parameterCount: 0, requiredParameterCount: 0 };
    let totalCount = 0;
    let requiredCount = 0;
    const paramTypes = [];
...
function computeRubyCallArity(callNode) {
    const argList = callNode.childForFieldName('arguments');
    if (argList === null)
        return 0;
    let count = 0;
 ...
 */
function findFollowingMethod(commentNode) {
    ...
    if (sibling === null)
        return null;
    if (sibling.type === 'method' || sibling.type === 'singleton_method')
        return sibling;
 ...
 */
function findFollowingAttrCall(commentNode) {
    let sibling = commentNode.nextNamedSibling;
    while (sibling !== null && sibling.type === 'comment') {
        sibling = sibling.nextNamedSibling;
    }
 ...
 */
function normalizeYardType(raw) {
