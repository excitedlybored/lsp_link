...
function emitRubyMixinEdges(graph, parsedFiles, nodeLookup) {
    ...
    for (const parsed of parsedFiles) {
        for (const imp of parsed.parsedImports) {
            ...
            const classGraphId = graphIdByName.get(className);
            if (classGraphId === undefined || propName === undefined)
                continue;
            const edgeKey = `${classGraphId}->prop:${propName}`;
            if (emitted.has(edgeKey) || existingProps.has(edgeKey))
                continue;
            emitted.add(edgeKey);
...
function buildRubyMro(graph, parsedFiles, nodeLookup) {
    ...
    const prependByChild = new Map();
    const includeByChild = new Map();
    for (const rel of graph.iterRelationshipsByType('IMPLEMENTS')) {
        const childDefId = defIdByGraphId.get(rel.sourceId);
        const parentDefId = defIdByGraphId.get(rel.targetId);
        if (childDefId === undefined || parentDefId === undefined)
            continue;
 ...
 */
function expandRubyWildcardNames(targetModuleScope, parsedFiles) {
...
export const rubyScopeResolver = {
    ...
    resolveImportTarget: (targetRaw, fromFile, allFilePaths, resolutionConfig) => resolveRubyImportTarget(targetRaw, fromFile, allFilePaths, resolutionConfig),
    expandsWildcardTo: (targetModuleScope, parsedFiles) => expandRubyWildcardNames(targetModuleScope, parsedFiles),
    ...
    emitHeritageEdges: (graph, parsedFiles, nodeLookup) => emitRubyMixinEdges(graph, parsedFiles, nodeLookup),
