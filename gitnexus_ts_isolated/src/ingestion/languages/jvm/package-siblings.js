...
function getMaxInjectedSiblings() {
    const raw = process.env.GITNEXUS_MAX_INJECTED_SIBLINGS;
    if (raw === undefined || raw === '')
        return DEFAULT_MAX_INJECTED_SIBLINGS;
    const parsed = Number(raw);
    return Number.isInteger(parsed) && parsed >= 0 ? parsed : DEFAULT_MAX_INJECTED_SIBLINGS;
}
export function createJvmPackageSiblingVisibility(options) {
    const incompleteFiles = new Set();
    function populateNamespaceSiblings(parsedFiles, indexes, ctx) {
        incompleteFiles.clear();
        const buckets = new Map();
        const unknownPackageFiles = new Set();
        ...
        for (const filePath of ctx.fileContents.keys()) {
            if (!parsedPaths.has(filePath))
                unknownPackageFiles.add(filePath);
        }
        ...
        if (unknownPackageFiles.size > 0) {
            for (const parsed of parsedFiles)
                incompleteFiles.add(parsed.filePath);
            logger.warn(`[${options.languageLabel}-package-siblings] ${unknownPackageFiles.size} file(s) lacked reliable package facts; wildcard attribution disabled for this language workspace`);
        }
        ...
        for (const bucket of buckets.values()) {
            ...
            if (bucket.moduleScopes.length > MAX_PACKAGE_FILES) {
                for (const parsed of bucket.parsed)
                    incompleteFiles.add(parsed.filePath);
                logger.warn(`[${options.languageLabel}-package-siblings] skipping package with ${bucket.moduleScopes.length} files (cap=${MAX_PACKAGE_FILES}); same-package implicit visibility disabled for this package`);
                continue;
            }
            const classDefs = [];
            for (const parsed of bucket.parsed) {
                ...
                for (const scope of parsed.scopes) {
                    if (scope.kind !== 'Class' || scope.parent !== moduleScopeId)
                        continue;
            ...
            const moduleScopeByPath = new Map();
            ...
            for (const sibling of bucket.moduleScopes)
                moduleScopeByPath.set(sibling.filePath, sibling);
            ...
            for (const { filePath, scope } of bucket.moduleScopes) {
                ...
                for (const candidate of candidates) {
                    if (!proximityCache.has(candidate.filePath)) {
                        proximityCache.set(candidate.filePath, sharedSegmentCount(segmentsByPath.get(candidate.filePath) ?? [], ownSegments));
                    }
                }
                ...
                candidates.sort((a, b) => (proximityCache.get(b.filePath) ?? 0) - (proximityCache.get(a.filePath) ?? 0));
                const injectedIds = new Set();
                const injectedPaths = new Set();
                let truncated = false;
                for (const { def, filePath: defPath } of candidates) {
                    if (injectedIds.has(def.nodeId) || def.qualifiedName === undefined)
                        continue;
                    ...
                    if (!scopeAug.has(simpleName))
                        scopeAug.set(simpleName, bindings);
                ...
                const typeBindings = scope.typeBindings;
        ...
        if (truncatedFiles > 0) {
            logger.warn(`[${options.languageLabel}-package-siblings] sibling injection truncated at ${maxInjectedSiblings} per file for ${truncatedFiles} file(s); wildcard attribution disabled for those files (raise or unset GITNEXUS_MAX_INJECTED_SIBLINGS to widen, 0 for unbounded)`);
        }
    }
    return {
...
function sharedSegmentCount(aSegments, bSegments) {
    let index = 0;
