...
import { createLeadingDocDescriptionExtractor, isCjsDefaultExportAssignment, isPrototypeMemberAssignmentNode, } from '../utils/ast-helpers.js';
import { cjsExportedNameFor, isModuleLevelThisAssignment, isModuleLevelThisExport, isShadowedCjsExportAssignmentNode, } from './typescript/cjs-export-assignment.js';
const rootOf = (node) => node.tree?.rootNode;
/** `exports.X = fn` where the module already declares `X` — see #2723. */
const isShadowedCjsExportNode = (node) => {
 ...
 */
const memberAssignmentLabel = (node) => {
    const root = rootOf(node);
    ...
    if (root !== undefined && isModuleLevelThisExport(node, root))
        return 'Function';
    // `module.exports = fn` — the module itself is the callable.
    if (isCjsDefaultExportAssignment(node))
        return 'Function';
    ...
    if (isModuleLevelThisAssignment(node))
        return null;
    if (isPrototypeMemberAssignmentNode(node))
        return 'Method';
    if (isShadowedCjsExportNode(node))
        return null;
    const value = node.childForFieldName('right');
    if (root === undefined || value === null)
        return null;
...
import { emitJsScopeCaptures, interpretJsImport, interpretJsTypeBinding, jsBindingScopeFor, jsImportOwningScope, jsReceiverBinding, jsMergeBindings, jsArityCompatibility, } from './javascript/index.js';
 ...
 * (or to the file).
 */
const tsExtractFunctionName = (node, filePath) => {
...
export const typescriptProvider = defineLanguage({
    ...
    labelOverride: (functionNode, defaultLabel) => defaultLabel !== 'Function'
        ? defaultLabel
