 ...
 */
export function resolveRustImportTarget(targetRaw, fromFile, allFilePaths, _resolutionConfig) {
    if (!targetRaw)
        return null;
...
function findSrcRoot(filePath) {
    ...
    if (srcIdx !== -1)
        return normalized.slice(0, srcIdx + 4); // includes trailing /src
    if (normalized.startsWith('src/'))
        return 'src';
    return '';
}
function resolveModulePath(pathSegments, baseDir, allFilePaths) {
    if (pathSegments.length === 0) {
        const modPath = baseDir ? `${baseDir}/mod.rs` : 'mod.rs';
        if (allFilePaths.has(modPath))
            return modPath;
        return null;
    }
    ...
    if (pathSegments.length >= 2) {
        ...
        if (allFilePaths.has(parentFile))
            return parentFile;
        const parentModFile = baseDir ? `${baseDir}/${parentPath}/mod.rs` : `${parentPath}/mod.rs`;
        if (allFilePaths.has(parentModFile))
            return parentModFile;
    }
    ...
    for (let i = pathSegments.length - 2; i >= 1; i--) {
        ...
        if (allFilePaths.has(prefixFile))
            return prefixFile;
        const prefixModFile = baseDir ? `${baseDir}/${prefix}/mod.rs` : `${prefix}/mod.rs`;
        if (allFilePaths.has(prefixModFile))
            return prefixModFile;
    }
    return null;
}
function resolveWorkspaceCrate(segments, allFilePaths) {
