...
import { buildRustModuleIndex, couldNameAModule, moduleOfDef, moduleOfFile, resolveAnchoredModulePath, sameModule, } from './module-path.js';
/**
 ...
 */
const moduleIndexFor = perFileSet((allFilePaths) => buildRustModuleIndex(allFilePaths));
export function resolveRustQualifiedFreeCall(site, callerParsed, scopes, workspaceIndex, allFilePaths) {
    ...
    if (callerModule === undefined)
        return undefined;
    const anchored = resolveAnchoredModulePath(qualifier, callerModule);
    if (anchored === undefined)
        return undefined;
    for (const targetModule of candidateModules(anchored, qualifier, callerModule, callerParsed, scopes, workspaceIndex, index)) {
        const hit = findMemberInModule(targetModule, site.name, scopes, workspaceIndex, index) ??
 ...
 */
function callerModuleOf(callerParsed, inScope, scopes, index) {
    const fileModule = moduleOfFile(callerParsed.filePath, index);
    if (fileModule === undefined)
        return undefined;
    const inline = [];
    let scopeId = inScope;
    while (scopeId !== null) {
 ...
 */
function* candidateModules(anchored, qualifier, callerModule, callerParsed, scopes, workspaceIndex, index) {
    ...
    for (const edge of scopes.imports.get(callerParsed.moduleScope) ?? []) {
        if (edge.localName !== head || edge.targetFile === null)
            continue;
        const importedModule = moduleOfFile(edge.targetFile, index);
        if (importedModule === undefined)
            continue;
        if (!importNamesModule(edge.targetExportedName, importedModule))
            continue;
        yield {
 ...
 */
function headBoundLocally(callerParsed, name, scopes) {
    if (name === undefined)
        return false;
 ...
 */
function declaresSubmodule(callerParsed, callerModule, name, workspaceIndex, index, scopes) {
    // Inline modules at any depth, keyed by full path.
    const inline = passIndexFor(workspaceIndex, index, scopes).inlineModuleKeys;
    if (inline.has(moduleKey({ crateRoot: callerModule.crateRoot, segments: [...callerModule.segments, name] }))) {
        return true;
    }
 ...
 */
function importNamesModule(targetExportedName, module) {
 ...
 */
function isModuleLevelMember(def, name, workspaceIndex) {
    if (def.namespacePrefix !== undefined && def.namespacePrefix !== '')
        return true;
    const moduleScope = workspaceIndex.moduleScopeByFile.get(def.filePath);
    if (moduleScope === undefined)
        return false;
    for (const ref of moduleScope.bindings.get(name) ?? []) {
        if (ref.origin === 'local' && ref.def.nodeId === def.nodeId)
            return true;
    }
    return false;
}
/** A callable named `name` declared directly in `targetModule`. Refuses on a tie. */
function findMemberInModule(targetModule, name, scopes, workspaceIndex, index) {
    let unique;
    let count = 0;
    for (const defId of scopes.qualifiedNames.get(name)) {
        const def = scopes.defs.get(defId);
        if (def === undefined || !isOverloadableCallable(def.type))
            continue;
        const defModule = moduleOfDef(def.filePath, def.namespacePrefix, index);
        if (defModule === undefined || !sameModule(defModule, targetModule))
            continue;
        if (!isModuleLevelMember(def, name, workspaceIndex))
            continue;
        unique = def;
        count++;
    }
 ...
 */
function findReexportedMember(targetModule, name, scopes, workspaceIndex, index) {
    let unique;
    let count = 0;
    for (const moduleScope of moduleScopesFor(targetModule, workspaceIndex, index, scopes)) {
        for (const edge of scopes.imports.get(moduleScope.id) ?? []) {
            ...
            if (edge.kind !== 'reexport' && edge.kind !== 'alias')
                continue;
            ...
            if (unique !== undefined && resolved.nodeId !== unique.nodeId)
                return undefined;
            unique = resolved;
            count++;
        }
...
function passIndexFor(workspaceIndex, index, scopes) {
    let pass = MODULE_SCOPE_CACHE.get(workspaceIndex);
    if (pass === undefined) {
        const scopesByModule = new Map();
        const knownModuleNames = new Set(index.moduleNames);
        ...
        const inlineModuleKeys = new Set();
        for (const def of scopes.defs.byId.values()) {
            ...
            if (declaringModule !== undefined)
                inlineModuleKeys.add(moduleKey(declaringModule));
        }
        for (const [filePath, moduleScope] of workspaceIndex.moduleScopeByFile) {
            ...
            if (bucket === undefined)
                scopesByModule.set(key, [moduleScope]);
            else
                bucket.push(moduleScope);
        }
...
function moduleScopesFor(targetModule, workspaceIndex, index, scopes) {
    return (passIndexFor(workspaceIndex, index, scopes).scopesByModule.get(moduleKey(targetModule)) ??
        EMPTY_SCOPES);
}
const EMPTY_SCOPES = Object.freeze([]);
/** Follow one re-export edge to the definition it exposes. */
function resolveReexportTarget(edge, name, scopes, workspaceIndex) {
...
function findExportedCallable(targetFile, name, workspaceIndex) {
    const moduleScope = workspaceIndex.moduleScopeByFile.get(targetFile);
    if (moduleScope === undefined)
        return undefined;
    for (const ref of moduleScope.bindings.get(name) ?? []) {
        if (ref.origin === 'local' && isOverloadableCallable(ref.def.type))
            return ref.def;
    }
    return undefined;
}
