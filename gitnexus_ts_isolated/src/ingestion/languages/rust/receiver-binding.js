 ...
 */
export function synthesizeRustReceiverBinding(fnNode, implNode) {
    if (fnNode.type !== 'function_item')
        return null;
    if (implNode === null)
        return null;
 ...
 */
export function getImplTargetType(implNode) {
    if (implNode.type !== 'impl_item')
        return null;
    // Look for `for` keyword — if present, impl is `impl Trait for Type`
    let hasFor = false;
    let typeAfterFor = null;
 ...
 */
export function getImplTraitName(implNode) {
    if (implNode.type !== 'impl_item')
        return null;
    let afterImpl = false;
    for (let i = 0; i < implNode.childCount; i++) {
