 ...
 */
function getOrParseTree(parser, filePath, ctx, store) {
    const cached = (ctx.treeCache?.get(filePath) ?? store?.get(filePath));
    if (cached !== undefined)
        return cached;
...
export function populateRustRangeBindings(parsedFiles, indexes, ctx) {
    const parser = getRustParser();
    const allReturnTypes = new Map();
    const ambiguousReturnTypes = new Set();
    const allFieldTypes = new Map();
    const ambiguousFieldTypes = new Set();
    ...
    const returnTypeByFile = new Map();
    ...
    const treeStore = totalSourceBytes <= TREE_REUSE_SOURCE_BUDGET_BYTES ? new Map() : undefined;
    for (const parsed of parsedFiles) {
    ...
    for (const parsed of parsedFiles) {
        ...
        if (moduleScope === undefined)
            continue;
        processForLoops(tree.rootNode, parsed, scopeMap, moduleScope, allReturnTypes, indexes, returnTypeByFile);
        processPatternBindings(tree.rootNode, parsed, scopeMap, moduleScope);
        processStructDestructuring(tree.rootNode, parsed, scopeMap, moduleScope, allFieldTypes, indexes, fieldTypeByFile);
        processPendingAssignments(tree.rootNode, parsed, parsedFiles, scopeMap, moduleScope, allReturnTypes);
    }
}
function processFieldTypeBindings(root, parsed, scopeMap) {
...
function findScopeForNode(node, parsed, scopeMap) {
...
function normalizeFieldType(text) {
    ...
    if (lastColon !== -1)
        t = t.slice(lastColon + 2);
    return t.trim();
}
/** Get-or-create the inner map for `file` and record `name -> value`. */
function recordByFile(byFile, file, name, value) {
...
function simpleName(qualifiedName, bareName) {
 ...
 */
function resolveImportedDef(name, moduleScope, indexes, byFile) {
    const explicit = uniqueDefs(lookupBindingsAt(moduleScope.id, name, indexes)
    ...
    const defs = explicit.length > 0
        ? explicit
        : uniqueDefs((indexes.imports.get(moduleScope.id) ?? [])
            .filter((e) => e.kind === 'wildcard-expanded' &&
            e.targetFile !== null &&
            byFile.get(e.targetFile)?.has(name) === true)
            .map((e) => ({ file: e.targetFile, name })));
    if (defs.length !== 1)
        return null;
    return byFile.get(defs[0].file)?.get(defs[0].name) ?? null;
}
function processForLoops(root, parsed, scopeMap, moduleScope, allReturnTypes, indexes, returnTypeByFile) {
    for (const forNode of root.descendantsOfType('for_expression')) {
        ...
        if (patternNode === null || valueNode === null)
            continue;
        const varName = extractVarName(patternNode);
        if (varName === null)
            continue;
        const elementType = resolveIterableElementType(valueNode, parsed, scopeMap, moduleScope, allReturnTypes, indexes, returnTypeByFile);
        if (elementType === null)
            continue;
...
function processPatternBindings(root, parsed, scopeMap, moduleScope) {
    for (const nodeType of ['let_condition', 'match_arm']) {
        for (const node of root.descendantsOfType(nodeType)) {
            const patternNode = nodeType === 'let_condition'
...
function processCapturedPattern(patternNode, contextNode, _parsed, scopeMap, moduleScope) {
    ...
    if (varNode === undefined || structPatternNode === undefined)
        return;
...
function processTupleStructPattern(patternNode, contextNode, parsed, scopeMap, moduleScope) {
    ...
    const innerIdent = patternNode.namedChildren.find((c, i) => i > wrapperIdx && c.type === 'identifier');
    if (innerIdent === null || innerIdent === undefined)
        return;
    const varName = innerIdent.text;
    let sourceVarNode = null;
    if (contextNode.type === 'let_condition') {
        sourceVarNode = contextNode.childForFieldName('value');
    }
    else {
        let matchExpr = contextNode.parent;
    ...
    if (sourceVarNode === null || sourceVarNode === undefined)
        return;
...
function processStructDestructuring(root, parsed, scopeMap, moduleScope, allFieldTypes, indexes, fieldTypeByFile) {
    for (const letNode of root.descendantsOfType('let_declaration')) {
        const patternNode = letNode.childForFieldName('pattern');
        if (patternNode === null || patternNode.type !== 'struct_pattern')
            continue;
...
function processIdentityMethodBindings(parsed) {
    for (const scope of parsed.scopes) {
        if (scope.kind !== 'Class')
            continue;
...
function processPendingAssignments(root, parsed, allParsedFiles, scopeMap, moduleScope, allReturnTypes) {
    for (let pass = 0; pass < 3; pass++) {
        for (const letNode of root.descendantsOfType('let_declaration')) {
            ...
            if (varName === null)
                continue;
            const targetScope = findEnclosingFunctionScope(letNode, scopeMap) ?? moduleScope;
            if (targetScope.typeBindings.has(varName))
                continue;
            const valueNode = letNode.childForFieldName('value');
            if (valueNode === null)
                continue;
            ...
            if (valueNode.type === 'field_expression') {
                ...
                if (receiver !== null && field !== null && receiver.type === 'identifier') {
                    const receiverType = lookupTypeInScopes(receiver.text, letNode, parsed, scopeMap, moduleScope);
            ...
            if (valueNode.type === 'call_expression') {
                ...
                if (func !== null && func.type === 'field_expression') {
                    ...
                    if (receiver !== null && method !== null && receiver.type === 'identifier') {
                        const receiverType = lookupTypeInScopes(receiver.text, letNode, parsed, scopeMap, moduleScope);
                        if (receiverType !== null) {
                            const retType = findMethodReturnTypeAcrossFiles(receiverType, method.text, allParsedFiles);
                            if (retType !== null) {
...
function resolveIterableElementType(valueNode, parsed, scopeMap, moduleScope, allReturnTypes, indexes, returnTypeByFile) {
    ...
    if (iterableNode.type === 'call_expression') {
        ...
        if (func.type === 'identifier') {
            ...
            if (crossFileReturn !== undefined)
                return unwrapGeneric(crossFileReturn);
            const rawReturn = lookupRawFunctionReturnType(func.text, valueNode);
            if (rawReturn !== null)
                return unwrapGeneric(rawReturn);
            ...
            if (importedReturn !== null)
                return unwrapGeneric(importedReturn);
            const returnType = lookupReturnTypeInScopes(func.text, parsed, scopeMap, moduleScope);
            if (returnType !== null)
                return unwrapGeneric(returnType);
        }
    }
    return null;
}
function findFieldTypeAcrossFiles(structName, fieldName, allParsedFiles) {
    for (const pf of allParsedFiles) {
        for (const scope of pf.scopes) {
            if (scope.kind !== 'Class')
                continue;
            const hasDef = scope.ownedDefs.some((d) => d.qualifiedName === structName || d.qualifiedName?.endsWith('.' + structName));
            if (!hasDef)
                continue;
...
function findMethodReturnTypeAcrossFiles(structName, methodName, allParsedFiles) {
    for (const pf of allParsedFiles) {
        for (const scope of pf.scopes) {
            if (scope.kind !== 'Class')
                continue;
            ...
            if (tb !== undefined && tb.source === 'return-annotation')
                return tb.rawName;
        }
    }
    return null;
}
function lookupRawFunctionReturnType(funcName, contextNode) {
...
function lookupRawParameterType(paramName, contextNode) {
        ...
        if (current.type === 'function_item') {
            ...
            if (params !== null) {
                for (let i = 0; i < params.namedChildCount; i++) {
                    ...
                    if (param === null || param.type !== 'parameter')
                        continue;
...
function lookupTypeInScopes(name, contextNode, parsed, scopeMap, moduleScope) {
...
function lookupReturnTypeInScopes(funcName, parsed, _scopeMap, moduleScope) {
    const tb = moduleScope.typeBindings.get(funcName);
    if (tb !== undefined && tb.source === 'return-annotation')
        return tb.rawName;
    for (const scope of parsed.scopes) {
        const stb = scope.typeBindings.get(funcName);
        if (stb !== undefined && stb.source === 'return-annotation')
            return stb.rawName;
    }
    return null;
}
function lookupFieldType(structName, fieldName, parsed, _scopeMap, _moduleScope) {
    for (const scope of parsed.scopes) {
        if (scope.kind !== 'Class')
            continue;
        const hasDef = scope.ownedDefs.some((d) => d.qualifiedName === structName || d.qualifiedName?.endsWith('.' + structName));
...
function unwrapGeneric(rawType) {
...
function extractNthGenericArg(rawType, n) {
...
function injectTypeBinding(scope, name, typeName) {
    if (scope.typeBindings.has(name))
        return;
    scope.typeBindings.set(name, {
        rawName: typeName,
        declaredAtScope: scope.id,
        source: 'annotation',
    });
