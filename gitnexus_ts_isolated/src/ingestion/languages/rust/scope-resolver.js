 ...
 */
function emitRustTraitImplEdges(graph, parsedFiles, nodeLookup, scopes, recordTypeArguments) {
    if (scopes === undefined)
        return;
...
function buildRustMro(graph, parsedFiles, nodeLookup) {
    ...
    for (const rel of graph.iterRelationshipsByType('IMPLEMENTS')) {
        ...
        if (existing !== undefined) {
            if (!existing.includes(parentDefId))
                existing.push(parentDefId);
        }
        else {
...
export const rustScopeResolver = {
    language: SupportedLanguages.Rust,
    languageProvider: rustProvider,
    importEdgeReason: 'rust-scope: use',
    resolveImportTarget: (targetRaw, fromFile, allFilePaths, resolutionConfig) => resolveRustImportTarget(targetRaw, fromFile, allFilePaths, resolutionConfig),
    ...
    resolveQualifiedFreeCall: (site, callerParsed, scopes, workspaceIndex, allFilePaths) => resolveRustQualifiedFreeCall(site, callerParsed, scopes, workspaceIndex, allFilePaths),
    buildMro: (graph, parsedFiles, nodeLookup) => buildRustMro(graph, parsedFiles, nodeLookup),
    emitHeritageEdges: (graph, parsedFiles, nodeLookup, scopes, recordTypeArguments) => emitRustTraitImplEdges(graph, parsedFiles, nodeLookup, scopes, recordTypeArguments),
