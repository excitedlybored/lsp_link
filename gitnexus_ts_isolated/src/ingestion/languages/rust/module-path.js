 ...
 */
export function buildRustModuleIndex(allFilePaths) {
    const roots = new Set();
    for (const filePath of allFilePaths) {
        const slash = filePath.lastIndexOf('/');
        const base = slash === -1 ? filePath : filePath.slice(slash + 1);
        if (!CRATE_ROOT_FILES.has(base))
            continue;
    ...
    const standaloneRootFiles = new Map();
    for (const filePath of allFilePaths) {
    ...
    const index = { crateRoots, standaloneRootFiles, moduleNames: new Set() };
    for (const filePath of allFilePaths) {
        const module = moduleOfFile(filePath, index);
        if (module === undefined)
            continue;
        for (const segment of module.segments)
            moduleNames.add(segment);
    }
 ...
 */
export function couldNameAModule(qualifier, knownModuleNames) {
 ...
 */
export function moduleOfFile(filePath, index) {
    ...
    if (standalone !== undefined)
        return { crateRoot: standalone, segments: [] };
    for (const crateRoot of index.crateRoots) {
        const prefix = crateRoot === '' ? '' : `${crateRoot}/`;
        if (crateRoot !== '' && !filePath.startsWith(prefix))
            continue;
        const rel = filePath.slice(prefix.length);
        if (!rel.includes('/') && CRATE_ROOT_FILES.has(rel))
...
export function moduleOfDef(filePath, namespacePrefix, index) {
 ...
 */
export function resolveAnchoredModulePath(qualifier, callerModule) {
    ...
    if (head === 'super') {
        ...
        while (i < qualifier.length && qualifier[i] === 'super') {
            if (base.length === 0)
                return undefined; // above the crate root
            base.pop();
            i++;
        }
 ...
 */
export function sameModule(a, b) {
    return (a.crateRoot === b.crateRoot &&
