...
export function interpretRustTypeBinding(captures) {
    ...
    if (captures['@type-binding.self'] !== undefined) {
        ...
        if (ENUM_VARIANT_NAMES.has(type))
            return null;
        source = 'constructor-inferred';
...
export function normalizeRustTypeName(text) {
    let t = text.trim();
    // Strip reference prefixes (&, &mut, *const, *mut)
    while (t.startsWith('&'))
        t = t.replace(REF_PREFIX_RE, '');
    while (t.startsWith('*'))
        t = t.replace(PTR_PREFIX_RE, '');
...
function extractFirstGenericArg(text) {
...
function normalizeRustCallReturnType(text) {
    let t = text.trim();
...
function normalizeRustReturnType(text) {
    let t = text.trim();
    while (t.startsWith('&'))
        t = t.replace(REF_PREFIX_RE, '');
