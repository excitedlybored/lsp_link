    ...
    for (const m of rawMatches) {
        ...
        if (grouped['@type-binding.type']?.text === 'Self' && tbTypeNode !== undefined) {
            ...
            if (implTypeNode !== null) {
                grouped['@type-binding.type'] = syntheticCapture('@type-binding.type', tbTypeNode, implTypeNode.text);
            }
        }
...
function synthesizeRustInheritanceReferences(root) {
    ...
    walkNamedTree(root, (node) => {
        ...
        const traitArguments = traitField.type === 'generic_type' ? findChild(traitField, 'type_arguments') : null;
        out.push({
