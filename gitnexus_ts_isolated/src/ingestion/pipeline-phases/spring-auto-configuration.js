...
import { SPRING_AUTO_CONFIGURATION_FACTORY_REASON, SPRING_AUTO_CONFIGURATION_IMPORT_REASON, SPRING_AUTO_CONFIGURATION_SYNTHETIC_DESCRIPTION, } from '../frameworks/spring/auto-configuration.js';
import { isDev } from '../utils/env.js';
import { getPhaseOutput } from './types.js';
const MAX_SPRING_METADATA_BYTES = 2 * 1024 * 1024;
const ENABLE_AUTO_CONFIGURATION_KEY = 'org.springframework.boot.autoconfigure.EnableAutoConfiguration';
const AUTO_CONFIGURATION_IMPORTS = 'META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports';
 ...
 */
function hasPathSuffix(filePath, suffix) {
...
export function classifySpringAutoConfigurationMetadata(filePath) {
...
export function parseSpringAutoConfigurationImports(content) {
    const entries = [];
    const seen = new Set();
...
function logicalFactoryLines(content) {
    const logical = [];
    let current = '';
    let startLine = 1;
...
export function parseSpringFactoriesAutoConfigurations(content) {
    const entries = [];
    const seen = new Set();
    for (const logical of logicalFactoryLines(content)) {
        ...
        if (key !== ENABLE_AUTO_CONFIGURATION_KEY)
            continue;
        const value = trimmed
            .slice(separator + 1)
            .replace(/\s+#.*$/, '')
...
function normalizedQualifiedName(value) {
    return value.replaceAll('$', '.');
}
function simpleClassName(qualifiedName) {
...
function sourceClassIndexes(graph) {
    const byQualifiedName = new Map();
    for (const node of graph.iterNodes()) {
        if (node.label !== 'Class')
            continue;
        const qualifiedName = typeof node.properties.qualifiedName === 'string'
...
function resolveOrCreateAutoConfigurationClass(ctx, metadata, qualifiedName, line, classesByQualifiedName) {
...
export const springAutoConfigurationPhase = {
    ...
    async execute(ctx, deps) {
        ...
        for (const scanned of scannedFiles) {
            ...
            metadataFiles++;
            const entries = metadata.kind === 'imports'
            ...
            if (ctx.graph.getNode(fileId) === undefined)
                continue;
            const declaredQualifiedNames = new Set();
            for (const entry of entries) {
                const qualifiedName = normalizedQualifiedName(entry.className);
                if (declaredQualifiedNames.has(qualifiedName))
                    continue;
                ...
                if (autoConfiguration === undefined) {
                    ...
                    autoConfiguration =
                        resolveOrCreateAutoConfigurationClass(ctx, metadata, qualifiedName, entry.line, classesByQualifiedName) ?? null;
                ...
                ctx.graph.addRelationship({
                    id: generateId('DECLARES', `${fileId}->${autoConfiguration.id}:${metadata.kind}:${entry.className}`),
                    sourceId: fileId,
                    targetId: autoConfiguration.id,
                    type: 'DECLARES',
                    confidence: 1,
                    reason: metadata.kind === 'imports'
        ...
        if (isDev && ambiguousQualifiedNames.size > 0) {
            logger.debug(`Spring auto-configuration: skipped ${ambiguousQualifiedNames.size} ambiguous FQN(s) ` +
