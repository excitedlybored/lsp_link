...
function handlerSymbolContent(content, handlerNode) {
...
const TEMPLATE_NAMED_ROUTE_PATTERNS = [
...
function hasRouteParameters(routeUrl) {
    return /\{[^}]+\}/.test(routeUrl);
}
export const isTemplateRouteCandidate = (filePath) => {
...
export function extractTemplateStaticFetchCalls(filePath, content, namedRouteUrls = new Map()) {
    const calls = [];
    const seen = new Set();
    ...
    for (const pattern of TEMPLATE_NAMED_ROUTE_PATTERNS) {
        ...
        while ((match = pattern.exec(content)) !== null) {
            ...
            if (!routeUrl)
                continue;
            if (hasRouteParameters(routeUrl))
                continue;
...
export const routesPhase = {
    ...
    async execute(ctx, deps) {
        ...
        for (const dr of reconcileDispatchGuardRoutes(allDecoratorRoutes)) {
            ...
            if (dr.source === DATA_ROUTE_TABLE_SOURCE && !routeHandlerSymbols.has(routeKey))
                continue;
            addRoute(url, {
        ...
        if (routeRegistry.size > 0) {
            const handlerPathFor = (routeKey, entry) => {
                if (entry.source !== DATA_ROUTE_TABLE_SOURCE)
            ...
            for (const [routeKey, entry] of routeRegistry) {
                ...
                const handlerSymbolId = routeHandlerSymbols.get(routeKey);
                const analysisContent = entry.source === DATA_ROUTE_TABLE_SOURCE && content
                    ? handlerSymbolContent(content, handlerSymbolId ? ctx.graph.getNode(handlerSymbolId) : undefined)
                    : content;
            ...
            if (isDev) {
                logger.info(`🗺️ Route registry: ${routeRegistry.size} routes${duplicateRoutes > 0 ? ` (${duplicateRoutes} duplicate URLs skipped)` : ''}`);
            }
        }
            ...
            if (middlewareCandidates.length > 0) {
                ...
                for (const [mwPath, mwContent] of mwContents) {
                    ...
                    if (isDev && linkedCount > 0) {
                        logger.info(`🛡️ Linked ${mwPath} middleware [${mwLabel.join(', ')}] to ${linkedCount} routes`);
                    }
                }
            }
        }
        ...
        if (htmlCandidates.length > 0 && routeRegistry.size > 0) {
            ...
            for (const [filePath, content] of htmlContents) {
                allFetchCalls.push(...extractTemplateStaticFetchCalls(filePath, content, namedRouteRegistry));
            }
        }
