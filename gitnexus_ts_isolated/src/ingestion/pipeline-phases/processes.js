...
export const processesPhase = {
    ...
    async execute(ctx, deps) {
        ...
        }, { maxProcesses: dynamicMaxProcesses, minSteps: 3 }, outwardActionSites);
        if (isDev) {
            logger.info(`🔄 Process detection: ${processResult.stats.totalProcesses} processes found (${processResult.stats.crossCommunityCount} cross-community)`);
        }
        ...
        const entryPointCandidates = processResult.stats.entryPointsFound + truncation.entryPointCandidatesDropped;
        const flowsMissing = truncation.entryPointCandidatesDropped > 0 ||
            truncation.entryPointsUnexplored > 0 ||
            truncation.processesDropped > 0;
        const shape = `${truncation.entryPointCandidatesDropped} of ${entryPointCandidates} candidate entry point(s) never ranked in, ` +
        ...
        if (flowsMissing) {
            logger.warn({ truncation }, `[processes] ${processResult.stats.totalProcesses} flows reported, but whole flows are MISSING: ` +
                `${shape} An absent flow does NOT mean the code path does not exist.`);
        }
        else if (truncation.truncated) {
            logger.debug({ truncation }, `[processes] ${processResult.stats.totalProcesses} flows reported; every flow found is present, ` +
