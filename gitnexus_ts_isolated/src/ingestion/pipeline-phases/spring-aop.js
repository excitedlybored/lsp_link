...
import { decodeSpringAopReason, encodeSpringAopReason, getSpringAopGraphMetadata, parseSpringAopPointcut, SPRING_AOP_EVIDENCE_DESCRIPTION_PREFIX, springAopPointcutMatches, } from '../frameworks/spring/aop.js';
import { createSpringAopCandidateIndex, } from '../frameworks/spring/aop-candidates.js';
...
function eligibleMethod(node, owner, singletonInstanceClassIds) {
...
function eligibleOwner(node) {
    return node.label === 'Class' || node.label === 'Interface';
}
function addEvidenceNode(ctx, owner, annotation, line, discriminator, description) {
    const evidenceId = generateId('CodeElement', `spring-aop:${owner.id}:${line}:${annotation}:${discriminator}`);
    const evidence = {
        id: evidenceId,
        label: 'CodeElement',
        properties: {
...
function emitBehavior(ctx, record, classMethods, ownerByMethod, singletonInstanceClassIds) {
    ...
    const evidence = addEvidenceNode(ctx, owner, record.annotation, record.line, `behavior:${record.behavior}`, `${record.behavior} interceptor; activation unknown; proxy possible`);
    ...
    const sources = record.ownerKind === 'class' ? [owner, ...(classMethods.get(owner.id) ?? [])] : [owner];
    let edges = 0;
    for (const source of sources) {
        ...
        ctx.graph.addRelationship({
            id: generateId('ADVISED_BY', `${source.id}->${evidence.id}:${record.line}:${record.behavior}`),
            sourceId: source.id,
...
function emitAspect(ctx, record) {
    const owner = ctx.graph.getNode(record.ownerId);
    if (owner === undefined || !eligibleOwner(owner))
        return 0;
    const evidence = addEvidenceNode(ctx, owner, record.annotation, record.line, 'aspect', 'AspectJ aspect marker; registration unknown; activation unknown');
    ctx.graph.addRelationship({
...
function pointcutReason(record, resolved) {
    return {
        kind: 'pointcut',
        annotation: record.annotation,
...
function budgetReached(limit, used) {
    return limit !== 0 && used >= limit;
}
function emitAdvice(ctx, record, aspectClassIds, ownerByMethod, candidateIndex, methodAnnotations, budget) {
    ...
    const evidence = addEvidenceNode(ctx, adviceNode, record.annotation, record.line, `pointcut:${record.advice}:${record.pointcut ?? '<dynamic>'}`, `${record.advice} pointcut ${record.pointcut ?? '<non-static>'}; resolution ${resolved ? 'resolved' : 'unknown'}`);
    ctx.graph.addRelationship({
    ...
    for (const candidate of candidateIndex.candidatesFor(parsed)) {
        if (budgetReached(budget.maxInspectionsPerAdvice, inspections) ||
        ...
        if (candidate.method.id === adviceNode.id)
            continue;
        if (aspectClassIds.has(candidate.owner.id))
            continue;
        if (!springAopPointcutMatches(parsed, candidate.owner, candidate.method, methodAnnotations.get(candidate.method.id))) {
            continue;
        }
        if (budgetReached(budget.maxEdgesPerAdvice, edges) ||
            budgetReached(budget.maxEdges, budget.edges)) {
            truncated = true;
            break;
        }
        ctx.graph.addRelationship({
            id: generateId('ADVISED_BY', `${candidate.method.id}->${adviceNode.id}:${record.line}:${record.advice}`),
...
export const springAopPhase = {
    name: 'springAop',
    deps: ['scopeResolution'],
    async execute(ctx) {
        const metadata = getSpringAopGraphMetadata(ctx.graph);
        if (metadata.aspects.length === 0 &&
        ...
        for (const relationship of ctx.graph.iterRelationshipsByType('HAS_METHOD')) {
            ...
            if (owner === undefined || !eligibleOwner(owner) || method?.label !== 'Method')
                continue;
            ...
            if (!eligibleMethod(method, owner, metadata.singletonInstanceClassIds))
                continue;
        ...
        const budget = {
            maxInspectionsPerAdvice: resolveBudget(ctx.options?.springAopMaxCandidateInspectionsPerAdvice, DEFAULT_SPRING_AOP_MAX_CANDIDATE_INSPECTIONS_PER_ADVICE),
            maxInspections: resolveBudget(ctx.options?.springAopMaxCandidateInspections, DEFAULT_SPRING_AOP_MAX_CANDIDATE_INSPECTIONS),
            maxEdgesPerAdvice: resolveBudget(ctx.options?.springAopMaxAdvisedEdgesPerAdvice, DEFAULT_SPRING_AOP_MAX_ADVISED_EDGES_PER_ADVICE),
            maxEdges: resolveBudget(ctx.options?.springAopMaxAdvisedEdges, DEFAULT_SPRING_AOP_MAX_ADVISED_EDGES),
            inspections: 0,
            edges: 0,
        };
        for (const aspect of metadata.aspects)
            evidenceNodes += emitAspect(ctx, aspect);
        for (const behavior of metadata.behaviors) {
            ...
            if (behavior.ownerKind === 'callable') {
                const annotations = methodAnnotations.get(behavior.ownerId) ?? new Set();
                annotations.add(behavior.annotation);
                methodAnnotations.set(behavior.ownerId, annotations);
            }
            const emitted = emitBehavior(ctx, behavior, classMethods, ownerByMethod, metadata.singletonInstanceClassIds);
        ...
        for (const advice of metadata.advices) {
            const emitted = emitAdvice(ctx, advice, metadata.aspectClassIds, ownerByMethod, candidateIndex, methodAnnotations, budget);
            advisedByEdges += emitted.edges;
            evidenceNodes += emitted.evidence;
            ...
            if (emitted.truncated) {
                truncatedAdvices += 1;
                logger.warn(`[spring-aop] truncated advice ${advice.ownerId}: ` +
...
function sameMethodSignature(left, right) {
    ...
    return !(Array.isArray(leftTypes) &&
 ...
 */
export const springAopInheritancePhase = {
    ...
    async execute(ctx) {
        ...
        for (const relationship of ctx.graph.iterRelationshipsByType('ADVISED_BY')) {
            if (decodeSpringAopReason(relationship.reason)?.kind !== 'behavior')
                continue;
        ...
        for (let cursor = 0; cursor < queue.length; cursor += 1) {
            ...
            if (eligibleOwner(source)) {
                for (const childId of childrenByParent.get(source.id) ?? []) {
                    ...
                    if (child === undefined || !eligibleOwner(child))
                        continue;
                    enqueue(child, item);
                    for (const method of classMethods.get(child.id) ?? [])
                        enqueue(method, item);
                }
                continue;
            }
            ...
            for (const classId of overridingClasses.get(source.id) ?? []) {
                const matches = (classMethods.get(classId) ?? []).filter((method) => sameMethodSignature(method, source));
                const [match] = matches;
