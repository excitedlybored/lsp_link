...
const SPRING_YAML_SCHEMA = yaml.CORE_SCHEMA.withTags(yaml.mergeTag, yaml.timestampTag, yaml.binaryTag, yaml.omapTag, yaml.pairsTag, yaml.setTag);
...
export function classifySpringConfigFile(filePath) {
...
export function parseSpringProperties(content, filePath, profile) {
    const keys = [];
    const seen = new Set();
...
function consumeYamlTraversalBudget(state, depth) {
...
function resolveYamlAliasEvent(event) {
 ...
 */
function findYamlMappingLocation(event, key, traversal) {
    const visited = new Set();
    const stack = [{ event, depth: 0 }];
 ...
 */
function flattenYamlValue(value, event, prefix, out, traversal) {
    const stack = [
    ...
    while (stack.length > 0) {
        ...
        if (trackedObject !== undefined) {
            if (traversal.activeObjects.has(trackedObject))
                continue;
        ...
        if (current !== null &&
            ...
            const entries = current instanceof Set
        ...
        if (currentPrefix.length > 0 && !out.has(currentPrefix))
            out.set(currentPrefix, sourceLine);
    }
}
// js-yaml 5 reports node positions as source offsets; map them to 1-based lines.
function makeLineResolver(source) {
    const lineStarts = [0];
 ...
 */
function buildYamlEventTree(events, source) {
    const lineOf = makeLineResolver(source);
    const anchors = new Map();
    const stack = [];
    const documentRoots = [];
    const anchorName = (start, end) => start >= 0 && end > start ? source.slice(start, end) : null;
    const attach = (node) => {
    ...
    for (const event of events) {
        switch (event.type) {
            ...
            case yaml.EVENT_POP: {
                ...
                if (done !== undefined && stack.length === 0)
                    documentRoots.push(done.children[0]);
                break;
            }
        }
    }
...
export function parseSpringYaml(content, filePath, profile) {
    ...
    documents.forEach((document, index) => flattenYamlValue(document, documentEvents[index], '', flattened, traversal));
...
export const springConfigPhase = {
    name: 'springConfig',
    deps: ['structure'],
