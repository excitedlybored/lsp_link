 ...
 */
import { DI_RESOLVERS, isSupportedLanguage, } from '../di-extractors/index.js';
import { isDev } from '../utils/env.js';
import { logger } from '../../logger.js';
const AMBIGUOUS = Symbol('ambiguous');
function addProvidedType(index, language, typeName, providerId) {
    const byName = index.get(language) ?? new Map();
    const names = new Set([typeName, typeName.slice(typeName.lastIndexOf('.') + 1)]);
    for (const name of names) {
        const providers = byName.get(name) ?? new Set();
...
function addIndexedName(index, node) {
    const qualifiedName = node.properties.qualifiedName;
    if (typeof qualifiedName === 'string') {
        index.byQualifiedName.set(qualifiedName, index.byQualifiedName.has(qualifiedName) ? AMBIGUOUS : node.id);
    }
...
function resolveIndexedName(index, name) {
    if (index === undefined)
        return undefined;
    return name.includes('.') ? index.byQualifiedName.get(name) : index.bySimpleName.get(name);
}
function providerCandidates(ids, providers) {
    const all = [...ids];
...
export const diPhase = {
    ...
    async execute(ctx) {
        ...
        const providedTypes = new Map();
        const providerNames = new Map();
        const providerNodes = new Map();
        const providersByDeclarer = new Map();
        ctx.graph.forEachNode((node) => {
            ...
            if (provider !== null) {
                ...
                const namesByLanguage = providerNames.get(language) ?? new Map();
                for (const name of provider.names) {
                    const namedProviders = namesByLanguage.get(name) ?? new Set();
                ...
                if (provider.declaredByNodeId !== undefined) {
                    const declared = providersByDeclarer.get(provider.declaredByNodeId) ?? new Set();
        ...
        for (const rel of ctx.graph.iterRelationshipsByType('IMPLEMENTS')) {
            const set = interfaceToImplementers.get(rel.targetId) ?? new Set();
            set.add(rel.sourceId);
            interfaceToImplementers.set(rel.targetId, set);
            const supertypes = directSupertypes.get(rel.sourceId) ?? new Set();
        ...
        const memberToClass = new Map();
        for (const relationType of ['HAS_PROPERTY', 'HAS_METHOD']) {
        ...
        const interfacesByLanguage = new Map();
        const classesByLanguage = new Map();
        ctx.graph.forEachNode((node) => {
            if (node.label !== 'Class' && node.label !== 'Interface')
                return;
            const language = node.properties.language;
            if (typeof language !== 'string' || !candidateLanguages.has(language))
                return;
        ...
        const assignableNamesByProvidedType = new Map();
        for (const [providerId, provider] of providers) {
            ...
            if (assignableTypeNames === undefined) {
                const classEntry = resolveIndexedName(classesByLanguage.get(language), provider.providedTypeName);
                const interfaceEntry = resolveIndexedName(interfacesByLanguage.get(language), provider.providedTypeName);
                const rootTypeId = typeof classEntry === 'string' && interfaceEntry === undefined
                    ? classEntry
                ...
                if (rootTypeId !== undefined) {
                    ...
                    while (queue.length > 0) {
                        ...
                        if ((typeNode?.label === 'Class' || typeNode?.label === 'Interface') &&
        ...
        const queueEdge = (edge) => {
            if (edge.sourceId === edge.targetId)
                return;
            const id = `INJECTS:${edge.sourceId}->${edge.targetId}`;
            const existing = pending.get(id);
            if (existing === undefined || edge.confidence > existing.confidence)
                pending.set(id, edge);
        };
        for (const candidate of candidates) {
            const siteNode = ctx.graph.getNode(candidate.siteNodeId);
            const consumerClassId = siteNode?.label === 'Class' ? siteNode.id : memberToClass.get(candidate.siteNodeId);
            if (consumerClassId === undefined)
                continue;
            const edgeSourceId = candidate.edgeSource === 'site' && siteNode !== undefined ? siteNode.id : consumerClassId;
            const classEntry = resolveIndexedName(classesByLanguage.get(candidate.language), candidate.targetTypeName);
            const interfaceEntry = resolveIndexedName(interfacesByLanguage.get(candidate.language), candidate.targetTypeName);
            if (classEntry === AMBIGUOUS ||
            ...
            if (typeof classEntry === 'string')
                structural.add(classEntry);
            if (typeof interfaceEntry === 'string') {
                for (const id of interfaceToImplementers.get(interfaceEntry) ?? [])
                    structural.add(id);
            }
            for (const id of providedTypes.get(candidate.language)?.get(candidate.targetTypeName) ?? []) {
                structural.add(id);
            }
            for (const id of providersByDeclarer.get(edgeSourceId) ?? [])
                structural.delete(id);
        ...
        if (isDev && ambiguousSkipped > 0) {
            logger.debug(`DI: ${ambiguousSkipped} site(s) skipped because requested type names were ambiguous: ${[...ambiguousTypeNames].sort().join(', ')}`);
        }
        if (isDev && (pending.size > 0 || ambiguousInjections > 0)) {
            logger.info(`DI: ${pending.size} INJECTS edges from ${candidates.length} injection sites (${ambiguousInjections} ambiguous single-site resolutions)`);
        }
        return {
