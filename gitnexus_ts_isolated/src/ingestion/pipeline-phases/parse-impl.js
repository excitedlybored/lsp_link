...
export function projectParseHeapNeedBytes(parseableFileCount) {
...
export function shouldAbortForHeapPressure(heapUsedBytes, heapLimitBytes) {
 ...
 */
export function heapPressureRemedy(heapLimitBytes) {
...
export async function runChunkedParseAndResolve(graph, scannedFiles, allPaths, totalFiles, repoPath, pipelineStart, onProgress, options) {
    ...
    if (isDev) {
        ...
        logger.info(`📂 Scan: ${totalFiles} paths, ${totalParseable} parseable (${totalMB.toFixed(0)}MB), ${numChunks} chunks @ ${chunkByteBudget / (1024 * 1024)}MB budget`);
    }
    ...
    const dataRouteFilePaths = new Set(allDecoratorRoutes
        .filter((route) => route.source === DATA_ROUTE_TABLE_SOURCE)
        .map((route) => route.filePath));
    const routeResolutionConfigs = new Map();
    for (const filePath of dataRouteFilePaths) {
        const language = getLanguageFromFilename(filePath);
        if (language === null || routeResolutionConfigs.has(language))
            continue;
        const resolver = SCOPE_RESOLVERS.get(language);
        routeResolutionConfigs.set(language, resolver?.loadResolutionConfig === undefined
            ? undefined
            : await resolver.loadResolutionConfig(repoPath));
    }
    let routeResolutionFiles = allParsedFiles;
    const resolveRouteImportTarget = (parsedImport, fromFile) => {
        const language = getLanguageFromFilename(fromFile);
        if (language === null)
            return null;
        const target = SCOPE_RESOLVERS.get(language)?.resolveImportTarget(parsedImport.targetRaw ?? '', fromFile, routeFilePaths, routeResolutionConfigs.get(language), { parsedFiles: routeResolutionFiles, parsedImport });
    ...
    if (parsedFileStorePath !== undefined && dataRouteFilePaths.size > 0) {
        ...
        for (const parsed of allParsedFiles) {
            if (dataRouteFilePaths.has(parsed.filePath))
        ...
        for (const parsed of importedFiles.values())
            byPath.set(parsed.filePath, parsed);
        routeResolutionFiles = [...byPath.values()];
    }
    ...
    const routeHandlerSymbols = resolveRouteHandlerSymbols(model, allExtractedRoutes, allDecoratorRoutes, {
        ...
        isExportedSymbol: (nodeId) => graph.getNode(nodeId)?.properties.isExported === true,
    });
    return {
        exportedTypeMap,
        allFetchCalls,
