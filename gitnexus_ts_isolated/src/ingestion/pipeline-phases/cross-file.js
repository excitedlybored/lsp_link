...
export const crossFilePhase = {
    ...
    async execute(ctx, deps) {
        const { totalFiles, bindingAccumulator } = getPhaseOutput(deps, 'parse');
        try {
            ...
            if (isDev) {
                if (bindingAccumulator.totalBindings > 0) {
                    ...
                    logger.info(`📦 BindingAccumulator: ${bindingAccumulator.totalBindings} bindings across ${bindingAccumulator.fileCount} files (~${memKB} KB)`);
                }
                else if (totalFiles > 0) {
                    logger.info(`📦 BindingAccumulator: EMPTY — 0 bindings across 0 files despite ${totalFiles} parsed files. If the codebase has typed bindings, this indicates an upstream regression.`);
                }
            }
