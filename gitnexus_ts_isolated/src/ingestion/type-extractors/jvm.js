import { findChild, synthesizeJavaTypeIdentity } from '../utils/ast-helpers.js';
...
const anonymousInitializerTypeName = (declarator) => {
...
const extractJavaDeclaration = (node, env) => {
    ...
    for (let i = 0; i < node.namedChildCount; i++) {
        ...
        if (nameNode) {
            ...
            if (varName)
                env.set(varName, anonymousInitializerTypeName(child) ?? typeName);
        }
    }
};
