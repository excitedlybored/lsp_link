...
function installFlushHook() {
    if (_flushHookInstalled)
        return;
    if (isTruthyEnv(process.env.VITEST))
        return;
 ...
 */
let _prettyAvailable = null;
const _require = createRequire(import.meta.url);
function isPrettyAvailable() {
    if (_prettyAvailable !== null)
    ...
    try {
        ...
        process.stderr.write('[gitnexus:logger] pino-pretty unavailable; falling back to NDJSON on stderr\n');
    }
    return _prettyAvailable;
}
/**
 ...
 */
export function _resetPrettyAvailableCache() {
...
export function _captureLogger(level) {
    ...
    if (_activeDestination !== undefined) {
        throw new Error('_captureLogger: a previous capture is still active — call restore() before starting a new one.');
    }
