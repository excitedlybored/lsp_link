...
function makeEdgeDrafts(parsed, file, hooks, workspace) {
    ...
    const targetFile = hooks.resolveImportTarget(parsed.targetRaw ?? '', file.filePath, workspace, parsed);
    ...
    return targetFiles.map((tf) => {
        const base = {
            ...
            targetFile: tf,
            targetExportedName: extractExportedName(parsed),
            kind: hooks.isNamespaceImport?.(parsed, tf, file.filePath) === true
                ? 'namespace'
                : edgeKindFor(parsed),
            ...typeOnlyFor(parsed),
            ...runsOnlyWhenCalledFor(parsed),
 ...
 */
function carriedPresenceFlags(edge) {
    return {
 ...
 */
function runsOnlyWhenCalledFor(parsed) {
...
function tryFinalize(draft, byFilePath, reexportClosures) {
    ...
    const transitiveVia = draft.source.kind === 'reexport' || viaFiles.length > 1 ? viaFiles : undefined;
    return {
        ...draft.base,
        targetModuleScope: targetModule.moduleScope,
 ...
 * simple name.
 */
function contributesReexportEdge(draft) {
    if (draft.base.kind === 'namespace')
        return false;
    if (draft.source.kind === 'wildcard')
        return true;
 ...
 */
function isNamedReexport(draft) {
    if (draft.base.kind === 'namespace')
        return false;
 ...
 */
function collectAmbiguousReexports(drafts, byFilePath) {
    const firstTarget = new Map();
    const conflicting = new Set();
    for (const draft of drafts) {
        if (!isNamedReexport(draft))
            continue;
        if (draft.source.kind === 'reexport')
            continue; // explicit form: duplicates are illegal upstream
        const targetFile = draft.targetFile;
        if (targetFile === null || !byFilePath.has(targetFile))
            continue;
        const localName = draft.source.localName;
        const seen = firstTarget.get(localName);
        if (seen === undefined)
            firstTarget.set(localName, targetFile);
        else if (seen !== targetFile)
            conflicting.add(localName);
    }
    return conflicting;
}
/**
 ...
 */
function populateFileClosure(filePath, byFilePath, edgeIndex, closures, ambiguousByFile) {
    ...
    for (const draft of drafts) {
        ...
        if (targetModule === undefined)
            continue;
        const localName = draft.source.localName;
        if (ambiguous.has(localName) || myClosure.has(localName))
            continue;
    ...
    for (const draft of drafts) {
        ...
        for (const def of targetModule.localDefs) {
            ...
            if (name === null || ambiguous.has(name) || myClosure.has(name))
                continue;
        ...
        if (targetClosure !== undefined) {
            for (const [name, entry] of targetClosure) {
                if (ambiguous.has(name) || myClosure.has(name))
                    continue;
                myClosure.set(name, {
                    def: entry.def,
...
const VIA_TRUNCATED = '…';
function extendVia(head, inherited) {
    if (inherited.length + 1 <= MAX_VIA_LENGTH)
        return Object.freeze([head, ...inherited]);
...
function indexExportsByName(defs) {
