 ...
 */
import { CircuitOpenError, getBreaker, } from './circuit-breaker.js';
 ...
 * can drift.
 */
export function isTerminalNetworkError(err) {
 ...
 */
export async function resilientFetch(input, init, opts = {}) {
    const fetchImpl = opts.fetchImpl ?? globalThis.fetch;
    const now = opts.now ?? (() => Date.now());
    const breaker = opts.breaker ?? getBreaker(opts.breakerKey ?? defaultBreakerKey(input), opts.breakerOptions);
    const retryConfig = {
    ...
    for (let attempt = 0; attempt < retryConfig.maxAttempts; attempt++) {
        ...
        switch (outcome.kind) {
            ...
            case 'retryable-status':
                ...
                await sleep(computeBackoffMs(attempt, retryConfig.baseDelayMs, retryConfig.capDelayMs, outcome.afterMs, random));
                break;
            case 'retryable-network':
                ...
                await sleep(computeBackoffMs(attempt, retryConfig.baseDelayMs, retryConfig.capDelayMs, undefined, random));
                break;
            default: {
